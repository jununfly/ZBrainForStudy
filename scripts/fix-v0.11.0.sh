...
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/garrytan/zbrain/v0.11.1/scripts/fix-v0.11.0.sh | bash
#
# What it does:
#   1. zbrain init --migrate-only — applies schema v7 without touching config.
#   2. zbrain jobs smoke — fails loudly if Minions isn't healthy.
#   3. Prompts for minion_mode (or defaults to pain_triggered on non-TTY).
#   4. Atomically writes ~/.zbrain/preferences.json (0o600).
#   5. Appends ~/.zbrain/migrations/completed.jsonl with status:"partial" and
...
command -v zbrain >/dev/null 2>&1 || die "zbrain not found on \$PATH. Install it first (\`bun add -g zbrain\` or download a binary)."

ZBRAIN_DIR="${HOME}/.zbrain"
PREFS_PATH="${ZBRAIN_DIR}/preferences.json"
COMPLETED_PATH="${ZBRAIN_DIR}/migrations/completed.jsonl"

mkdir -p "${ZBRAIN_DIR}/migrations"

# ------------------------------------------------------------
# Step 1: schema
# ------------------------------------------------------------
info "[1/8] Applying schema (zbrain init --migrate-only)..."
if ! zbrain init --migrate-only; then
  die "Schema migration failed. Check ~/.zbrain/config.json has a valid database_url (or database_path for PGLite), then re-run."
fi
ok "      schema ok"
...
# Step 2: smoke
# ------------------------------------------------------------
info "[2/8] Running Minions smoke test (zbrain jobs smoke)..."
if ! zbrain jobs smoke; then
...
if [ -t 0 ] && [ -t 1 ]; then
  ...
  warn "      non-interactive shell → defaulting to pain_triggered (change later: \`zbrain config set minion_mode <mode>\`)"
fi
ok "      mode=${MODE}"
...
# ------------------------------------------------------------
info "[4/8] Writing ~/.zbrain/preferences.json..."
...
if [ ${#AGENTS_FOUND[@]} -eq 0 ]; then
  ...
  for F in "${AGENTS_FOUND[@]}"; do
    ...
    echo "            2. The v0.11.1 binary's \`zbrain apply-migrations --yes\` will inject"
...
if [ ${#CRON_FOUND[@]} -eq 0 ]; then
  ...
  for F in "${CRON_FOUND[@]}"; do
    ...
    echo "              extract/backlinks/autopilot-cycle) to zbrain jobs submit"
...
echo ""
echo "      ${GREEN}zbrain autopilot --install${NC}"
echo ""
...
echo "        ${GREEN}zbrain upgrade && zbrain apply-migrations${NC}"
