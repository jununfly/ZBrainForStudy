 ...
 * tests/unit/build-llms.test.ts (asserts paths resolve, content contract holds).
 *
 ...
 *   LLMS_REPO_BASE=https://raw.githubusercontent.com/fork-org/zbrain/main bun run build:llms
 */
...
export const PROJECT = {
  name: "ZBrain",
  summary:
    "ZBrain is a personal knowledge brain and GStack mod for agent platforms. Pluggable engines (PGLite default, Postgres+pgvector for scale), contract-first operations, 26 fat-markdown skills. Teaches agents brain ops, ingestion, enrichment, scheduling, identity, and access control.",
  repoUrl: "https://github.com/garrytan/zbrain",
  rawBaseUrl:
    process.env.LLMS_REPO_BASE ??
    "https://raw.githubusercontent.com/garrytan/zbrain/master",
};
...
export const SECTIONS: DocSection[] = [
  ...
  },
  {
    ...
    entries: [
      ...
      {
        title: "docs/ZBRAIN_RECOMMENDED_SCHEMA.md",
        description:
          "MECE directory structure (people/, companies/, concepts/).",
        path: "docs/ZBRAIN_RECOMMENDED_SCHEMA.md",
      ...
      },
      {
        ...
        description:
          "5-minute walkthrough: fork the bundled pack, add a custom `researcher` type, backfill existing pages via `zbrain schema sync --apply`, prove the T1.5 wiring via `zbrain whoknows` (v0.40.7.0).",
      ...
      },
      {
        ...
        description:
          "Deploying the zbrain jobs worker: crontab + watchdog, inline --follow, systemd/Procfile/fly.toml, upgrade checklist.",
      ...
      },
      {
        ...
        description:
          "Three-tier architecture for agents with 300+ skills: always-loaded, resolver-routed, and dormant. Per-turn token math, the v0.41.7.0 compact list-format resolver, and the `zbrain doctor` safety net. 306 skills, ~21K tokens freed per turn, zero capability loss.",
  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        description:
          "Local reranker via llama.cpp --reranking: Qwen3-Reranker or self-hosted ZE weights, --alias setup, zbrain config keys, cold-start timeout, budget-cap interaction.",
...
export const INLINE_TIPS = [
  "`zbrain doctor [--json] [--fast] [--fix]` - built-in health checks.",
  "`zbrain orphans [--json]` - pages with zero inbound wikilinks.",
  "`zbrain repair-jsonb [--dry-run]` - repair v0.12.0 double-encoded JSONB rows.",
  "`zbrain upgrade` runs post-upgrade + apply-migrations.",
];
