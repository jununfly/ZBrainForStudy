...
# `zbrain rebuild` (v0.32.3) would lose the data because the fence
# wasn't updated.
#
# This script grep-bans the direct-write surface across src/ and
# scripts/ (NOT tests/unit/ — tests legitimately seed fixtures via direct
# inserts, per Codex R2-#8). A function-scoped allow-list lets the
# legitimate extract / reconcile / migration call sites pass: add
# `// zbrain-allow-direct-insert: <reason>` on the SAME LINE as the
...
#   1. $ZBRAIN_SCAN_ROOT explicit override — tests pass this so they
...
if [ -n "${ZBRAIN_SCAN_ROOT:-}" ]; then
  ROOT="$ZBRAIN_SCAN_ROOT"
else
...
# Scan src/ and scripts/ only. tests/unit/ is deliberately excluded per Codex
...
#   2. Does NOT contain the `zbrain-allow-direct-insert:` comment
...
violations=$(
  for dir in "${SCOPE_DIRS[@]}"; do
    ...
    | grep -vE 'zbrain-allow-direct-insert:' \
    | grep -vE ':[[:space:]]*\*[[:space:]]+' \
    | grep -vE ':[[:space:]]*//' \
    | grep -vE '`[^`]*\\.\w+\(' \
    || true
...
if [ -n "$violations" ]; then
  ...
  echo "       carry an explicit \`// zbrain-allow-direct-insert: <reason>\`"
