...
//! `src/core/autopilot/cycle.ts` consolidate phase. Wired to the Rust
//! `run_cycle` orchestrator (Part12 port) via `CyclePhase::Consolidate`.
...
impl MinionHandler for ConsolidateHandler {
    async fn handle(&self, ctx: &MinionJobContext) -> Result<Value> {
        ...
        let opts = CycleOpts {
            ...
            phases: Some(vec![CyclePhase::Consolidate]),
            brain_dir,
...
mod tests {
    ...
    #[tokio::test]
    async fn consolidate_runs_phase() {
        let eng = engine();
