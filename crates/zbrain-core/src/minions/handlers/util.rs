...
//! pointer to `docs/plans/KNOWN-GAPS.md` rather than silently returning
//! `not_implemented`.

use crate::Result;

/// Build the "this job type is unsupported / wontfix" error for an orphaned
/// handler. `gap_id` is the `KNOWN-GAPS.md` row (e.g. `"G80"`) documenting why.
#[must_use]
    ...
    crate::Error::new(
        ...
        &format!(
            "minion job '{name}' is not available: the underlying TS command was removed \
             (option C) and has no Rust replacement. See docs/plans/KNOWN-GAPS.md ({gap_id})."
        ),
    )
}
