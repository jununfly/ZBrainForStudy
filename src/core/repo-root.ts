 ...
 *   - `env_explicit`                 — $ZBRAIN_SKILLS_DIR (operator override; v0.31.7)
 ...
 *   - `repo_root`                    — walked up from cwd, found zbrain repo
 ...
 *
 * Tier 0 ($ZBRAIN_SKILLS_DIR) is safe for both read and write paths because
 ...
 * `export OPENCLAW_WORKSPACE=...; zbrain check-resolvable` run from
 * inside the zbrain repo silently shadowed the env var by walking up
 * to zbrain's own skills/. Explicit env should win. Unset env → behavior
 ...
 * a write-path install-path fallback would let `zbrain skillpack install`
 * from `~` silently target the bundled zbrain repo's skills/ instead of the
 * user's workspace.
 *
...
export function autoDetectSkillsDir(
  ...
  if (env.ZBRAIN_SKILLS_DIR) {
    const explicit = isAbsolute(env.ZBRAIN_SKILLS_DIR)
      ? env.ZBRAIN_SKILLS_DIR
      : resolvePath(startDir, env.ZBRAIN_SKILLS_DIR);
  ...
  //     && zbrain skillpack scaffold X` finds the agent repo, not an
 ...
 * walks up from this module's install location to find a zbrain repo root,
 ...
 * `zbrain skillpack install` from `~` silently target the bundled zbrain
 ...
 * github:garrytan/zbrain && cd ~ && zbrain doctor`) without expanding the
...
export function autoDetectSkillsDirReadOnly(
  ...
  // that vendored zbrain in a subdir).
  try {
 ...
 * `autoDetectSkillsDirReadOnly` adds for `zbrain doctor` /
 * `zbrain check-resolvable` / `zbrain routing-eval`.
 */
export const AUTO_DETECT_HINT_READ_ONLY = [
  AUTO_DETECT_HINT,
  `  7. (read-only) walk up from zbrain's install path`,
].join('\n');
