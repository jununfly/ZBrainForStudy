/**
 * skillpack/harvest.ts — `zbrain skillpack harvest <slug> --from <host-repo-root>`.
 *
 ...
 * symlink could leak secrets into zbrain's source tree.
 *
...
export function runHarvest(opts: HarvestOptions): HarvestResult {
  ...
  const zbrainSkillDir = join(opts.zbrainRoot, 'skills', opts.slug);
  if (existsSync(zbrainSkillDir) && !opts.overwriteLocal) {
    throw new HarvestError(
      `Slug collision: zbrain already has skills/${opts.slug}/. Pass --overwrite-local to replace.`,
      'slug_collision',
    );
  }
  ...
  for (const item of walkSourceDir(hostSkillDir, zbrainSkillDir)) {
    items.push(item);
  }
  for (const src of pairedSources) {
    items.push({
      source: join(opts.hostRepoRoot, src),
      target: join(opts.zbrainRoot, src),
    });
  }
  ...
  if (!opts.noLint && !dryRun) {
    try {
      ...
      if (err instanceof PrivacyLintError) {
        ...
        rollbackHarvest(zbrainSkillDir, pairedItems.map(i => i.target));
        return {
          status: 'lint_failed',
          slug: opts.slug,
          hostSkillDir,
  ...
  if (!dryRun) {
    manifestUpdated = addToBundleManifest(opts.zbrainRoot, opts.slug);
  }
...
function rollbackHarvest(zbrainSkillDir: string, pairedTargets: string[]): void {
  try {
    if (existsSync(zbrainSkillDir)) {
      rmSync(zbrainSkillDir, { recursive: true, force: true });
    }
  } catch {}
 ...
 */
export function addToBundleManifest(zbrainRoot: string, slug: string): boolean {
  const manifestPath = join(zbrainRoot, PLUGIN_JSON);
