/**
 * skillpack/doctor.ts — `zbrain skillpack doctor` runner.
 *
...
export async function runDoctor(opts: DoctorOptions): Promise<DoctorResult> {
  ...
  // Log the run for the zbrain doctor activity surface.
  logSkillpackEvent({
    event: 'doctor_run',
    pack: manifest.name,
...
function buildResult(
  ...
  return {
    ...
    full_mode_hint:
      opts.mode === 'full'
        ? '--full mode runs the publish-gate test + LLM-judge + routing-eval suites in a sandbox. Implementation lands in a follow-up wave; for now use `zbrain skillpack test <pack-dir>` once W4 ships.'
        : null,
  };
}
  ...
  for (const dim of score.dimensions) {
    ...
    switch (dim.name) {
      ...
      case 'unit_tests_present': {
        const target = join(packRoot, 'tests/unit/example.test.ts');
      ...
      case 'bootstrap_runbook_present': {
        ...
        if (!existsSync(path)) {
          const stub = [
            ...
            '2. (edit me) agent: zbrain put_page wiki/_bootstrap-stub --frontmatter type=stub',
            '',
            '<!-- v0.36 contract: zbrain displays this post-scaffold but DOES NOT auto-execute. -->',
            '',
          ].join('\n');
