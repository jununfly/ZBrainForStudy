...
export const llamaServerReranker: Recipe = {
  ...
  touchpoints: {
    reranker: {
      ...
      // by the user via `zbrain config set search.reranker.model
  ...
  setup_hint:
    ...
    'path). Then `zbrain config set search.reranker.model ' +
    'llama-server-reranker:<short-id>` and `zbrain config set ' +
