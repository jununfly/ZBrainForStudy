 ...
 * doctor scan.
 *
 * File: `~/.zbrain/audit/backpressure-YYYY-Www.jsonl` (override dir via
 * `ZBRAIN_AUDIT_DIR` for container/sandbox deployments where `$HOME` is read-only).
 *
 * `zbrain jobs stats` will surface coalesce counts from this file in a v0.19.2+
...
/** Honors `ZBRAIN_AUDIT_DIR` for container/sandbox deployments. */
