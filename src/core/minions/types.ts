/**
 * Minions — BullMQ-inspired Postgres-native job queue for ZBrain.
 *
 * Usage:
...
export interface MinionWorkerOpts {
   ...
   *  Automatically disabled when running under a supervisor (ZBRAIN_SUPERVISED=1).
   *  Provides DB liveness probes and stall detection for bare `zbrain jobs work`
...
export interface SubagentHandlerData {
   ...
   * `zbrain agent run` does not yet accept a `--brain` flag that would
