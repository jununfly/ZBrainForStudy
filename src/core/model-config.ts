 ...
 *   5. Env var (process.env[envVar] or ZBRAIN_MODEL)
...
export interface ResolveModelOpts {
  ...
  /** Env var to consult after global default. Defaults to `ZBRAIN_MODEL`. */
  envVar?: string;
  /**
 ...
 *
 * Users override via `zbrain config set models.tier.<tier> <model>`.
 */
...
export async function resolveModel(
  ...
  const envVar = opts.envVar ?? 'ZBRAIN_MODEL';
