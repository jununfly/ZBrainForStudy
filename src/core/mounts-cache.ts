 ...
 * host's checked-in `skills/RESOLVER.md`, zbrain publishes an aggregated
 * `~/.zbrain/mounts-cache/RESOLVER.md` + `manifest.json` whenever mounts
...
function getMountsCacheDir(): string {
  return join(homedir(), '.zbrain', 'mounts-cache');
}
...
export function renderResolverMarkdown(composed: ComposedResolver): string {
  const lines: string[] = [];
  lines.push('# ZBrain Skill Resolver (aggregated)');
  lines.push('');
  lines.push('Auto-generated by `zbrain mounts add|remove|sync`. Do not edit by hand.');
...
/**
 * Write the aggregated cache to ~/.zbrain/mounts-cache/. Safe to call
...
export function writeMountsCache(
  ...
  // Unique tmp names per call so concurrent `zbrain mounts add|remove`
...
/** Remove the aggregated cache dir. Called by `zbrain mounts remove` and tests. */
