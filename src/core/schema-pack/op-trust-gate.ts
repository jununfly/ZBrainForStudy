...
export function validateSchemaPackTrustGate(
  ...
  if (ctx.remote !== false) {
    throw new SchemaPackTrustGateError(
      ...
      'Pass via zbrain.yml `schema:` section, ~/.zbrain/config.json `schema_pack`, ' +
      'ZBRAIN_SCHEMA_PACK env var, or `zbrain config set schema_pack <name>`. ' +
    ...
    if (scope.sourceIds.length === 1) {
      ...
      for (const sid of scope.sourceIds) {
        const res = resolveActivePackName({
          ...
          envVar: process.env.ZBRAIN_SCHEMA_PACK?.trim() || undefined,
          sourceId: sid,
