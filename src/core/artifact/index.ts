...
// `.zbrain-schema` is a fan-out hazard. The structural fix is one
 ...
 *   - .zbrain-schema or .zbrain-skillpack file extension (tarball)
 *   - directory with pack.yaml + api_version 'zbrain-schema-pack-v1' (schemapack)
 *   - directory with skillpack.json + api_version 'zbrain-skillpack-v1' (skillpack)
 *
 * Returns null on unrecognized input.
 */
export function detectArtifactKind(path: string): ArtifactKind | null {
  if (path.endsWith('.zbrain-schema')) return 'schemapack';
  if (path.endsWith('.zbrain-skillpack')) return 'skillpack';
  ...
  try {
    ...
    if (existsSync(join(path, 'pack.yaml'))) {
      ...
      if (raw.includes('zbrain-schema-pack-v1')) return 'schemapack';
    }
...
/**
 * Install-target directory by kind. Both kinds land under ~/.zbrain/
 * but at distinct subdirectories so doctor + uninstall can scope
 * cleanly.
 */
export function targetDirForKind(kind: ArtifactKind, zbrainHome: string): string {
  return kind === 'schemapack'
    ? join(zbrainHome, 'schema-packs')
    : join(zbrainHome, 'skillpacks');
}

/**
 * Validate a manifest by kind. For schemapack: shape check against
 * SchemaPackManifest v1. For skillpack: shape check against
 * zbrain-skillpack-v1. Throws on validation failure with a
 * descriptive message.
 */
export function validateManifestByKind(kind: ArtifactKind, manifest: unknown): void {
  if (kind === 'schemapack') {
    ...
    if (m.api_version !== 'zbrain-schema-pack-v1') {
      throw new Error(`schemapack manifest api_version must be "zbrain-schema-pack-v1"; got ${JSON.stringify(m.api_version)}`);
    }
    return;
  }
  if (kind === 'skillpack') {
    ...
    const m = manifest as { api_version?: unknown };
    if (m.api_version !== 'zbrain-skillpack-v1') {
      throw new Error(`skillpack manifest api_version must be "zbrain-skillpack-v1"; got ${JSON.stringify(m.api_version)}`);
    }
    return;
  }
...
export function installArtifact(
  ...
  const targetParent = targetDirForKind(desc.kind, zbrainHome);
 ...
 */
export function listInstalledArtifacts(kind: ArtifactKind, zbrainHome: string): string[] {
  const dir = targetDirForKind(kind, zbrainHome);
