/**
 * zbrain backfill — first-class bulk operations (v0.30.1 Fix 3).
 *
 ...
 *
 * Usage:
 *   zbrain backfill <kind> [--batch-size N] [--concurrency N] [--resume]
 *                          [--dry-run] [--keep-index] [--max-errors N]
 *   zbrain backfill list
 *
 * X5: --concurrency clamps to ZBRAIN_DIRECT_POOL_SIZE - 1 with a warning,
...
function printHelp(): void {
  console.log(`zbrain backfill — first-class bulk operations.

Usage:
  zbrain backfill <kind> [flags]      Run a registered backfill.
  zbrain backfill list                 Show registered backfills + checkpoints.
...
Flags:
  ...
  --concurrency N    Parallel batches; clamped to ZBRAIN_DIRECT_POOL_SIZE - 1
...
function clampConcurrency(requested: number | undefined): { effective: number; warning?: string } {
  ...
  if (requested > ceiling) {
    return {
      effective: ceiling,
      warning: `[backfill] --concurrency ${requested} clamped to ${ceiling} (pool size ${poolSize}, reserved 1 for HNSW/heartbeat). Bump ZBRAIN_DIRECT_POOL_SIZE if you need more concurrency.`,
    };
  }
...
export async function runBackfillCommand(args: string[]): Promise<void> {
  ...
  if (!cli.kind) {
    console.error('Usage: zbrain backfill <kind> [flags]   |   zbrain backfill list');
    process.exit(2);
  }

  const reg = getBackfill(cli.kind);
  if (!reg) {
    console.error(`No backfill registered with name "${cli.kind}". Run \`zbrain backfill list\`.`);
    process.exit(2);
  }
