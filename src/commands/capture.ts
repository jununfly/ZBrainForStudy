/**
 * zbrain capture — the single human-facing entrypoint for getting content
 ...
 *
 *   zbrain capture "thought to remember"
 *   zbrain capture --file ./notes/2026-05-20.md
 *   echo "from stdin" | zbrain capture --stdin
 *   zbrain capture "..." --slug inbox/specific
 *   zbrain capture "..." --quiet           # slug-only output for pipelines
 *
 * Behavior:
 *   - Local install: writes to ~/.zbrain/inbox/<slug>.md OR routes through
...
const HELP = `Usage: zbrain capture [content] [options]
...
Modes (mutually exclusive — first match wins):
  zbrain capture "thought"          inline content
  zbrain capture --file PATH        read content from a file
  zbrain capture --stdin            read content from stdin (piped)
...
Options:
  ...
  --source ID          Multi-source brains: write under a non-default source.
                       Resolution: --source flag > ZBRAIN_SOURCE env >
                       .zbrain-source dotfile (walk-up) > local_path >
...
Examples:
  zbrain capture "remember to follow up on the X deal"
  echo "from a pipe" | zbrain capture --stdin
  zbrain capture --file ./notes/today.md --slug daily/2026-05-20
  JOB=$(zbrain capture "..." --quiet)
`;
...
export async function runCapture(engine: BrainEngine | null, args: string[]): Promise<void> {
  ...
  if (parsed.source && isThinClient(cfg)) {
    console.error(`zbrain capture: --source is not supported on thin-client installs.`);
    console.error(`Server-side OAuth client registration determines source scope.`);
    console.error(`On the server, run:`);
    console.error(`  zbrain auth register-client <name> --source ${parsed.source} --scopes "read write"`);
    process.exit(1);
  }
  ...
  if (parsed.stdin) {
    ...
    try {
      ...
      console.error(
        `zbrain capture: failed to read ${parsed.filePath}: ${e instanceof Error ? e.message : String(e)}`,
      );
      process.exit(1);
    }
    ...
    console.error('zbrain capture: provide content positionally, --file PATH, or --stdin');
    console.error('Run `zbrain capture --help` for examples.');
    process.exit(1);
  }
  ...
  if (nullByteOffset !== -1) {
    console.error(
      `zbrain capture: refusing to capture binary content from ${inputLabel}\n` +
  ...
  if (normalizedBody.length === 0) {
    console.error('zbrain capture: refusing to capture empty content');
    process.exit(1);
  }
  ...
  if (!isThinClient(cfg) && engine) {
    try {
      ...
      console.error(`zbrain capture: ${e instanceof Error ? e.message : String(e)}`);
      process.exit(1);
    }
  }
  ...
  if (isThinClient(cfg)) {
    ...
    try {
      ...
      if (hint) {
        console.error(`zbrain capture: ${hint}`);
      } else if (e instanceof RemoteMcpError) {
        console.error(`zbrain capture: remote put_page failed: ${e.message}`);
        console.error('Run `zbrain remote doctor` to diagnose the connection.');
      } else {
        console.error(
          `zbrain capture: remote put_page failed: ${e instanceof Error ? e.message : String(e)}`,
        );
        console.error('Run `zbrain remote doctor` to diagnose the connection.');
      }
      process.exit(1);
  ...
  if (!engine) {
    console.error('zbrain capture: engine not connected');
    process.exit(1);
  }
  const putPageOp = operations.find((o) => o.name === 'put_page');
  if (!putPageOp) {
    console.error('zbrain capture: put_page operation missing (zbrain build issue)');
    process.exit(1);
  }
  ...
  try {
    ...
    if (hint) {
      ...
      console.error(
        `zbrain capture: put_page failed: ${e instanceof Error ? e.message : String(e)}`,
      );
    }
    process.exit(1);
  }
