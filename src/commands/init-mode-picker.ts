 ...
 *
 * Runs as a phase inside `zbrain init` AFTER `engine.initSchema()` so DB
 ...
 * `balanced` and prints the one-line hint pointing at `zbrain config set
...
The "cost" isn't zbrain itself — it's the downstream agent's input cost
reading the retrieved chunks back into its context window. zbrain's own
...
You can change this any time with: zbrain config set search.mode <mode>
Per-knob tuning + recommendation engine ships at: zbrain search tune
`;
...
export async function runModePicker(
  ...
  if (!process.stdin.isTTY) {
    ...
    console.log('');
    console.log('═══════════════════════════════════════════════════════════════');
    console.log(`[zbrain] search mode tentatively set to: ${rec.mode}`);
    console.log(`[zbrain] Reason: ${rec.reason}`);
    console.log('═══════════════════════════════════════════════════════════════');
    console.log('');
    ...
    console.log('[AGENT] v0.31.x retrieval shape. Run `zbrain config set search.mode');
    ...
    console.log('');
    console.log('To change later: zbrain config set search.mode <mode>');
    console.log('To see what is running: zbrain search modes');
    console.log('');
    return rec.mode;
  }
  ...
  try {
    ...
    console.error(`[zbrain] WARN: failed to persist search.mode (${(err as Error).message ?? 'unknown'}). Defaulting to ${picked} at search-time.`);
  }
