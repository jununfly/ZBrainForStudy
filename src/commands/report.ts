/**
 * zbrain report — Save a structured report to brain/reports/.
 *
 ...
 *
 * Usage:
 *   zbrain report --type enrichment-sweep --title "Enrichment Sweep" --content "..."
 *   echo "report body" | zbrain report --type meeting-sync --title "Meeting Sync"
 *   zbrain report --type enrichment-sweep --dir /path/to/brain
 */
...
export async function runReport(args: string[]) {
  ...
  if (!reportType) {
    console.error('Usage: zbrain report --type <name> --title "..." --content "..." [--dir <brain>]');
    console.error('  Or pipe content via stdin:');
    console.error('    echo "report body" | zbrain report --type meeting-sync --title "Daily Sync"');
    console.error('');
