/**
 * v0.32.3 — `zbrain eval run-all` orchestrator.
 *
 * Sweeps every requested mode × suite combination and writes per-run
 * results to `<repo>/.zbrain-evals/eval-results.jsonl` [CDX-23]. Personal
 * brain (~/.zbrain) is never touched; the repo's git history is the
 * audit trail.
 *
...
function printHelp(): void {
  process.stderr.write(
    `zbrain eval run-all [flags]\n\n` +
    `Sweeps every requested search-lite mode × eval suite. Writes per-run results to\n` +
    `<repo>/.zbrain-evals/eval-results.jsonl. Personal brain is never touched.\n\n` +
    `Flags:\n` +
    ...
    `  --output DIR                  Override .zbrain-evals/ location.\n` +
...
function evalResultsPath(repoRoot: string, outputDirOverride?: string): string {
  ...
  return join(repoRoot, '.zbrain-evals', 'eval-results.jsonl');
}
...
export async function runEvalRunAll(_engine: BrainEngine | null, args: string[]): Promise<void> {
  ...
  // responsibility today — `zbrain eval run-all` is the orchestrator's
  // shape + cost guard + audit trail. The per-suite per-mode calls land
  // as a follow-up: each suite's CLI is already exposed (zbrain eval
  // longmemeval --mode X, zbrain eval replay --mode X), so wiring them
