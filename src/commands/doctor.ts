...
export interface Check {
   ...
   * `zbrain doctor --remediation-plan` / `--remediate`. Optional and
 ...
 *   - zbrain doctor --json (CLI)
 *   - run_doctor MCP op (remote callers)
 *   - zbrain remote doctor (renders this from the MCP op response)
 *
...
/**
 * Focused doctor for `run_doctor` MCP op + `zbrain remote doctor` CLI.
 *
 ...
 */
const WHOKNOWS_FIXTURE_RELATIVE_PATH = 'tests/unit/fixtures/whoknows-eval.jsonl';
...
export function resolveWhoknowsFixturePath(
  ...
  if (env.ZBRAIN_WHOKNOWS_FIXTURE_PATH) {
    return isAbsolute(env.ZBRAIN_WHOKNOWS_FIXTURE_PATH)
      ? env.ZBRAIN_WHOKNOWS_FIXTURE_PATH
      : resolvePath(process.cwd(), env.ZBRAIN_WHOKNOWS_FIXTURE_PATH);
  }
 ...
 * not the eval gate outcome (that runs via `zbrain eval whoknows`).
 *
 ...
 * regression detection lives in `zbrain eval whoknows --json` and is
  ...
  try {
    ...
    if (!fixturePath) {
      return {
        ...
        status: 'warn',
        message: 'whoknows eval fixture path could not be resolved. Set ZBRAIN_WHOKNOWS_FIXTURE_PATH to the absolute path for tests/unit/fixtures/whoknows-eval.jsonl.',
      };
    }
    ...
    return {
      name: 'whoknows_health',
      status: 'ok',
      message: `whoknows eval fixture present (${rows.length} queries). Run \`zbrain eval whoknows tests/unit/fixtures/whoknows-eval.jsonl\` to grade.`,
    };
  } catch (e) {
  ...
  try {
    ...
    if (ratio > 0.10) {
      return {
        ...
        status: 'fail',
        message: `${offGrid}/${total} takes off the 0.05 grid (${(ratio * 100).toFixed(1)}%). Fix: zbrain apply-migrations --yes`,
      };
    }
    if (ratio > 0.01) {
      return {
...
export async function doctorReportRemote(engine: BrainEngine): Promise<DoctorReport> {
  ...
  try {
    ...
    if (version >= LATEST_VERSION) {
      ...
      checks.push({
        name: 'schema_version',
        status: 'fail',
        message: `No schema version recorded. Migrations never ran. Run \`zbrain apply-migrations --yes\` on the host.`,
      });
    } else {
      checks.push({
        name: 'schema_version',
        status: 'warn',
        message: `Version ${version}, latest is ${LATEST_VERSION}. Run \`zbrain apply-migrations --yes\` on the host.`,
      });
    }
  } catch {
  ...
  try {
    ...
    if (wedged.length > 0) {
      const cmd = wedged.map(v => `zbrain apply-migrations --force-retry ${v}`).join(' && ');
      checks.push({
      ...
      checks.push({
        name: 'minions_migration',
        status: 'fail',
        message: `MINIONS HALF-INSTALLED on brain host: ${stuck.join(', ')}. Run on the host: zbrain apply-migrations --yes`,
      });
    }
  } catch {
  ...
  try {
    ...
    const { zbrainPath } = await import('../core/config.ts');
    const path = zbrainPath('sync-failures.jsonl');
    let unacked = 0;
    if (existsSync(path)) {
    ...
    checks.push({
      ...
      message: unacked === 0
        ? 'No unacked failures'
        : `${unacked} unacked failure(s) — run \`zbrain sync --skip-failed\` on the host to acknowledge`,
    });
  } catch {
      ...
      if (result.walk_truncated) {
        ...
        checks.push({
          ...
          message:
            ...
            `Verify on the brain host: \`zbrain sources status\` then \`zbrain sync --source <id> --full\`.`,
        });
      } else {
        checks.push({
          name: 'multi_source_drift',
    ...
    try {
      ...
      checks.push({
        ...
        message: stalled === 0
          ...
          : `${stalled} active job(s) stalled > 1h — \`zbrain jobs cancel <id>\` or \`zbrain jobs retry <id>\` on the host`,
      });
    } catch {
  ...
  // buildChecks). Surfaces when ZBRAIN_EMBEDDING_* env vars disagree
  ...
  // so the user sees it at the next `zbrain doctor` run instead of at the
  ...
  // ~/.zbrain/audit/rerank-failures-*.jsonl. Failure-only (no success
 ...
 *
 * Reads `~/.zbrain/audit/synopsis-failures-YYYY-Www.jsonl` via
  ...
  try {
    ...
    const fixHint =
      chunkerDrift > 0 || modeNull > 0
        ? ` Run \`zbrain reindex --markdown\` to align.`
        : '';
    return {
      name: 'contextual_retrieval_coverage',
...
export async function checkAbandonedThreads(engine: BrainEngine): Promise<Check> {
  try {
    ...
    return {
      name: 'abandoned_threads',
      status: 'ok',
      message: `${count} high-conviction take(s) older than 12 months and never revisited — see \`zbrain calibration\` for details`,
    };
  } catch (e) {
    return {
      name: 'abandoned_threads',
  ...
  try {
    ...
    if (ageDays > staleDays) {
      return {
        name: 'calibration_freshness',
        status: 'warn',
        message: `Calibration profile is ${ageDays} days old (stale at >${staleDays}d). Run \`zbrain calibration --regenerate\``,
      };
    }
    return {
      name: 'calibration_freshness',
  ...
  try {
    ...
    if (bounces >= 1000) {
      return {
        name: 'subagent_health',
        status: 'fail',
        message: `${bounces} lease-pressure bounces in last 24h — this is blocking real work. Raise the cap: \`export ZBRAIN_ANTHROPIC_MAX_INFLIGHT=64\` (or \`unlimited\` for Azure / Bedrock / self-hosted upstreams with no provider-side rate limit). After raising, restart \`zbrain jobs work\`.`,
      };
    }
    ...
    if (bounces >= 100 && completed === 0) {
      return {
        name: 'subagent_health',
        status: 'warn',
        message: `${bounces} lease-pressure bounces in last 24h with no completed subagent jobs — cap is too tight. Raise via \`export ZBRAIN_ANTHROPIC_MAX_INFLIGHT=64\` (or \`unlimited\` for upstreams with no provider-side cap).`,
      };
    }
    return {
      name: 'subagent_health',
 ...
 *   2) Walk last 7 days of `~/.zbrain/audit/rerank-failures-*.jsonl`.
  ...
  try {
    ...
    if (authFails.length > 0) {
      return {
        name: 'reranker_health',
        status: 'warn',
        message: `${authFails.length} reranker auth failure(s) in last 7 days. Fix: verify ZEROENTROPY_API_KEY and run \`zbrain models doctor\`.`,
      };
    }
 ...
 *              sparse to fire often; fix: `zbrain extract all` to
...
export async function checkGraphSignalsCoverage(engine: BrainEngine): Promise<Check> {
  try {
    ...
    if (cfgVal !== null && cfgVal !== undefined) {
      ...
      // this, `zbrain config set search.graph_signals TRUE` enables
    ...
    if (pct < 10) {
      return {
        name: 'graph_signals_coverage',
        status: 'warn',
        message: `graph_signals enabled but only ${pctStr}% of pages have inbound links (<10%). Signal will rarely fire. Fix: \`zbrain extract all\` to populate the link graph from frontmatter + markdown.`,
      };
    }
 ...
 *
 * Surfaces three readiness signals for `zbrain brainstorm` / `zbrain lsd`:
 *
 ...
 *      fallback only). Fix: `zbrain apply-migrations --yes`.
 *
 ...
 *      Fix: `zbrain config set search.track_retrieval true`.
 *
 ...
 *      Fix: `zbrain calibration --regenerate` once enough takes are resolved.
 *
  ...
  try {
    ...
    if (!columnPresent) {
      return {
        ...
        status: 'warn',
        message: `pages.last_retrieved_at column missing. LSD stale-bias degraded to corpus-sampling. Fix: \`zbrain apply-migrations --yes\``,
      };
    }
  } catch (e) {
  ...
  try {
    ...
    if (trackCfg === 'false' || trackCfg === '0' || trackCfg === 'off' || trackCfg === 'no') {
      return {
        name: 'brainstorm_health',
        status: 'warn',
        message: `search.track_retrieval is explicitly off — LSD's stale-page signal never accumulates. Fix: \`zbrain config set search.track_retrieval true\` (or accept and use brainstorm only).`,
      };
    }
  } catch {
  ...
  try {
    ...
    if (!Array.isArray(tags) || tags.length === 0) {
      return {
        name: 'brainstorm_health',
        status: 'ok',
        message: `Migration v79 applied; tracking enabled. Calibration cold-start (no active_bias_tags) — judge runs unbiased. Fix when ready: \`zbrain calibration --regenerate\`.`,
      };
    }
    return {
      name: 'brainstorm_health',
 ...
 * `zbrain models doctor` probe covers that, and we don't want every
 * `zbrain doctor` run to spend tokens. Surfaces a paste-ready fix when the
 * key is missing.
 */
export async function checkZeEmbeddingHealth(engine: BrainEngine): Promise<Check> {
  try {
    ...
    // File plane: zeroentropy_api_key on ZBrainConfig (added by C.3).
    ...
    if (!envKey && !fileKey) {
      return {
        ...
        message:
          ...
          `\`export ZEROENTROPY_API_KEY=...\` or edit ~/.zbrain/config.json ` +
          `to add "zeroentropy_api_key": "...". (zbrain config set writes the DB plane, which the embed pipeline ignores.)`,
      };
    }
    return {
      name: 'ze_embedding_health',
 ...
 * crashed, or vice versa). Surfaces a paste-ready `zbrain ze-switch
 * --resume` hint.
 */
  ...
  try {
    ...
    if (!existing.exists) {
      return {
        name: 'embedding_width_consistency',
        status: 'warn',
        message: 'content_chunks.embedding column not found. Fix: run `zbrain init --migrate-only` or check schema.',
      };
    }
    ...
    if (existing.dims !== configDim) {
      ...
      // the no-op `zbrain config set` path. The recipe is paste-ready
 ...
 * the message and paste a `zbrain search modes --reset` fix command.
 */
    ...
    if (emptySources.length > 0) {
      return {
        ...
        message:
          ...
          `If you've recently run \`zbrain import --source-id <id>\` against these, the writes may have ` +
          `silently fallen to the default source pre-v0.37.7.0. Re-run with --source-id; verify via ` +
          `\`zbrain sources current --json\`.`,
      };
    }
    return {
      name: 'source_routing_health',
  ...
  try {
    ...
    for (const m of metrics) {
      ...
      if (m.lag_seconds !== null && m.lag_seconds > 24 * 3600) {
        fails.push(`${m.source_id}: stale ${Math.floor(m.lag_seconds / 3600)}h — run \`zbrain sync trigger --source ${m.source_id}\``);
        continue;
      }
      if (m.embed_coverage_pct < 50 && m.total_chunks > 1000) {
        fails.push(`${m.source_id}: ${m.embed_coverage_pct.toFixed(1)}% embed coverage (${m.total_chunks.toLocaleString()} chunks) — run \`zbrain jobs submit embed-backfill --params '{"sourceId":"${m.source_id}"}'\``);
        continue;
      }
      // Warns
      if (m.federated && m.lag_seconds !== null && m.lag_seconds > 3600) {
        warns.push(`${m.source_id}: federated source ${Math.floor(m.lag_seconds / 3600)}h+ stale — run \`zbrain sync trigger --source ${m.source_id}\``);
      }
      if (m.embed_coverage_pct < 95 && m.total_chunks > 100) {
        warns.push(`${m.source_id}: ${m.embed_coverage_pct.toFixed(1)}% embed coverage — run \`zbrain jobs submit embed-backfill --params '{"sourceId":"${m.source_id}"}'\``);
      }
      if (m.failed_jobs_24h >= 3) {
        warns.push(`${m.source_id}: ${m.failed_jobs_24h} failures in 24h — check \`zbrain jobs list --status failed\``);
      }
    }
    ...
    if (broken.length > 0) {
      return {
        ...
        message:
          ...
          `. Fix: \`zbrain auth revoke-client <id> && zbrain auth register-client …\` for each, OR \`zbrain upgrade\` if pre-v0.37.7.0.`,
      };
    }
    return {
      name: 'oauth_confidential_client_health',
 ...
 *
 * Detects stale autopilot lockfiles. When `ZBRAIN_HOME` is set, the
 * canonical lock path lives under `zbrainPath('autopilot.lock')`.
 * If a hardcoded `~/.zbrain/autopilot.lock` ALSO exists outside the
 * current `ZBRAIN_HOME`, that's a pre-v0.37.7.0 leftover or a
...
export function checkAutopilotLockScope(): Check {
  try {
    const canonical = zbrainPath('autopilot.lock');
    const home = process.env.HOME || '';
    const legacy = home ? `${home}/.zbrain/autopilot.lock` : '';
    ...
    // legacy lock exists outside ZBRAIN_HOME. Read its PID for a safe hint.
    ...
    return {
      ...
      status: 'warn',
      message:
        `Stale lockfile outside ZBRAIN_HOME: ${legacy} (owning PID: ${owningPid}). ` +
 ...
 * `zbrain-sync:<source>` lock-key shape so users can copy-paste the
...
export async function checkStaleLocks(engine: BrainEngine): Promise<Check> {
  try {
    ...
    const lines = stale.slice(0, 10).map(s => {
      ...
      const source = s.id.startsWith('zbrain-sync:') ? s.id.slice('zbrain-sync:'.length) : null;
      const breakHint = source ? `zbrain sync --break-lock --source ${source}` : `zbrain sync --break-lock`;
  ...
  try {
    ...
    if (!mode) {
      return {
        name: 'search_mode',
        status: 'ok',
        message: 'search.mode is unset (using balanced fallback). Run `zbrain search modes` to see what is running and pick a mode explicitly.',
      };
    }
    ...
    return {
      name: 'search_mode',
      status: 'ok',
      message: `Mode: ${mode} with ${overrideKeys.length} per-key override(s) (${overrideKeys.join(', ')}). To consolidate to the pure mode bundle: zbrain search modes --reset`,
    };
  } catch (e) {
    return {
      name: 'search_mode',
...
export async function checkEvalDrift(engine: BrainEngine): Promise<Check> {
  try {
    ...
    return {
      name: 'eval_drift',
      status: 'ok',
      message: `${drifted.length} retrieval-affecting file(s) changed since HEAD: ${summary}. Re-run \`zbrain eval run-all\` after committing these changes.`,
    };
  } catch (e) {
    return {
      name: 'eval_drift',
 ...
 *
 * ZBRAIN_EMBEDDING_MODEL / ZBRAIN_EMBEDDING_DIMENSIONS win over DB+file
...
async function checkEmbeddingEnvOverride(engine: BrainEngine): Promise<Check> {
  const envModel = process.env.ZBRAIN_EMBEDDING_MODEL?.trim();
  const envDim = process.env.ZBRAIN_EMBEDDING_DIMENSIONS?.trim();
  if (!envModel && !envDim) {
    return {
  ...
  if (envModel && dbModel && envModel !== dbModel) {
    mismatches.push({ key: 'ZBRAIN_EMBEDDING_MODEL', env: envModel, db: dbModel });
  }
  if (envDim && dbDim && envDim !== dbDim) {
    mismatches.push({ key: 'ZBRAIN_EMBEDDING_DIMENSIONS', env: envDim, db: dbDim });
  }
...
async function checkSubagentCapability(engine: BrainEngine): Promise<Check> {
  try {
      ...
      if (verdict === 'unusable:no_tools') {
        return {
          ...
          message:
            ...
            `Fix: \`zbrain config set ${source} <provider>:<model-with-tools>\` (e.g. anthropic:claude-sonnet-4-6 or openai:gpt-5.2).`,
        };
      }
      if (verdict === 'unknown') {
        return {
          ...
          message:
            ...
            `Fix: \`zbrain config set ${source} anthropic:claude-sonnet-4-6\` or pick another known provider.`,
        };
      }
      if (verdict === 'degraded:no_caching') {
        return {
          ...
          message:
            ...
            `\`zbrain config set models.tier.subagent anthropic:claude-sonnet-4-6\`.`,
        };
      }
      return null;
    };
    ...
    // require Anthropic at runtime; without the key, zbrain dream / zbrain
    // agent run / zbrain autopilot will all fail at job submission. Catches
      ...
      if (chatModel && !isAnthropicProvider(chatModel) && !process.env.ANTHROPIC_API_KEY) {
        return {
          ...
          message:
            ...
            `Subagent features (zbrain dream, zbrain agent run, zbrain autopilot) will fail at job submission ` +
            `unless agent.use_gateway_loop=true. Chat alone (zbrain think) still works. ` +
            `Either set ANTHROPIC_API_KEY or enable: \`zbrain config set agent.use_gateway_loop true\`.`,
        };
      }
...
function _resolveSyncFreshnessHours(varName: string, fallback: number): number {
  ...
  if (!Number.isFinite(n) || n <= 0) {
    if (!_syncFreshnessEnvWarned) {
      ...
      console.warn(
        `[zbrain doctor] Ignoring invalid ${varName}=${raw}; using default ${fallback}h.`,
      );
    }
    return fallback;
  }
 ...
 * been synced recently. Detects the silent failure mode where `zbrain sync`
 ...
 *     ZBRAIN_DRIFT_LIMIT + ZBRAIN_DRIFT_TIMEOUT_MS guards.
 *
 * Thresholds (env-overridable, default = 24h warn / 72h fail):
 *   - ZBRAIN_SYNC_FRESHNESS_WARN_HOURS
 *   - ZBRAIN_SYNC_FRESHNESS_FAIL_HOURS
 ...
 * `zbrain sync --source <id>` matches what the user copy-pastes.
 */
/**
...
export function computeNightlyQualityProbeHealthCheck(
  ...
  if (!probeEnabled && events.length === 0) {
    ...
    return {
      name,
      status: 'ok',
      message: `disabled (opt-in). Enable with: zbrain config set autopilot.nightly_quality_probe.enabled true`,
    };
  }
  if (events.length === 0) {
    return {
  ...
  try {
    ...
    if (!enabled) {
      return {
        ...
        status: 'ok',
        message:
          'disabled (opt-in). Enable with: zbrain config set cycle.conversation_facts_backfill.enabled true',
      };
    }
    ...
    if (backlog > 10) {
      const fixHint =
        'zbrain extract-conversation-facts --background --max-cost-usd 5';
      return {
        name,
        status: 'warn',
...
export async function checkSyncFreshness(
  ...
  try {
    ...
    const warnHours = _resolveSyncFreshnessHours('ZBRAIN_SYNC_FRESHNESS_WARN_HOURS', 24);
    const failHours = _resolveSyncFreshnessHours('ZBRAIN_SYNC_FRESHNESS_FAIL_HOURS', 72);
    ...
    for (const source of sources) {
      // Embed source.id in user-visible messages so `zbrain sync --source <id>`
    ...
    if (hasFailures) {
      return {
        name: 'sync_freshness',
        status: 'fail',
        message: `${issues.join('; ')}. Run \`zbrain sync --source <id>\` for each stale source`,
      };
    }
    if (hasWarnings) {
      return {
        name: 'sync_freshness',
        status: 'warn',
        message: `${issues.join('; ')}. Run \`zbrain sync --source <id>\` to refresh`,
      };
    }
    return {
      name: 'sync_freshness',
 ...
 *   - ZBRAIN_CYCLE_FRESHNESS_WARN_HOURS (default 6)
 *   - ZBRAIN_CYCLE_FRESHNESS_FAIL_HOURS (default 24)
 */
export async function checkCycleFreshness(
  ...
  try {
    ...
    const warnHours = _resolveSyncFreshnessHours('ZBRAIN_CYCLE_FRESHNESS_WARN_HOURS', 6);
    const failHours = _resolveSyncFreshnessHours('ZBRAIN_CYCLE_FRESHNESS_FAIL_HOURS', 24);
    const warnMs = warnHours * 60 * 60 * 1000;
    const failMs = failHours * 60 * 60 * 1000;
    const now = opts?.nowMs ?? Date.now();
    ...
    if (hasFailures) {
      return {
        name: 'cycle_freshness',
        status: 'fail',
        message: `${issues.join('; ')}. Run \`zbrain dream --source <id>\` for each stale source, or start \`zbrain autopilot\`.`,
      };
    }
    if (hasWarnings) {
      return {
 ...
 */
/**
 * Build the full check list for `zbrain doctor` against an engine + arg vector.
 *
 ...
 * `tests/unit/doctor-cli-smoke.test.ts` covers the render + exit paths that
...
export async function buildChecks(
  ...
  // run from `~` (e.g., `bun install -g github:garrytan/zbrain && cd ~ &&
  // zbrain doctor`) can still find the bundled skills/ dir without warning.
  ...
  if (skillsDir) {
    ...
    // to SKILL.md files; a user running `cd ~ && zbrain doctor --fix`
    // without an explicit signal would have install_path resolve to the
    // bundled zbrain repo and silently rewrite the install-tree skills.
    // Codex caught this leak in the v0.31.7 ship review (D6 lock).
    if (doFix) {
      if (detected.source === 'install_path') {
        process.stderr.write(
          'zbrain doctor --fix refused: skills dir resolved via install-path fallback (read-only).\n' +
          'The --fix flag writes to SKILL.md files; running it against the bundled install\n' +
          'tree would silently mutate zbrain itself. Set $ZBRAIN_SKILLS_DIR, $OPENCLAW_WORKSPACE,\n' +
  ...
  //
  // Audit trail: snapshot+diff at ~/.zbrain/audit/skill-brain-first-
  ...
  // `zbrain apply-migrations --yes` afterward. This check fires on every
  // `zbrain doctor` invocation so your OpenClaw's health skill catches it.
  //
  ...
  try {
    ...
    // `zbrain apply-migrations --force-retry <v>` once before plain --yes
    ...
    if (wedged.length > 0) {
      ...
      checks.push({
        name: 'minions_migration',
        status: 'fail',
        message: `MINIONS HALF-INSTALLED (partial migration: ${stuck.join(', ')}). Run: zbrain apply-migrations --yes`,
      });
    }
  ...
  // 3b. Upgrade-error trail (v0.13+). `zbrain upgrade` silently swallows
  // best-effort failures in `zbrain post-upgrade`; the failure record is
  // appended to ~/.zbrain/upgrade-errors.jsonl so we can surface it here
  // with a paste-ready recovery hint. Without this, users end up with
  // half-upgraded brains and no signal.
  try {
    const home = process.env.HOME || '';
    const errPath = join(home, '.zbrain', 'upgrade-errors.jsonl');
  ...
  // Reads the default PID file (`~/.zbrain/supervisor.pid` unless the user
  // overrode with ZBRAIN_SUPERVISOR_PID_FILE) and the latest audit file
  ...
  try {
    ...
    // Shared classifier — same code path runs in `zbrain jobs supervisor
    ...
    if (supervisorPid !== null || events.length > 0) {
      if (maxCrashesEvent) {
        checks.push({
          name: 'supervisor',
          status: 'fail',
          message: `Supervisor gave up at ${maxCrashesEvent.ts} (max_crashes_exceeded). Restart with: zbrain jobs supervisor start --detach`,
        });
      } else if (!running && events.length > 0) {
        checks.push({
          name: 'supervisor',
          status: 'warn',
          message: `Supervisor not running (last_start=${lastStart ?? 'unknown'}). Restart with: zbrain jobs supervisor start --detach`,
        });
        ...
        checks.push({
          name: 'supervisor',
          status: 'warn',
          message: `Worker crashed ${crashes24h}x in last 24h (${causeStr}). Check ~/.zbrain/audit/supervisor-*.jsonl for context.`,
        });
      } else {
        checks.push({
          name: 'supervisor',
  ...
  // ~/.zbrain/audit/stub-guard-YYYY-Www.jsonl. This check is the operator
    ...
    if (events.length > 10) {
      ...
      checks.push({
        ...
        message:
          ...
          `missing a case. Check ~/.zbrain/audit/stub-guard-*.jsonl for the slugs ` +
          `that hit it.`,
      });
  ...
  // to ~/.zbrain/sync-failures.jsonl with the commit hash + exact error.
  ...
  try {
    ...
    if (unacked.length > 0) {
      ...
      // run `zbrain sync --skip-failed` manually.
      ...
      checks.push({
        ...
        message:
          ...
          `Fix the file(s) and re-run 'zbrain sync', or use 'zbrain sync --skip-failed' to acknowledge.`,
  ...
  if (engine) {
    try {
      ...
      // Wire a remediation step on WARN so `zbrain doctor --remediate`
  ...
  // 3e. home_dir_in_worktree (v0.35.8.0). Walks up from `zbrainPath()`
  // looking for a `.git` directory OR file. If found, warns: `~/.zbrain/`
  ...
  // `~/.zbrain/.gitignore` (single-line `*`) laid down by saveConfig +
  ...
  // who set ZBRAIN_HOME=/tmp/something). Handles `.git` as both a directory
  // (main repo) and a file (linked worktree pointing at parent's worktrees/).
  // Honors ZBRAIN_HOME via zbrainPath().
  try {
    const zbrainHome = zbrainPath();
    const home = process.env.HOME || '';
    let worktreeRoot: string | null = null;
    if (zbrainHome && home && zbrainHome.startsWith(home + '/')) {
      // Walk up from zbrainHome's parent toward $HOME, stopping at $HOME.
      // We don't check zbrainHome itself: a `.git` directly inside ~/.zbrain
      // isn't a containing-worktree, it would be a brain repo cloned there.
      let cur = dirname(zbrainHome);
    ...
    if (worktreeRoot) {
      const homeEnvHint = process.env.ZBRAIN_HOME
        ? `# Or move \`~/.zbrain\` outside the worktree by setting ZBRAIN_HOME elsewhere.`
        : `# Fix: \`export ZBRAIN_HOME=/some/path/outside/the/worktree\` (zbrain appends \`.zbrain\`).`;
      checks.push({
        name: 'home_dir_in_worktree',
        status: 'warn',
        message:
          `~/.zbrain lives inside git worktree at ${worktreeRoot}. ` +
          `Config + brain DB could be committed by accident. ` +
          `A retroactive ~/.zbrain/.gitignore blocks casual \`git add\`, but does NOT cover ` +
      ...
      checks.push({
        name: 'home_dir_in_worktree',
        status: 'ok',
        message: 'zbrain home is outside any enclosing git worktree.',
      });
    }
  } catch {
      ...
      if (result.walk_truncated) {
        checks.push({
          ...
          message:
            ...
            `Re-run on a quieter brain or shorter walk via ZBRAIN_DRIFT_LIMIT/ZBRAIN_DRIFT_TIMEOUT_MS.`,
        });
      } else if (result.count > 0) {
        const sampleStr = result.sample.map(s => `${s.slug} (intended=${s.intended_source})`).join(', ');
        checks.push({
          ...
          message:
            ...
            `Verify with 'zbrain sources status', then either re-sync with ` +
            `'zbrain sync --source <id> --full' or 'zbrain delete <slug>' if the default-source ` +
            `row is the misroute. (A 'zbrain sources rehome' cleanup command is tracked for v0.32.0.)`,
        });
      } else {
        checks.push({
          name: 'multi_source_drift',
  ...
  // 3c. Orphan clone temp dirs (v0.28 P1). `zbrain sources add --url` clones
  // into $ZBRAIN_HOME/clones/.tmp/<id>-<rand>/ and renames atomically; if the
      ...
      if (stale.length === 0) {
        ...
        checks.push({
          ...
          message:
            ...
            `. Run \`zbrain sources purge-orphan-clones\` or wait for the autopilot purge phase.`,
        });
      }
    }
  } catch {
  ...
  if (fastMode || !engine) {
    if (!engine) {
      ...
      if (fastMode && dbSource) {
        ...
        msg = 'No database configured (filesystem checks only). Set ZBRAIN_DATABASE_URL or run `zbrain init`.';
      }
        ...
        if (parsed.port === '6543') {
          checks.push({
            ...
            message:
              ...
              'Fix: unset ZBRAIN_PREPARE (or set =false), or add ?prepare=false to the connection URL.',
          });
        }
      } catch {
  ...
  // 5. RLS — check ALL public tables, not just zbrain's own.
  ...
  //
  //     ZBRAIN:RLS_EXEMPT reason=<non-empty reason>
  //
  ...
  // `zbrain rls-exempt add` CLI on purpose. Doctor re-enumerates the
  ...
  if (engine.kind === 'pglite') {
    ...
    try {
      ...
      `;
      const EXEMPT_RE = /^ZBRAIN:RLS_EXEMPT\s+reason=\S.{3,}/;
      const exempt: string[] = [];
      const gaps: string[] = [];
      ...
      if (gaps.length === 0) {
        ...
        checks.push({
          ...
          message:
            ...
            `Fix: ${fixes} ` +
            `If a table should stay readable by the anon key on purpose, see docs/guides/rls-and-you.md for the ZBRAIN:RLS_EXEMPT comment escape hatch.`,
        });
      }
    } catch {
  ...
  try {
    ...
    if (schemaVersion >= LATEST_VERSION) {
      ...
      checks.push({
        name: 'schema_version',
        status: 'fail',
        message: `No schema version recorded. Migrations never ran. Fix: zbrain apply-migrations --yes. ` +
                 `If you installed via 'bun install -g github:...', see https://github.com/garrytan/zbrain/issues/218.`,
      });
    } else {
      checks.push({
        name: 'schema_version',
        status: 'warn',
        message: `Version ${schemaVersion}, latest is ${LATEST_VERSION}. Fix: zbrain apply-migrations --yes`,
      });
    }
  } catch {
    checks.push({ name: 'schema_version', status: 'warn', message: 'Could not check schema version' });
  ...
  // That's a valid fresh-install state after `zbrain init` — the migration
    ...
    try {
      ...
      if (rows.length === 0) {
        checks.push({
          ...
          status: 'warn',
          message:
            'Auto-RLS event trigger missing. New tables created outside zbrain may not get RLS. ' +
            'Fix: zbrain apply-migrations --force-retry 35',
        });
  ...
  try {
    ...
    if (health.embed_coverage >= 0.9) {
      ...
      checks.push({ name: 'embeddings', status: 'warn', message: `${pct}% coverage, ${health.missing_embeddings} missing. Run: zbrain embed --stale` });
    } else {
      checks.push({ name: 'embeddings', status: 'warn', message: 'No embeddings yet. Run: zbrain embed --stale' });
    }
  } catch {
  ...
  try {
    ...
    //   - empty brain (no embedded chunks) → `zbrain init --force --embedding-model …`
    //   - non-empty brain → `zbrain retrieval-upgrade --to … --reindex`
    // The bug-reporter's `rm -rf ~/.zbrain` recovery is never the right answer.
        ...
        if (colDim.exists && colDim.dims !== null && colDim.dims !== configuredDims) {
          ...
          if (totalChunks > 0) {
            const fix = embeddedCount === 0
              ? `No embeddings yet — drop the empty schema and re-init at the right dim:\n        zbrain init --force --pglite --embedding-model ${configuredModel} --embedding-dimensions ${configuredDims}`
              : `Non-empty brain (${embeddedCount} embedded chunks). Migrate cleanly:\n        zbrain retrieval-upgrade --to ${configuredModel} --reindex`;
            ...
            checks.push({
              ...
              message:
                ...
                `Persist your provider choice with \`zbrain config set embedding_model ${configuredModel}\` AND fix the schema:\n      ${fix}`,
            });
    ...
    if (surfacedUnconfiguredDrift) {
      ...
      const vec = await embedOne('zbrain doctor embedding smoke test');
  ...
  try {
    ...
    if (alternatives.length > 0) {
      checks.push({
        ...
        status: 'ok',
        message: `Detected ${alternatives.length} alternative embedding provider${alternatives.length > 1 ? 's' : ''} ready to use: ${alternatives.join(', ')}. Run \`zbrain providers list\` to switch.`,
      });
    }
        ...
        if (actualType !== entry.type) {
          issues.push(
            `${colName}: declared type=${entry.type} but actual is ${actual}. ` +
              `Fix: zbrain config set embedding_columns '<JSON>' OR ` +
      ...
      if (activeCol && actualByName.has(activeCol)) {
        ...
        if (total > 0 && pct < 90) {
          coverageWarn =
            ...
            `Fix: zbrain embed --column ${activeCol} --stale (write-side support v2) ` +
            `OR zbrain config set search_embedding_column embedding`;
        }
      }
  ...
  try {
    ...
    if (entityCount === 0) {
      ...
      checks.push({
        name: 'graph_coverage',
        status: 'warn',
        message: `Entity link coverage ${linkPct}%, timeline ${timelinePct}% (${entityCount} entity pages). Run: zbrain extract all`,
      });
    }
  ...
  // `zbrain orphans --count` (D1), so the two surfaces cannot disagree.
  //
  ...
  // `zbrain extract links --by-mention` as the fix.
  ...
  try {
    ...
    if (entityCount < 100) {
      ...
      const hint =
        'Run: zbrain extract links --by-mention   (auto-links entity mentions in body text). ' +
        'Run zbrain orphans for the list.';
      if (ratio > 0.8) {
        checks.push({
          name: 'orphan_ratio',
          status: 'fail',
  ...
  // warning. Full-brain scan: `zbrain integrity check`.
  ...
  try {
    ...
    if (total === 0) {
      ...
      // Three-bucket repair handled by `zbrain integrity auto` (the
      ...
      checks.push({
        name: 'integrity',
        status: 'warn',
        message: `Sampled ${res.pagesScanned} pages; ${res.bareHits.length} bare-tweet phrase(s), ${res.externalHits.length} external link(s). Run: zbrain integrity check (or integrity auto to repair).`,
  ...
  try {
    ...
    if (totalBad === 0) {
      ...
      checks.push({
        ...
        status: 'warn',
        message: `${totalBad} row(s) double-encoded (${breakdown.join(', ')}). Fix: zbrain repair-jsonb`,
      });
    }
  } catch {
  ...
  // gate itself runs via `zbrain eval whoknows`; this check is the
  ...
  // surface the gap so operators can run `zbrain backfill modality`.
      ...
      if (unifiedOnlyOn && lowestCoverage < 0.99) {
        checks.push({
          ...
          message:
            ...
            `Run \`zbrain reindex --multimodal\` to bring coverage to 99%+ or disable strict mode.`,
        });
        ...
        checks.push({
          ...
          message:
            ...
            `Run \`zbrain reindex --multimodal\` to fill the gap.`,
        });
      } else {
        checks.push({
          name: 'unified_multimodal_coverage',
  ...
  try {
    ...
    if (rows.length === 0) {
      ...
      checks.push({
        ...
        status: 'warn',
        message: `${rows.length} page(s) appear truncated (sample: ${sample}). Re-import with: zbrain sync --force`,
      });
    }
  } catch {
  ...
  // - content_sanity_audit_recent: reads ~/.zbrain/audit/content-sanity-*.jsonl
  //   over the last 7 days, aggregates by event type + source. Caveat
  //   (Codex r1 #14): JSONL is local-only — multi-host operators should
  //   share ZBRAIN_AUDIT_DIR. Message names this so the limitation is
  ...
  try {
    ...
    if (events.length === 0) {
      checks.push({
        ...
        status: 'ok',
        message: 'No content-sanity events in last 7 days (audit JSONL is local to this host; share ZBRAIN_AUDIT_DIR for multi-host visibility)',
      });
    } else {
      ...
      checks.push({
        name: 'content_sanity_audit_recent',
        status,
        message: `${events.length} events (hard=${summary.by_type.hard_block} soft=${summary.by_type.soft_block} warn=${summary.by_type.warn})${topPatterns ? ', patterns: ' + topPatterns : ''}${topSources ? ', sources: ' + topSources : ''}. (Local audit only — multi-host operators set ZBRAIN_AUDIT_DIR.)`,
      });
    }
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
  ...
  // `zbrain frontmatter validate <source-path> --fix`, which writes .bak
  ...
  //    JSON consumer.
  //  - Configurable via ZBRAIN_DOCTOR_FM_TIMEOUT_MS (default 30000ms).
  progress.heartbeat('frontmatter_integrity');
  const fmHb = startHeartbeat(progress, 'scanning frontmatter…');
  const fmTimeoutMs = (() => {
    const raw = process.env.ZBRAIN_DOCTOR_FM_TIMEOUT_MS;
    ...
    if (report.total === 0 && !report.partial) {
      ...
      for (const src of report.per_source) {
        if (src.status === 'skipped') {
          // Codex adversarial #1: `zbrain frontmatter validate` takes a
          ...
          sourceMessages.push(
            `${src.source_id}: NOT SCANNED (timeout — run \`zbrain frontmatter validate ${src.source_path}\`)`,
          );
          continue;
        }
      ...
      const fixHint = report.partial
        ? `Raise ZBRAIN_DOCTOR_FM_TIMEOUT_MS or run \`zbrain frontmatter validate <source>\` directly. Fix issues: \`zbrain frontmatter validate <source> --fix\``
        : `Fix: zbrain frontmatter validate <source-path> --fix`;
      checks.push({
        name: 'frontmatter_integrity',
        status: 'warn',
        message:
  ...
  // can see drops cross-process (the MCP server captures; `zbrain doctor`
    ...
    if (failures.length === 0) {
      ...
      checks.push({
        ...
        message: `${failures.length} capture failure(s) in the last 24h (${breakdown}). ` +
          ...
          `in ~/.zbrain/config.json to silence.`,
      });
    }
  } catch (err) {
  ...
  try {
    ...
    if (recent.length === 0) {
      checks.push({
        name: 'contradictions',
        status: 'ok',
        message: 'No probe runs in the last 7 days. Run `zbrain eval suspected-contradictions --query "..." --top-k 5` to populate.',
      });
    } else {
      ...
      if (total === 0) {
        ...
        if (highFindings.length > 3) {
          lines.push(`  …and ${highFindings.length - 3} more — see \`zbrain eval suspected-contradictions review\``);
        }
        checks.push({
          name: 'contradictions',
  ...
  try {
      ...
      checks.push({
        ...
        message: anyOverThreshold
          ? `Facts:absorb failures over the threshold (${threshold}) in the last 24h: ${summary}. ` +
            `Run \`zbrain recall --since 24h --json\` to inspect what landed; ` +
            `tune the gate via \`zbrain config set facts.absorb_warn_threshold N\`.`
    ...
    if (code === '42P01' || code === '42703') {
      ...
      checks.push({
        name: 'facts_extraction_health',
        status: 'ok',
        message: 'Skipped (ingest_log.source_id unavailable — run `zbrain apply-migrations --yes`).',
      });
    } else if (code === '42501') {
      checks.push({
        name: 'facts_extraction_health',
  ...
  try {
    ...
    if (fallbackWithFm > 0 || future > 0 || pre1990 > 0) {
      ...
      checks.push({
        ...
        status: 'warn',
        message: `${parts.join('; ')} (sample of last 1000 pages). Run \`zbrain reindex-frontmatter\` to recompute.`,
      });
    } else {
      checks.push({
        name: 'effective_date_health',
    ...
    if (code === '42703') {
      ...
      checks.push({ name: 'effective_date_health', status: 'ok', message: 'Skipped (effective_date column unavailable — run zbrain apply-migrations)' });
    } else {
  ...
  try {
    ...
    if (zeroWithTakes > 0) {
      checks.push({
        ...
        status: 'warn',
        message: `${zeroWithTakes} pages with active takes have emotional_weight=0. Run \`zbrain dream --phase recompute_emotional_weight\` to populate. Brain has ${nonzero} pages with non-zero emotional_weight.`,
      });
    } else if (nonzero === 0) {
      checks.push({
        name: 'salience_health',
  ...
  //      Surface the ID so the operator can `zbrain jobs get <id>` to inspect
  //      or `zbrain jobs cancel <id>` to force-kill.
  //
  //   2. backpressure-missed: per-name waiting depth exceeds the threshold
  //      (default 10, override via ZBRAIN_QUEUE_WAITING_THRESHOLD env). Signal
  ...
  if (engine.kind === 'pglite') {
    ...
    try {
      ...
      const rawThreshold = process.env.ZBRAIN_QUEUE_WAITING_THRESHOLD;
      ...
      if (stalledRows.length > 0) {
        ...
        problems.push(
          ...
          `Fix: zbrain jobs get <id> to inspect; zbrain jobs cancel <id> to force-kill.`
        );
      }
      if (depthRows.length > 0) {
        ...
        problems.push(
          ...
          `Fix: set maxWaiting on the submitter (or raise ZBRAIN_QUEUE_WAITING_THRESHOLD).`
        );
      }
      if (rssKillCount > 0) {
        problems.push(
          ...
          `Fix: raise the limit (e.g. \`zbrain jobs work --max-rss 4096\`) or opt out (\`--max-rss 0\`). ` +
      ...
      if (promptTooLongCount > 0) {
        problems.push(
          ...
          `Fix: \`zbrain dream --phase synthesize --dry-run --json\` to identify fat transcripts; ` +
      ...
      if (rows.length === 0) {
        ...
        checks.push({
          ...
          message: `${vanished} of ${rows.length} image(s) missing from disk (e.g. ${vanishedPaths.join(', ')}). ` +
                   `Fix: restore from git, or \`zbrain sync --skip-failed\` to acknowledge.`,
        });
      }
    } catch {
...
/**
 * CLI entry point for `zbrain doctor`. Thin wrapper around buildChecks +
 ...
 * tests/unit/doctor-behavioral.test.ts for the in-process seam coverage and
 * tests/unit/doctor-cli-smoke.test.ts for the subprocess wrapper coverage.
 */
...
export function skillBrainFirstCheck(skillsDir: string): Check {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    process.stderr.write(`[zbrain] skill_brain_first audit step failed (${msg}); check continues\n`);
  }
  ...
  summary.push(
    ...
    `Fix via 'zbrain doctor --fix' (adds canonical Convention callout) ` +
  ...
  if (formerlyExempt.length > 0) {
    summary.push(
      ...
      `These need explicit opt-out now: run 'zbrain doctor --fix' to add the canonical callout, ` +
...
/**
 * `zbrain doctor --locks` — list idle-in-transaction backends older
...
async function runLocksCheck(engine: BrainEngine | null, jsonOutput: boolean): Promise<void> {
  if (!engine) {
    if (jsonOutput) {
      ...
      console.log('zbrain doctor --locks requires a database connection. Configure a URL and retry.');
    }
    process.exit(1);
  }
  ...
  if (engine.kind !== 'postgres') {
    if (jsonOutput) {
      ...
      console.log(`zbrain doctor --locks is Postgres-only. Current engine: ${engine.kind}. No blockers possible (no connection pool).`);
    }
    return;
  }
  ...
  console.log('After terminating, retry: zbrain apply-migrations --yes');
  process.exit(1);
}
    ...
    if (err instanceof BudgetExhausted) {
      ...
      console.error(
        `\n[remediate] BudgetExhausted (${err.reason}): spent $${err.spent.toFixed(4)} > cap $${err.cap.toFixed(2)}.\n` +
          ...
          `  zbrain doctor --remediate --resume ${planHash}\n`,
      );
    } else {
      throw err;
    }
...
async function loadRecommendationContext(engine: BrainEngine) {
  ...
  // in ZBrainConfig + propagates to the gateway env dict.
...
async function checkSchemaPackActive(engine: BrainEngine): Promise<Check> {
  try {
    ...
    return {
      name: 'schema_pack_active',
      status: 'warn',
      message: `Active pack failed to resolve: ${(e as Error).message}. Run \`zbrain schema active\` to debug.`,
    };
  }
}
  ...
  try {
    ...
    if (worstPct >= 0.1) {
      return {
        ...
        status: 'warn',
        message: `Source \`${worstSrc}\`: ${worstUntyped} of ${worstTotal} pages (${pctStr}%) have no type matching the active pack. Run \`zbrain schema detect --source ${worstSrc}\` to propose a pack matching your content shape.`,
      };
    }
    return {
      name: 'schema_pack_consistency',
...
async function checkSchemaPackSourceDrift(engine: BrainEngine): Promise<Check> {
  try {
    ...
    return {
      ...
      status: 'warn',
      message: `Per-source pack divergence detected: ${distinctPacks.size} distinct packs across ${rows.length} sources. Run \`zbrain sources list\` then \`zbrain schema active --source <id>\` per source to audit.`,
    };
  } catch (e) {
    return {
      name: 'schema_pack_source_drift',
