/**
 * zbrain skillpack <list|scaffold|reference|migrate-fence|scrub-legacy-fence-rows|harvest|diff|check>
 *
 ...
 *   - `harvest`                — inverse: lift host skill into zbrain
 *
...
const HELP_TOP = `zbrain skillpack <subcommand> [options]
...
Subcommands:
  ...
  scaffold --all             Scaffold every bundled skill (zbrain only).

  reference <name>           Read-only: diff zbrain's bundle vs your local copy.
  ...
  init <name>                Scaffold a fresh skillpack tree (cathedral
                             default; --minimal opts out of tests/unit/e2e/evals).
...
Run \`zbrain skillpack <subcommand> --help\` for per-subcommand options.
...
export async function runSkillpack(args: string[]): Promise<void> {
  ...
  switch (sub) {
    ...
    case 'install':
      console.error(
        "Error: 'zbrain skillpack install' was removed in v0.33. Use 'zbrain skillpack scaffold <name>' instead.\n" +
          "If you're upgrading from an older release, run 'zbrain skillpack migrate-fence' once to strip the legacy managed block, then scaffold any new skills.",
      );
      process.exit(2);
      return;
    case 'uninstall':
      console.error(
        "Error: 'zbrain skillpack uninstall' was removed in v0.33. The new scaffold model lets you own scaffolded files outright — to remove a skill, delete its directory (rm -rf skills/<slug>/) and any paired source files declared in its frontmatter.",
      );
      process.exit(2);
      return;
    default:
...
function findZbrainOrDie(): string {
  const root = findZbrainRoot();
  if (!root) {
    console.error('Error: could not find zbrain repo root.');
    process.exit(2);
  }
  return root;
}
...
async function cmdList(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log('zbrain skillpack list [--json]\n\nPrint every skill bundled in openclaw.plugin.json.');
    process.exit(0);
  }
  const json = args.includes('--json');
  const zbrainRoot = findZbrainOrDie();
  let manifest;
  try {
    manifest = loadBundleManifest(zbrainRoot);
  } catch (err) {
  ...
  if (json) {
    const entries = slugs.map(slug => {
      const skillMd = join(zbrainRoot, 'skills', slug, 'SKILL.md');
...
async function cmdScaffold(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'zbrain skillpack scaffold <name> | <source> | --all [--workspace PATH] [--dry-run] [--trust] [--no-cache] [--json]\n\n' +
      ...
      '  --all               Scaffold every bundled skill (zbrain only)\n' +
  ...
  if (!all && name !== null && !isThirdPartyShape) {
    ...
    const zbrainRoot = findZbrainRoot();
    if (zbrainRoot) {
      try {
        const manifest = loadBundleManifest(zbrainRoot);
        const slugs = bundledSkillSlugs(manifest);
      ...
      if (!found) {
        console.error(
          `Error: no skillpack named "${cls.normalized}" in the registry (${loaded.registry_url}).\n` +
            `Run \`zbrain skillpack search ${cls.normalized}\` for matches, or pass a full source (owner/repo, https URL, ./path, ./*.tgz).`,
        );
        process.exit(2);
      }
  ...
  try {
    ...
    if (opts.json) {
      ...
      if (!opts.dryRun && summary && summary.wroteNew > 0) {
        console.log(
          `\nNext: your agent walks skills/*/SKILL.md frontmatter triggers: for routing.\n` +
            `Run \`zbrain skillpack reference ${m.name}\` later if upstream changes.`,
        );
      }
    }
...
async function cmdReference(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'zbrain skillpack reference <name> | --all [--workspace PATH] [--apply-clean-hunks] [--since <version>] [--dry-run] [--json]\n\n' +
        '  --since <version>   With --all, restrict the sweep to skills whose source\n' +
        '                      changed in zbrain between <version> and HEAD. Useful\n' +
        '                      after `zbrain upgrade` to see only what moved.',
    );
    process.exit(0);
  }
  const json = args.includes('--json');
  ...
  try {
    if (apply) {
      ...
      const twoWayWarning =
        'WARNING: --apply-clean-hunks is a two-way diff against zbrain\'s CURRENT bundle.\n' +
        '         zbrain does NOT have access to the version you originally scaffolded.\n' +
        '         Hunks where your LOCAL edits differ from zbrain WILL be aligned to zbrain.\n' +
        '         If you have intentional local edits, run `zbrain skillpack reference ' + name + '`\n' +
        '         (read-only) first to inspect, OR pass --dry-run on this command.';
      if (!dryRun && !json) console.error(twoWayWarning);

      const result = runReferenceApply({ zbrainRoot, targetWorkspace, skillSlug: name!, dryRun });
    ...
    if (all) {
      ...
      // --since filter: keep only skills whose source changed in zbrain
      ...
      if (since) {
        ...
        const slugs = changedSlugsSinceVersion(zbrainRoot, since);
        if (slugs === null) {
          console.error(
    ...
    const result = runReference({ zbrainRoot, targetWorkspace, skillSlug: name! });
    if (json) console.log(JSON.stringify(result, null, 2));
    else {
      console.log(result.framing);
      console.log(
      ...
      if (result.summary.missing > 0 || result.summary.differs > 0) {
        ...
        if (result.summary.missing > 0) {
          console.log(
            '  missing → zbrain has a file you don\'t. Usually safe to `zbrain skillpack scaffold ' + name + '` again to land it.',
          );
        }
        if (result.summary.differs > 0) {
          console.log(
            '  differs → was your local edit intentional? Keep it (zbrain is reference, not law).\n            Accidental drift? Patch by hand, or `zbrain skillpack reference ' + name + ' --apply-clean-hunks`\n            (READ the two-way merge warning in that command\'s output first).',
          );
        }
      }
    }
...
async function cmdMigrateFence(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log('zbrain skillpack migrate-fence [--workspace PATH] [--dry-run] [--json]');
    process.exit(0);
  }
  const json = args.includes('--json');
  const dryRun = args.includes('--dry-run');
  ...
  const zbrainRoot = findZbrainOrDie();
  const targetWorkspace = resolveWorkspace({ workspace });
  const result = runMigrateFence({ targetWorkspace, zbrainRoot, dryRun });
  if (json) console.log(JSON.stringify(result, null, 2));
  else {
...
async function cmdScrubLegacy(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'zbrain skillpack scrub-legacy-fence-rows [--workspace PATH] [--dry-run] [--json]',
    );
    process.exit(0);
  }
  const json = args.includes('--json');
...
async function cmdSearch(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'zbrain skillpack search [<query>] [--tier endorsed|community|experimental|dead] [--json] [--refresh] [--url URL]',
    );
    process.exit(0);
  }
  const json = args.includes('--json');
...
async function cmdInfo(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log('zbrain skillpack info <name> [--json] [--refresh] [--url URL]');
    process.exit(0);
  }
  const json = args.includes('--json');
  const refresh = args.includes('--refresh');
      ...
      JSON.stringify(
        {
          ...
          zbrain_min_version: found.entry.zbrain_min_version,
  ...
  console.log(`  zbrain min:    ${found.entry.zbrain_min_version}`);
  ...
  console.log('\nTo scaffold:');
  console.log(`  zbrain skillpack scaffold ${found.entry.name}`);
  process.exit(0);
}
...
async function cmdRegistry(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'zbrain skillpack registry [--url URL] [--refresh] [--json]\n' +
        '\n' +
        '  --url URL    Set the registry URL (writes to ~/.zbrain/config.json)\n' +
  ...
  if (setUrl) {
    // Persist to ~/.zbrain/config.json under skillpack.registry_url.
    const { zbrainPath } = await import('../core/config.ts');
    const cfgPath = zbrainPath('config.json');
...
async function cmdDoctor(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'zbrain skillpack doctor <pack-dir> [--quick|--full] [--fix] [--yes] [--json]\n\n' +
...
async function cmdInit(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'zbrain skillpack init <name> [--target PATH] [--minimal] [--author NAME] [--license SPDX] [--homepage URL] [--dry-run] [--json]\n\n' +
        '  <name>       Pack name (lowercase kebab; becomes manifest.name + dir leaf)\n' +
        '  --target     Target dir (default: ./<name>)\n' +
        '  --minimal    Skip tests/unit/, e2e/, evals/ (advanced; doctor will score lower)\n' +
  ...
  try {
    ...
    if (json) {
      ...
      if (!dryRun) {
        console.log(
          `\nNext:\n  cd ${result.targetDir}\n  zbrain skillpack doctor . --quick\n  # iterate, then:\n  zbrain skillpack pack`,
        );
      }
    }
    process.exit(0);
...
async function cmdPack(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'zbrain skillpack pack [<pack-dir>] [--out PATH] [--dry-run] [--skip-doctor] [--json]\n\n' +
  ...
  try {
    ...
    if (json) {
      ...
      console.error('\nRun `zbrain skillpack doctor . --fix --yes` to auto-scaffold what you can, then re-run.');
      process.exit(2);
...
async function cmdEndorse(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'zbrain skillpack endorse <name> [--tier endorsed|community|experimental|dead] [--repo PATH] [--note TEXT] [--push] [--dry-run] [--json]\n\n' +
        ...
        'requires a clone of garrytan/zbrain-skillpack-registry (or any registry-shaped repo).',
    );
    process.exit(0);
  }
  const json = args.includes('--json');
...
async function cmdHarvest(args: string[]): Promise<void> {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'zbrain skillpack harvest <slug> --from <host-repo-root> [--no-lint] [--dry-run] [--overwrite-local] [--json]',
    );
    process.exit(0);
  }
  const json = args.includes('--json');
...
async function cmdDiff(args: string[]): Promise<void> {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log('zbrain skillpack diff <name> [--workspace PATH] [--json]');
    process.exit(0);
  }
  const json = args.includes('--json');
  let name: string | null = null;
  ...
  try {
    const diffs = diffSkill(zbrainRoot, name, targetSkillsDir);
    ...
    if (json) console.log(JSON.stringify({ ok: true, skillName: name, diffs }, null, 2));
      ...
      console.log(clean ? '\n✓ all files match the bundle.' : '\n(Run `zbrain skillpack reference ' + name + '` for a unified diff.)');
    }
