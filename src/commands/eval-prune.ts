/**
 * zbrain eval prune — delete old eval_candidates rows (v0.21.0).
 *
 * Retention is unlimited by default (matches ingest_log precedent).
 * This command is the explicit cleanup; pairs with `zbrain eval export`
 * (snapshot first, then prune if you want to reset).
 *
 * Usage:
 *   zbrain eval prune --older-than 30d
 *   zbrain eval prune --older-than 1h --dry-run
 */
...
function printHelp(): void {
  console.error(`zbrain eval prune — delete old eval_candidates rows

USAGE:
  zbrain eval prune --older-than DUR [--dry-run]
...
EXAMPLES:
  zbrain eval prune --older-than 30d
  zbrain eval prune --older-than 90d --dry-run
`);
}
