#!/usr/bin/env bun
/**
 * ZBrain token management.
 *
 * Wired into the CLI as of v0.22.5:
 *   zbrain auth create "claude-desktop"
 *   zbrain auth list
 *   zbrain auth revoke "claude-desktop"
 *   zbrain auth test <url> --token <token>
 *
 ...
 * configured for. The env-var DATABASE_URL / ZBRAIN_DATABASE_URL still
...
function generateToken(): string {
  return 'zbrain_' + randomBytes(32).toString('hex');
}
...
async function withConfiguredSql<T>(
  ...
  if (!config) {
    console.error('No ZBrain config found. Run `zbrain init` first, or set DATABASE_URL / ZBRAIN_DATABASE_URL.');
    process.exit(1);
  }
...
async function create(name: string, opts: { takesHolders?: string[] } = {}) {
  ...
  try {
    await withConfiguredSql(async (_sql, engine) => {
      ...
      // bug class (verified by tests/unit/e2e/auth-permissions.test.ts:67 on
      // Postgres and tests/unit/sql-query.test.ts on PGLite).
      ...
      console.log(`Revoke with: zbrain auth revoke "${name}"`);
      console.log(`Update visibility: zbrain auth permissions "${name}" set-takes-holders world,garry`);
    });
  } catch (e: any) {
    if (e.code === '23505') {
...
async function list() {
  await withConfiguredSql(async (sql) => {
    ...
    `;
    if (rows.length === 0) {
      console.log('No tokens found. Create one: zbrain auth create "my-client"');
      return;
    }
    ...
    const initRes = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        params: {
          ...
          capabilities: {},
          clientInfo: { name: 'zbrain-smoke-test', version: '1.0' },
        },
        id: 1,
      }),
    });
...
/**
 * Parse `zbrain auth register-client` argv. Walks the array once instead of
...
async function registerClient(name: string, args: string[]) {
  ...
  try {
    await withConfiguredSql(async (sql) => {
      const { ZBrainOAuthProvider } = await import('../core/oauth-provider.ts');
      const provider = new ZBrainOAuthProvider({ sql });
      ...
      console.log(`Revoke with: zbrain auth revoke-client "${clientId}"`);
    });
  } catch (e: any) {
    console.error('Error:', e.message);
    process.exit(1);
...
/**
 * Entry point for the `zbrain auth` CLI subcommand. Also reused by the
...
export async function runAuth(args: string[]): Promise<void> {
  ...
  switch (cmd) {
    ...
    case 'permissions': {
      // zbrain auth permissions <name> set-takes-holders world,garry
    ...
    default:
      console.log(`ZBrain Token Management

Usage:
  zbrain auth create <name> [--takes-holders world,garry,brain]
  ...
  zbrain auth list                                         List all tokens
  zbrain auth revoke <name>                                Revoke a legacy token
  zbrain auth permissions <name> set-takes-holders <h1,h2,h3>
                                                          Update visibility for an existing token
  zbrain auth register-client <name> [options]             Register an OAuth 2.1 client (v0.26+)
  ...
  zbrain auth revoke-client <client_id>                   Hard-delete an OAuth 2.1 client (cascades to tokens + codes)
  zbrain auth test <url> --token <token>                  Smoke-test a remote MCP server
`);
  }
}
