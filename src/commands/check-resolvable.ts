/**
 * zbrain check-resolvable — Standalone CLI gate for skill-tree integrity.
 *
...
const HELP_TEXT = `zbrain check-resolvable [options]
...
export function resolveSkillsDir(flags: Flags): {
  ...
  const sourceLabel = {
    env_explicit: '$ZBRAIN_SKILLS_DIR (explicit operator override)',
    ...
    cwd_skills: './skills',
    install_path: 'zbrain install path (read-only fallback)',
  }[detected.source!]!;
...
export async function runCheckResolvable(args: string[]): Promise<void> {
  ...
  if (flags.fix) {
    ...
    // the cwd tree would resolve to the bundled zbrain repo via the
    ...
    if (source === 'install_path') {
      process.stderr.write(
        'zbrain check-resolvable --fix refused: skills dir resolved via install-path fallback (read-only).\n' +
