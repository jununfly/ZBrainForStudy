/**
 * zbrain publish — Generate shareable HTML from brain markdown pages.
 *
 ...
 *
 * Usage:
 *   zbrain publish <page-path>                         # local HTML file
 *   zbrain publish <page-path> --password              # auto-generated pw
 *   zbrain publish <page-path> --password "secret"     # custom pw
 *   zbrain publish <page-path> --out /tmp/share.html   # custom output
 *   zbrain publish <page-path> --title "Custom Title"  # override title
 */
...
export async function runPublish(args: string[]) {
  ...
  if (!inputPath) {
    console.error('Usage: zbrain publish <page.md> [--password ["secret"]] [--title "Title"] [--out path]');
    console.error('');
