/**
 * `zbrain cache` \u2014 manage the semantic query cache (v0.32.x search-lite).
 *
 * Subcommands:
 *   zbrain cache stats   \u2014 print row/hit counts and freshness breakdown.
 *   zbrain cache clear   \u2014 wipe all cache rows.
 *   zbrain cache prune   \u2014 delete only stale (past-TTL) rows.
 *
...
Usage:
  zbrain cache stats        Print cache row counts, hit counts, freshness.
  zbrain cache clear        Wipe ALL cache rows. Requires --yes.
  zbrain cache prune        Delete only stale (past-TTL) rows.
...
export async function runCache(args: string[]): Promise<void> {
  ...
  if (!config) {
    // eslint-disable-next-line no-console
    console.error('zbrain cache: no brain configured. Run `zbrain init` first.');
    process.exit(1);
  }
  ...
  try {
    ...
    if (sub === 'clear') {
      ...
      if (!yes) {
        // eslint-disable-next-line no-console
        console.error('zbrain cache clear: refusing to wipe without --yes flag.');
        process.exit(1);
      }
    ...
    // eslint-disable-next-line no-console
    console.error(`zbrain cache: unknown subcommand "${sub}". See \`zbrain cache --help\`.`);
    process.exit(1);
  } finally {
