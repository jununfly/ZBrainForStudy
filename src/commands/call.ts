...
/**
 * `zbrain call <tool> <json>` — trusted local op-dispatch surface.
 *
 ...
 * 6-tier chain (--source > ZBRAIN_SOURCE > .zbrain-source dotfile > path-match
...
export async function runCall(engine: BrainEngine, args: string[]) {
  ...
  // tokens to keep the existing `zbrain call <tool> <json>` shape readable,
  ...
  if (!tool) {
    console.error("Usage: zbrain call [--source <id>] <tool> '<json>'");
    process.exit(1);
  }
