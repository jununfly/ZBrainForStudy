/**
 * zbrain skillify check — 11-item post-task audit.
 *
 ...
 * the CLI entry point is now `zbrain skillify check`.
 *
 ...
 *   6. Resolver entry
 *   7. Resolver trigger eval (heuristic via `tests/unit/resolver.test.ts`)
 *   8. check-resolvable gate (runs `zbrain check-resolvable --json`)
 ...
 *      receipt at `zbrainPath('eval-receipts')/<slug>-<sha8>.json`
...
function detectTestDir(root: string): string | null {
  for (const candidate of ['tests/unit', '__tests__', 'tests', 'spec']) {
...
function runCheckResolvableCached(): ResolverResult {
  ...
  try {
    ...
    if (res.error || res.status === null) {
      ...
      console.error(`[skillify] zbrain check-resolvable not runnable: ${reason}`);
    ...
    if (payload.ok === true) {
      ...
      _resolverCache = {
        ok: false,
        detail: `${count} issue(s)${err} — run: zbrain check-resolvable`,
      };
    }
    return _resolverCache;
  } catch (err) {
...
function runSkillifyCheckTarget(target: string, root: string): CheckResult {
  ...
  items.push(check('E2E test (either under e2e/ or integration test)', hasE2E, 'try /qa or tests/unit/e2e/'));
  ...
  // `zbrain skillify check` with exit 1 so new skills can't be born
  // non-compliant.
 ...
 *
 * Reads the receipt from `zbrainPath('eval-receipts')` (T5 correction:
 * this resolves to <ZBRAIN_HOME>/.zbrain/eval-receipts/, NOT the legacy
 * <ZBRAIN_HOME>/eval-receipts/ that the original plan claimed).
 */
function lookupCrossModalReceipt(
  ...
  try {
    status = findReceiptForSkill(skillMdPath, zbrainPath('eval-receipts'));
  } catch (err) {
...
const HELP = `zbrain skillify check [path] [--recent] [--json]
...
/**
 * Entry point invoked by `zbrain skillify check`. The outer
