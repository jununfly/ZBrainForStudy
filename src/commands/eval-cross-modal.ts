/**
 * zbrain eval cross-modal — multi-model quality gate (v0.27.x).
 *
 ...
 * before `zbrain init`. Receipts are bound to (slug, SKILL.md sha-8) so
 * `zbrain skillify check` can detect stale audits (T10=A).
 *
...
import { zbrainPath, loadConfig } from '../core/config.ts';
...
const HELP = `zbrain eval cross-modal — multi-model quality gate

USAGE:
  zbrain eval cross-modal --task "<description>" --output <path-or-skill-slug> [flags]
  zbrain eval cross-modal --batch <jsonl> [--limit N] [--output <receipt-path>] [flags]   (v0.40.1.0 Track D)
...
REQUIRED (batch mode, v0.40.1.0 Track D / T3):
  --batch <jsonl>          LongMemEval-shape JSONL (output of \`zbrain eval
...
BATCH FLAGS:
  ...
  --output PATH            Where to write the SUMMARY receipt (NOT where to
                           ...
                           ~/.zbrain/eval-receipts/cross-modal-batch-<sha8>.json
...
FLAGS:
  ...
  --receipt-dir <path>     Default: zbrainPath('eval-receipts').
...
CONFIGURATION:
  Models resolve via the zbrain AI gateway. Configure with:
    zbrain providers test            # see what's configured
    zbrain config                    # set keys
  Or set env vars: OPENAI_API_KEY, ANTHROPIC_API_KEY, GOOGLE_GENERATIVE_AI_API_KEY,
  TOGETHER_API_KEY, etc. The gateway reads from \`~/.zbrain/config.json\` plus
  process.env.

EXAMPLES:
  zbrain eval cross-modal \\
 ...
 *
 * Pinned by tests/unit/eval-cross-modal-batch.test.ts: never exceeds limit,
...
/**
 * Configure the AI gateway from `~/.zbrain/config.json` + process.env.
 *
...
export async function runEvalCrossModal(args: string[], opts: RunCrossModalOpts = {}): Promise<number> {
  ...
  const receiptDir = parsed.receiptDir ?? zbrainPath('eval-receipts');
  ...
  if (!isAvailable('chat')) {
    process.stderr.write(
      'Error: AI gateway has no usable chat provider. ' +
        ...
        'in your shell or run `zbrain config` to set keys.\n',
    );
    return 1;
  }
...
// at end of run (per D10 — keeps ~/.zbrain/eval-receipts/ clean).
// ---------------------------------------------------------------------------
 ...
 * `zbrain eval longmemeval`. Carries `question`+`question_type` and an
...
function readBatchRows(path: string): BatchReadResult {
  ...
  for (const line of raw.split('\n')) {
    ...
    // `zbrain eval longmemeval` carry an `error` field and an empty/missing
...
async function runBatchMode(parsed: ParsedArgs, opts: RunCrossModalOpts): Promise<number> {
  ...
  // end of the batch (per D10 — keeps ~/.zbrain/eval-receipts/ clean).
  const batchTempDir = mkdtempSync(join(tmpdir(), 'zbrain-batch-receipts-'));
  const runEvalFn = opts.runEval ?? runEval;
  ...
  try {
    ...
    const summaryPath = parsed.output ??
      join(zbrainPath('eval-receipts'), `cross-modal-batch-${batchSha8(summary)}.json`);
