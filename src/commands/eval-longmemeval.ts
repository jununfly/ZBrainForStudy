/**
 * v0.28.1: `zbrain eval longmemeval <dataset.jsonl>` — public LongMemEval
 ...
 * is invoked, so the user's ~/.zbrain brain is never opened. Tests stub
 ...
 * "zbrain + Haiku-preprocess pipeline" vs "zbrain alone", not directly
...
export interface RunOpts {
   ...
   * one inside this call. Production callers (the zbrain CLI) leave this
...
export async function runEvalLongMemEval(args: string[], runOpts: RunOpts = {}): Promise<void> {
  ...
  const work = async (engine: PGLiteEngine): Promise<void> => {
    ...
    for (const q of questions) {
      ...
      if (process.env.ZBRAIN_LME_DEBUG === '1') {
