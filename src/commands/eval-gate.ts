/**
 * zbrain eval gate — fail CI on retrieval regressions OR correctness drops (v0.41).
 *
 ...
 * the zbrain-version-drift bug class for source-tree CI runs.
 *
...
function printHelp(): void {
  console.log(`zbrain eval gate — fail CI on retrieval regressions or correctness drops

Usage:
  zbrain eval gate --baseline X.baseline.ndjson [flags]      # regression-only
  zbrain eval gate --qrels Y.qrels.json [flags]              # correctness-only
  zbrain eval gate --baseline X --qrels Y [flags]            # both required

Required (at least one):
  --baseline FILE              Baseline NDJSON from \`zbrain bench publish\`
