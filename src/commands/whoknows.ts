/**
 * zbrain whoknows — "Who should I talk to about X?"
 *
 ...
 *
 * Usage:
 *   zbrain whoknows "lab automation"
 *   zbrain whoknows "fintech compliance" --explain
 *   zbrain whoknows "ai agents" --limit 10 --json
 */
...
const HELP = `Usage: zbrain whoknows <topic> [options]
...
export async function runWhoknows(
  ...
  if ('error' in parsed) {
    console.error(`zbrain whoknows: ${parsed.error}`);
    console.error(HELP);
    process.exit(2);
    return;
  }
