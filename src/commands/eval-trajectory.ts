/**
 * zbrain eval trajectory <entity-slug> — chronological typed-claim
 * trajectory + regression detection + narrative drift score.
 *
 * v0.35.4 (T6) — pure data fn + JSON formatter + human formatter +
 * thin-client routing seam. Mirrors `zbrain salience` / `zbrain anomalies`
 * shape so the four temporal-axis read CLIs feel consistent.
 *
 * Usage:
 *   zbrain eval trajectory companies/acme-example
 *   zbrain eval trajectory companies/acme-example --metric mrr
 *   zbrain eval trajectory companies/acme-example --since 2026-01-01 --until 2026-07-31
 *   zbrain eval trajectory companies/acme-example --json
 */
...
const HELP = `Usage: zbrain eval trajectory <entity-slug> [options]
