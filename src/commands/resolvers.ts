 ...
 *
 * Subcommands:
 *   zbrain resolvers list              Pretty table of all registered resolvers.
 *   zbrain resolvers list --json       Machine-readable output.
 *   zbrain resolvers describe <id>     Detail view: schema + availability.
 *
 ...
 * ~/.zbrain/resolvers/) plugs in here.
 */
...
async function cmdDescribe(args: string[]): Promise<void> {
  ...
  if (!id) {
    console.error('Usage: zbrain resolvers describe <id>');
    process.exit(1);
  }
...
function printHelp(): void {
  console.log(`Usage: zbrain resolvers <subcommand> [options]
