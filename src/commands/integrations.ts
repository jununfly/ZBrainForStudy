/**
 * zbrain integrations — standalone CLI command for recipe discovery and health.
 *
 * NOT an operation (no database connection needed).
 * Reads embedded recipe files and heartbeat JSONL from ~/.zbrain/integrations/.
 *
 * ARCHITECTURE:
 ...
 *     └── (bare)  → dashboard view
 *
 *   ~/.zbrain/integrations/<id>/heartbeat.jsonl
 ...
 * legacy path that writes to ~/.zbrain/skills/). 'copy-into-host-repo'
 * recipes write their bundle into the operator's host agent repo via the
 * `zbrain integrations install` subcommand.
 */
...
// backward compat with existing callers + tests/unit/integrations.test.ts imports.
...
export async function executeHealthCheck(
  ...
  switch (check.type) {
    ...
    case 'command': {
      ...
      // (from $ZBRAIN_RECIPES_DIR or ./recipes) must NOT be able to spawn arbitrary binaries.
      if (!isEmbedded) {
...
//   TRUSTED (embedded=true):  package-bundled recipes shipped with zbrain
//     - source install: ../../recipes relative to this file
//     - global install: ~/.bun/install/global/node_modules/zbrain/recipes
...
export function getRecipeDirs(): Array<{ dir: string; trusted: boolean }> {
  ...
  const globalDir = join(homedir(), '.bun', 'install', 'global', 'node_modules', 'zbrain', 'recipes');
  if (existsSync(globalDir)) dirs.push({ dir: globalDir, trusted: true });
  if (process.env.ZBRAIN_RECIPES_DIR && existsSync(process.env.ZBRAIN_RECIPES_DIR)) {
    dirs.push({ dir: process.env.ZBRAIN_RECIPES_DIR, trusted: false });
  }
...
function heartbeatDir(id: string): string {
  return zbrainPath('integrations', id);
}
...
function cmdList(args: string[]): void {
  ...
  console.log("\n  Run 'zbrain integrations show <id>' for setup details.");
  console.log('');
}
...
function cmdShow(args: string[]): void {
  ...
  if (!id) {
    console.error('Usage: zbrain integrations show <recipe-id>');
    return;
  }
...
function cmdStatus(args: string[]): void {
  ...
  const id = args.find(a => !a.startsWith('-'));
  if (!id) {
    console.error('Usage: zbrain integrations status <recipe-id>');
    return;
  }
  ...
  if (heartbeat.length > 0) {
    ...
    if (ageMs > 24 * 60 * 60 * 1000) {
      ...
      console.log('  Run: zbrain integrations doctor');
    }
  } else {
...
function cmdTest(args: string[]): void {
  ...
  if (!filePath) {
    console.error('Usage: zbrain integrations test <recipe-file.md>');
    return;
  }
...
function printHelp(): void {
  console.log(`zbrain integrations — manage integration recipes

USAGE
  zbrain integrations                  Show integration dashboard
  zbrain integrations list [--json]    List available integrations
  zbrain integrations show <id>        Show recipe details
  zbrain integrations status <id>      Check secrets + health
  zbrain integrations doctor [--json]  Run health checks
  zbrain integrations stats [--json]   Show signal statistics
  zbrain integrations test <file>      Validate a recipe file
`);
}
...
// =============================================================================
// `zbrain integrations install <recipe-id>` — copy-into-host-repo path.
//
...
// <target>/services/voice-agent/.zbrain-source.json so future --refresh calls
...
//   - Must NOT be zbrain itself OR a parent of zbrain.
 ...
 *   - Not zbrain itself; not a parent of zbrain
...
function validateTargetRepo(
  ...
  // Refuse if target is zbrain itself or contains zbrain.
  let zbrainRoot: string | null = null;
  try {
    zbrainRoot = realpathSync(pathResolve(__dirname, '..', '..'));
  } catch {
    // ignore — non-fatal
  }
  if (zbrainRoot && (resolvedTarget === zbrainRoot || zbrainRoot.startsWith(resolvedTarget + '/'))) {
    return `refusing to install into zbrain itself (or a parent dir): ${resolvedTarget}`;
  }
...
function classifyForRefresh(
  ...
  for (const entry of manifestEntries) {
    ...
    try {
      ...
      // src missing? Skip — this would mean a manifest pointing at a missing file in zbrain.
      continue;
    }
...
function appendRefreshLog(targetVoiceAgentDir: string, event: object) {
  try {
    const logPath = pathResolve(targetVoiceAgentDir, '.zbrain-source.refresh.log');
...
/**
 * Refresh mode: read `.zbrain-source.json`, classify, apply decisions.
 */
async function refreshRecipeIntoHostRepo(
  ...
  if (!existsSync(sourceFilePath)) {
    throw new Error(`.zbrain-source.json not found at ${sourceFilePath} — this target was never installed via copy-into-host-repo; run without --refresh first`);
  }
  ...
  try {
    ...
    throw new Error(`failed to parse .zbrain-source.json: ${(err as Error).message}`);
  }

  if (recorded.recipe !== recipeId) {
    throw new Error(`.zbrain-source.json recipe="${recorded.recipe}" does not match requested recipe="${recipeId}"`);
  }
  ...
  for (const c of classifications) {
    ...
    switch (c.state) {
      ...
      case 'source-deleted': {
        // zbrain reference removed this file; offer cleanup with --auto take-theirs.
  ...
  // Re-write .zbrain-source.json with the updated SHAs.
  const gbrainVersion = (() => {
    try {
...
export async function installRecipeIntoHostRepo(
  ...
  // Write the .zbrain-source.json manifest into the target repo.
  ...
  if (manifest.resolver_rows_to_append && manifest.resolver_rows_to_append.length > 0) {
    ...
    if (resolverPath) {
      const rowsBlock = `\n\n<!-- zbrain:agent-voice:resolver-rows -->\n` +
...
async function cmdInstall(args: string[]): Promise<void> {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--target' || arg === '-t') {
      ...
      console.log('Usage:');
      console.log('  zbrain integrations install <recipe-id> --target <host-repo-path> [--overwrite] [--dry-run]');
      console.log('  zbrain integrations install <recipe-id> --target <host-repo-path> --refresh [--auto keep-mine|take-theirs] [--dry-run]');
      return;
  ...
  if (!recipeId) {
    console.error('Usage: zbrain integrations install <recipe-id> --target <host-repo-path>');
    process.exit(2);
  }
  if (!opts.target) {
