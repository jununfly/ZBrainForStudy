/**
 * `zbrain book-mirror` — flagship of the v0.25.1 skills wave.
 *
...
function printHelp(): void {
  console.log(`zbrain book-mirror — personalized chapter-by-chapter book analysis

USAGE
  zbrain book-mirror --chapters-dir <path> --slug <slug> [flags]
...
EXAMPLES
  ...
  zbrain book-mirror --chapters-dir ./chapters --slug test --dry-run
`);
}
...
async function confirmInteractive(estimateUsd: number, chapters: number): Promise<boolean> {
  if (process.stdin.isTTY !== true) {
    ...
    process.stderr.write(
      `zbrain book-mirror: refusing to spend ~$${estimateUsd.toFixed(2)} on ${chapters} chapters from a non-TTY context. ` +
      `Pass --yes to confirm.\n`
    );
    return false;
  }
...
`;
  ...
  const failedSection = opts.chapterAnalyses
    .filter(a => a.failed)
    .map(a => `> Chapter ${a.index}: analysis failed (${a.error ?? 'unknown error'}). Re-run \`zbrain book-mirror\` to retry; idempotent on the same inputs.`)
    .join('\n\n');
...
export async function runBookMirrorCmd(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  if (!flags.chaptersDir) {
    console.error('zbrain book-mirror: --chapters-dir is required. Run with --help.');
    process.exit(2);
  }
  if (!flags.slug) {
    console.error('zbrain book-mirror: --slug is required. Run with --help.');
    process.exit(2);
  }
  if (!/^[a-z0-9][a-z0-9-]*$/i.test(flags.slug)) {
    console.error(`zbrain book-mirror: invalid --slug "${flags.slug}". Use kebab-case (a-z, 0-9, hyphens).`);
    process.exit(2);
  }
  if (flags.contextFile && !fs.existsSync(flags.contextFile)) {
    console.error(`zbrain book-mirror: --context-file not found: ${flags.contextFile}`);
    process.exit(2);
  }
  ...
  try {
    ...
    console.error(`zbrain book-mirror: ${e instanceof Error ? e.message : String(e)}`);
    process.exit(2);
  }
  ...
  if (flags.dryRun) {
    process.stderr.write(`zbrain book-mirror: --dry-run — exiting without submission.\n`);
    return;
  }
  ...
  if (!flags.noConfirm) {
    ...
    if (!ok) {
      process.stderr.write(`zbrain book-mirror: cancelled by user.\n`);
      process.exit(0);
    }
  }
  ...
  if (!flags.follow) {
    ...
    process.stderr.write(
      `zbrain book-mirror: detached. Run \`zbrain jobs get <id>\` per child, then re-run with same args once all are complete.\n`
    );
    return;
  }
  ...
  if (completed === 0) {
    console.error(`zbrain book-mirror: every chapter failed. Not writing the brain page. Re-run after diagnosing.`);
    process.exit(1);
  }
