...
export async function startMcpServer(engine: BrainEngine) {
  ...
  // zbrain ops are synchronous, so we return the legacy `{ content, isError? }`
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request: any): Promise<any> => {
    ...
    // `zbrain call <op>` (sets remote=false in src/cli.ts).
    return dispatchToolCall(engine, name, params, {
      remote: true,
      takesHoldersAllowList: ['world'],
      // v0.31: source defaults to 'default' for stdio (no per-token scope).
      // Operators who want a different source on stdio MCP should set
      // ZBRAIN_SOURCE in the env or use --source via `zbrain call`.
      sourceId: process.env.ZBRAIN_SOURCE || 'default',
  ...
  const shutdown = (reason: string, code = 0) => {
    ...
    shuttingDown = true;
    process.stderr.write(`[zbrain-serve] shutdown: ${reason}\n`);
...
// Backward compat: used by `zbrain call` command (trusted local path).
// v0.31.8 (D22): accept opts.sourceId so `zbrain call --source X <op> <json>`
