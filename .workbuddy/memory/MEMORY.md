...
- **当前最前沿**：Part11 calibration 簇已收口并 push（e7d7ffe/0e93162）。Part12 cycle 大迁移：1-1 facts-extraction 簇，**1-1-1 extract-facts / 1-1-2 extract-atoms / 1-1-3 extract-takes / 1-1-4 propose-takes 均已 push**（1-1-4: d9162a4 feat + 687a4d9 chore；1-1-3: 1423330 feat + 30c5513 chore；gate 1-7 另 868eb49 fix + a454d99 chore）；**1-1-5 grade-takes 已实现待 push**（新建 grade_takes.rs + typed 方法 add_take_grade_cache/take_grade_cache_exists 三引擎；take_grade_cache 表已在 migration 0023，无需新迁移；纯函数 aggregate_ensemble 实现 T5 三模型陪审团、D17 auto_resolve 默认 off）。下一步 1-1-6 conversation-facts-backfill。**迁移范式**：cycle phase = `execute_phase` 真实 match 臂 + `autopilot/phases/<name>.rs` 模块函数（Orphans/Purge 先例）；改一个 phase 为真实臂后 `run_cycle_empty_brain` 的 skipped 断言要 -1 并加该 phase 状态断言。libsql 加 trait 方法：inherent `_impl` + 既有 impl 块内委托（开第二个 `impl BrainEngine` 块必 E0119）。
