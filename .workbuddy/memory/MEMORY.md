...
- **当前最前沿**：Part11 calibration 簇已收口并 push（e7d7ffe/0e93162）。Part12 cycle 大迁移：1-1 facts-extraction 簇，**1-1-1/1-1-2/1-1-3/1-1-4/1-1-5 均已 push**（1-1-5: 2566c75 feat + dff29e4 chore，push 687a4d9..dff29e4；1-1-4: d9162a4 + 687a4d9；1-1-3: 1423330 + 30c5513；gate 1-7 另 868eb49 + a454d99）；**1-1-6 conversation-facts-backfill 已实现、待 push**（新建 conversation_facts_backfill.rs + 补 migration 0025 双 dialect 建 op_checkpoints + 4 个 trait 方法 load/save/clear_op_checkpoint、peek_fact_row_num_start 三引擎；Windows 单测全绿：autopilot 223 pass + libsql_engine_migrations 7 pass 升 v25）。**迁移范式**：cycle phase = `execute_phase` 真实 match 臂 + `autopilot/phases/<name>.rs` 模块函数（Orphans/Purge 先例）；改一个 phase 为真实臂后 `run_cycle_empty_brain` 的 skipped 断言要 -1 并加该 phase 状态断言。libsql 加 trait 方法：inherent `_impl` + 既有 impl 块内委托（开第二个 `impl BrainEngine` 块必 E0119）。
