...
function seedPack(packName: string, format: 'json' | 'yaml', initial?: Partial<SchemaPackManifest>): string {
  // ZBRAIN_HOME=/tmp/x → zbrainPath('schema-packs', 'mine') = /tmp/x/.zbrain/schema-packs/mine
  const dir = join(tmpDir, '.zbrain', 'schema-packs', packName);
...
beforeEach(() => {
  ...
  tmpDir = mkdtempSync(join(tmpdir(), 'zbrain-mutate-test-'));
  auditDir = mkdtempSync(join(tmpdir(), 'zbrain-mutate-audit-'));
  lockDir = mkdtempSync(join(tmpdir(), 'zbrain-mutate-locks-'));
});
...
describe('locateMutablePackFile — bundled guard', () => {
  it('rejects zbrain-base with PACK_READONLY + fork hint', () => {
    expect(() => locateMutablePackFile('zbrain-base')).toThrow(SchemaPackMutationError);
    try { locateMutablePackFile('zbrain-base'); } catch (e) {
      const err = e as SchemaPackMutationError;
      expect(err.code).toBe('PACK_READONLY');
      expect(err.message).toContain('zbrain schema fork');
    }
  });

  it('rejects zbrain-recommended with PACK_READONLY', () => {
    try { locateMutablePackFile('zbrain-recommended'); } catch (e) {
      expect((e as SchemaPackMutationError).code).toBe('PACK_READONLY');
    }
  });

  it('BUNDLED_PACK_NAMES export contains both bundled packs', () => {
    expect(BUNDLED_PACK_NAMES.has('zbrain-base')).toBe(true);
    expect(BUNDLED_PACK_NAMES.has('zbrain-recommended')).toBe(true);
...
describe('addTypeToPack', () => {
  it('appends a new type to JSON pack and writes atomically', async () => {
    await withEnv({ ZBRAIN_HOME: tmpDir, ZBRAIN_AUDIT_DIR: auditDir }, async () => {
...
describe('YAML round-trip', () => {
  it('mutating a YAML pack preserves YAML format and reparses cleanly', async () => {
    await withEnv({ ZBRAIN_HOME: tmpDir, ZBRAIN_AUDIT_DIR: auditDir }, async () => {
      const dir = join(tmpDir, '.zbrain', 'schema-packs', 'yaml-pack');
      mkdirSync(dir, { recursive: true });
      ...
      const yamlBody = `api_version: zbrain-schema-pack-v1
name: yaml-pack
version: 1.0.0
description: ""
zbrain_min_version: 0.38.0
