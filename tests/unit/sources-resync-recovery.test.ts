 ...
 * tests/unit/e2e/sources-remote-mcp.test.ts.
 */
...
const FAKE_GIT_DIR = join(tmpdir(), `zbrain-resync-test-${process.pid}`);
...
describe('validateRepoState — full state matrix (sync re-clone driver)', () => {
  ...
  test('url-drift: remote points elsewhere', async () => {
    await withEnv(
      { ZBRAIN_HOME, PATH: fakePath(), REMOTE_GET_URL_OUTPUT: 'https://github.com/different/repo' },
      async () => {
        const row = await addSource(engine, {
          id: 'state-drift',
          remoteUrl: 'https://github.com/example/repo',
