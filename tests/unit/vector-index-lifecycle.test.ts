...
describe('checkActiveBuild', () => {
  ...
  test('Postgres with an active build returns the row', async () => {
    const fakeEngine = {
      kind: 'postgres' as const,
      executeRaw: async () => [
        { pid: 12345, query: 'CREATE INDEX CONCURRENTLY idx_chunks_embedding ON ...', application_name: 'zbrain' },
      ],
    } as never;
    const r = await checkActiveBuild(fakeEngine, 'idx_chunks_embedding');
    expect(r.active).toBe(true);
    expect(r.pid).toBe(12345);
    expect(r.application_name).toBe('zbrain');
  });
...
describe('isSupabaseAutoMaintenance', () => {
  ...
  test('false for zbrain', () => {
    expect(isSupabaseAutoMaintenance({ active: true, application_name: 'zbrain-worker' })).toBe(false);
  });
    ...
    const fakeEngine = {
      ...
      executeRaw: async (sql: string) => {
        if (sql.includes('pg_stat_activity')) {
          return [{ pid: 555, query: 'CREATE INDEX zombie_idx_a ...', application_name: 'zbrain' }];
        }
        if (sql.includes('pg_index')) {
...
describe('dropAndRebuild — A3 atomic-swap', () => {
  test('PGLite: no-op returns rebuilt: false', async () => {
    ...
    const r = await dropAndRebuild(fakeEngine, spec, { reason: 'tests/unit' });
