/**
 * v0.39 behavioral coverage for `zbrain doctor`'s check orchestrator.
 *
 ...
 * subprocess smoke at tests/unit/doctor-cli-smoke.test.ts which covers the
...
describe('buildChecks — orchestrator against PGLite', () => {
  ...
  test('snapshot: load-bearing check names always run against a fresh brain', async () => {
    ...
    // JSONL file exists. Tests run on isolated tmpdir ZBRAIN_HOMEs where
