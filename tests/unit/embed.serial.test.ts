...
// Proxy-based mock engine that matches tests/unit/import-file.test.ts pattern.
...
afterEach(() => {
  delete process.env.ZBRAIN_EMBED_CONCURRENCY;
  delete process.env.ZBRAIN_EMBED_TIME_BUDGET_MS;
});
...
describe('runEmbed --all (parallel)', () => {
  test('runs embedBatch calls concurrently across pages', async () => {
    ...
    process.env.ZBRAIN_EMBED_CONCURRENCY = '10';
  ...
  test('respects ZBRAIN_EMBED_CONCURRENCY=1 (serial)', async () => {
    ...
    process.env.ZBRAIN_EMBED_CONCURRENCY = '1';
  ...
  test('skips pages whose chunks are all already embedded when --stale', async () => {
    ...
    process.env.ZBRAIN_EMBED_CONCURRENCY = '5';
...
describe('runEmbedCore --dry-run never calls the embedding model', () => {
  ...
  test('non-dry-run path reports accurate embedded count (regression guard)', async () => {
    ...
    process.env.ZBRAIN_EMBED_CONCURRENCY = '2';
...
describe('embedAllStale wall-clock budget end-to-end (D3 + D3a)', () => {
  test('ZBRAIN_EMBED_TIME_BUDGET_MS=N cuts the outer loop short on stuck workers', async () => {
    const { runEmbedCore } = await import('../../src/commands/embed.ts');
    ...
    process.env.ZBRAIN_EMBED_TIME_BUDGET_MS = '100';
    process.env.ZBRAIN_EMBED_CONCURRENCY = '1';
