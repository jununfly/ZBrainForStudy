...
describe('executeHealthCheck', () => {
  ...
  test('http health_check blocks AWS metadata endpoint for embedded recipes', async () => {
    const result = await executeHealthCheck(
      { type: 'http', url: 'http://169.254.169.254/latests/unit/meta-data/iam/security-credentials/', label: 'aws' },
      'test-id',
      true,
    );
    expect(result.status).toBe('blocked');
...
describe('isInternalUrl', () => {
  ...
  test('blocks AWS EC2 metadata', () => { expect(isInternalUrl('http://169.254.169.254/latests/unit/')).toBe(true); });
...
describe('getRecipeDirs (B1 trust boundary)', () => {
  ...
  test('cwd/recipes fallback is NOT trusted', () => {
    ...
    for (const d of dirs) {
      if (d.dir === process.env.ZBRAIN_RECIPES_DIR) {
