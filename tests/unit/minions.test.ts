...
describe('MinionWorker', () => {
  test('register handler', () => {
    ...
    worker.register('tests/unit', async () => ({ ok: true }));
    expect(worker.registeredNames).toContain('tests/unit');
  });
...
describe('backpressure-audit (v0.19.1 Q1): JSONL on coalesce', () => {
  test('logBackpressureCoalesce writes one JSONL line per coalesce', async () => {
    ...
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'zbrain-audit-'));
    const prev = process.env.ZBRAIN_AUDIT_DIR;
    process.env.ZBRAIN_AUDIT_DIR = tmp;
    try {
      ...
      if (prev === undefined) delete process.env.ZBRAIN_AUDIT_DIR;
      else process.env.ZBRAIN_AUDIT_DIR = prev;
...
describe('MinionWorker: abort signal propagation (v0.20.5)', () => {
  ...
  test('handler ignoring abort signal still gets abort fired', async () => {
    ...
    // tests; the E2E test in tests/unit/e2e/worker-abort-recovery.test.ts covers
...
describe('MinionWorker: self-health-check', () => {
  test('health check is active when ZBRAIN_SUPERVISED is not set', async () => {
    // Save and clear the env var
    const saved = process.env.ZBRAIN_SUPERVISED;
    delete process.env.ZBRAIN_SUPERVISED;
    ...
    try {
      ...
      if (saved !== undefined) process.env.ZBRAIN_SUPERVISED = saved;
      else delete process.env.ZBRAIN_SUPERVISED;
    }
  }, 10_000);

  test('health check is skipped when ZBRAIN_SUPERVISED=1', async () => {
    const saved = process.env.ZBRAIN_SUPERVISED;
    process.env.ZBRAIN_SUPERVISED = '1';
