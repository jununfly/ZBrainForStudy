 ...
 * initialization on PGLite. Postgres parity is covered by tests/unit/e2e/schema-drift.test.ts.
 */
  ...
  test('default-inherited rows show chunker_version=1', async () => {
    ...
    await engine.executeRaw(
      `INSERT INTO pages (slug, type, title, source_path)
         VALUES ('tests/unit/cjk-migration', 'note', 'Test', 'tests/unit/cjk-migration.md')
         ON CONFLICT DO NOTHING`,
    );
    const rows = await engine.executeRaw<{ chunker_version: number; source_path: string | null }>(
      `SELECT chunker_version, source_path FROM pages WHERE slug = 'tests/unit/cjk-migration'`,
    );
    expect(rows.length).toBe(1);
    expect(Number(rows[0].chunker_version)).toBe(1);
    expect(rows[0].source_path).toBe('tests/unit/cjk-migration.md');
  });
});
