 ...
 *
 * The helper is unit-tested in tests/unit/takes-weight-rounding.test.ts. This
...
beforeAll(async () => {
  ...
  await engine.putPage('tests/unit/weight-integration', {
    type: 'note', title: 'integration', compiled_truth: 'b', frontmatter: {},
  });
  const rows = await engine.executeRaw<{ id: number }>(
    `SELECT id FROM pages WHERE slug = 'tests/unit/weight-integration' LIMIT 1`,
  );
  pageId = rows[0].id;
});
