...
describe('extractCodeRefs — basic patterns', () => {
  ...
  test('recognizes all seeded directory prefixes', () => {
    const cases = [
      'src/foo.ts', 'lib/foo.ts', 'app/foo.ts', 'tests/unit/foo.ts', 'tests/foo.ts',
...
describe('extractCodeRefs — integration with zbrain workflow', () => {
...
\`~/.zbrain/sync-failures.jsonl\`. The retry path is covered in
...
`;
    ...
    expect(paths.has('tests/unit/sync.test.ts')).toBe(true);
