 ...
 *
 * Hermetic — drives the file system + ZBRAIN_HOME + HOME envs directly via
 ...
 *   - ZBRAIN_HOME override outside any worktree — ok
 */
...
beforeEach(() => {
  scratch = join(tmpdir(), `zbrain-doctor-hw-${Date.now()}-${Math.random().toString(36).slice(2)}`);
 ...
 *  ZBRAIN_HOME, capture stdout AND prevent runDoctor's `process.exit(N)` from
...
describe('home_dir_in_worktree doctor check', () => {
  test('zbrain home outside any worktree → ok', async () => {
    // scratch/.zbrain — no parent has a .git, scratch IS our fake $HOME
    const home = scratch;
    const gbrainParent = home;
    const check = await getCheck('home_dir_in_worktree', {
      HOME: home,
      ZBRAIN_HOME: gbrainParent,
    });
    expect(check).toBeDefined();
    expect(check!.status).toBe('ok');
  });

  test('zbrain home inside dir-style .git worktree → warn', async () => {
    // scratch/home/myrepo/.git/    (directory)
    // scratch/home/myrepo/.zbrain/ ← zbrain home is inside the worktree
  ...
  test('zbrain home inside .git-AS-FILE linked worktree → warn (F4)', async () => {
  ...
  test('walk terminates at $HOME — .git ABOVE $HOME does NOT trigger warn (F4)', async () => {
    ...
    // scratch/home/.zbrain/  (no worktree below $HOME)
  ...
  test('ZBRAIN_HOME override pointing outside any worktree → ok', async () => {
    // Real $HOME might be inside a worktree, but the user pointed
    // ZBRAIN_HOME at a clean location. Doctor should report ok.
    const home = scratch;
