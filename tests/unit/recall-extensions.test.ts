/**
 * v0.32 — `zbrain recall` extensions: --since-last-run + --pending + --rollup
 ...
 * `tests/unit/thin-client-routing.test.ts` because those paths exercise the
...
// body inside withEnv({ ZBRAIN_HOME: tmpHome }) so process.env mutations are
// scoped + restored via try/finally instead of leaking across files).
function makeTmpHome(): string {
  return join(tmpdir(), `zbrain-recall-test-${process.pid}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
}
...
describe('briefing skill invocation surface', () => {
  ...
  //   zbrain recall --since-last-run --supersessions --pending --rollup --json
  //
  ...
  // covered by tests/unit/thin-client-routing.test.ts.
