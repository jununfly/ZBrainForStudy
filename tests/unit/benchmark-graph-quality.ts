 ...
 * makes zbrain measurably better for real-world questions.
 *
 ...
 *
 * Usage: bun run tests/unit/benchmark-graph-quality.ts
 *        bun run tests/unit/benchmark-graph-quality.ts --json   (machine-readable output)
 */
...
async function measureRanking(
  ...
  // searchKeyword joins content_chunks (a normal `zbrain import` populates
...
async function main() {
  ...
  if (failed.length > 0) {
    ...
    console.error('\nSee BENCHMARK_FAILURES comment block in tests/unit/benchmark-graph-quality.ts for fixes.');
    process.exit(1);
  } else {
