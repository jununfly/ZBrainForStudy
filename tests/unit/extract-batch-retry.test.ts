...
//   - Retry-matcher extension recognizes zbrain's ZBrainError shape
...
// Minimal ZBrainError shape — mirror the typed problem/detail fields used
// by db.ts:getConnection so the retry-matcher extension recognizes it.
class FakeZBrainError extends Error {
  problem: string;
  detail: string;
...
describe('isRetryableConnError extension (v0.41.2.1)', () => {
  test('ZBrainError with problem="No database connection" is retryable', () => {
    const err = new FakeZBrainError('No database connection', 'connect() has not been called');
    expect(isRetryableConnError(err)).toBe(true);
  });

  test('ZBrainError with other problem is NOT retryable', () => {
    const err = new FakeZBrainError('Schema mismatch', 'expected vector(1536), got vector(1024)');
...
describe('withRetry primitive (v0.41.2.1)', () => {
  ...
  test('retries on ZBrainError "No database connection"; second succeeds', async () => {
    let calls = 0;
    const result = await withRetry(
      async () => {
        calls++;
        if (calls === 1) throw new FakeZBrainError('No database connection', 'connect() has not been called');
        return 'ok';
      },
      { delayMs: 0 },
    );
