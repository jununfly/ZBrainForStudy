...
describe('pruneDir', () => {
  ...
  test('blocks ops (zbrain operational dir)', () => {
    expect(pruneDir('ops')).toBe(false);
  });

  test('blocks *.raw sidecar dirs (zbrain convention)', () => {
    expect(pruneDir('.raw')).toBe(false);
...
describe('pathToSlug', () => {
  ...
  test('handles nested paths', () => {
    expect(pathToSlug('projects/zbrain/spec.md')).toBe('projects/zbrain/spec');
  });
...
describe('performSync dry-run never writes', () => {
  ...
  beforeEach(async () => {
    ...
    repoPath = mkdtempSync(join(tmpdir(), 'zbrain-sync-dryrun-'));
