...
describe('v0.41.2 R-MIG: take_domain_assignments migration v94', () => {
  ...
  test('composite PK (take_id, domain) rejects duplicate (take, domain) pair', async () => {
    ...
    await engine.putPage('tests/unit/seed-1', {
      title: 'seed',
      type: 'person',
      compiled_truth: '',
      frontmatter: {},
    ...
    const pageRow = await engine.executeRaw<{ id: number }>(
      `SELECT id FROM pages WHERE slug = 'tests/unit/seed-1' LIMIT 1`
    );
    ...
    await engine.executeRaw(
      `INSERT INTO take_domain_assignments (take_id, domain, pack) VALUES ($1, 'deal_success', 'zbrain-investor')`,
      [takeId]
    );
  ...
  test('multi-domain assignment for same take is permitted', async () => {
    await engine.putPage('tests/unit/seed-multi', {
      title: 'seed',
      type: 'person',
      compiled_truth: '',
      frontmatter: {},
    ...
    const pageRow = await engine.executeRaw<{ id: number }>(
      `SELECT id FROM pages WHERE slug = 'tests/unit/seed-multi' LIMIT 1`
    );
    const pageId = pageRow[0].id;
    await engine.executeRaw(
    ...
    await engine.executeRaw(
      `INSERT INTO take_domain_assignments (take_id, domain, pack) VALUES ($1, 'market_call', 'zbrain-investor')`,
      [takeId]
    );
    const rows = await engine.executeRaw<{ count: number }>(
  ...
  test('FK ON DELETE CASCADE removes assignments when take is deleted', async () => {
    await engine.putPage('tests/unit/seed-cascade', {
      title: 'seed',
      type: 'person',
      compiled_truth: '',
      frontmatter: {},
    ...
    const pageRow = await engine.executeRaw<{ id: number }>(
      `SELECT id FROM pages WHERE slug = 'tests/unit/seed-cascade' LIMIT 1`
    );
    const pageId = pageRow[0].id;
    await engine.executeRaw(
  ...
  test('CHECK constraint rejects confidence outside [0, 1]', async () => {
    await engine.putPage('tests/unit/seed-check', {
      title: 'seed',
      type: 'person',
      compiled_truth: '',
      frontmatter: {},
    ...
    const pageRow = await engine.executeRaw<{ id: number }>(
      `SELECT id FROM pages WHERE slug = 'tests/unit/seed-check' LIMIT 1`
    );
    const pageId = pageRow[0].id;
    await engine.executeRaw(
    ...
    try {
      await engine.executeRaw(
        `INSERT INTO take_domain_assignments (take_id, domain, pack, confidence) VALUES ($1, 'deal_success', 'zbrain-investor', 1.5)`,
        [takeId]
      );
    } catch {
      threw = true;
    ...
    try {
      await engine.executeRaw(
        `INSERT INTO take_domain_assignments (take_id, domain, pack, confidence) VALUES ($1, 'deal_success', 'zbrain-investor', -0.1)`,
        [takeId]
      );
    } catch {
      threw = true;
  ...
  test('aggregator JOIN direction returns assignments per domain', async () => {
    ...
    for (let i = 1; i <= 3; i++) {
      await engine.putPage(`tests/unit/agg-${i}`, {
        title: `seed ${i}`,
        type: 'person',
        compiled_truth: '',
        frontmatter: {},
      ...
      const pageRow = await engine.executeRaw<{ id: number }>(
        `SELECT id FROM pages WHERE slug = 'tests/unit/agg-${i}' LIMIT 1`
      );
      const pageId = pageRow[0].id;
      await engine.executeRaw(
      ...
      await engine.executeRaw(
        `INSERT INTO take_domain_assignments (take_id, domain, pack) VALUES ($1, $2, 'zbrain-investor')`,
        [takeId, domain]
      );
    }
  ...
  test('pre-existing takes without assignment co-exist (backward compat)', async () => {
    await engine.putPage('tests/unit/legacy-take', {
      title: 'legacy',
      type: 'person',
      compiled_truth: '',
      frontmatter: {},
    ...
    const pageRow = await engine.executeRaw<{ id: number }>(
      `SELECT id FROM pages WHERE slug = 'tests/unit/legacy-take' LIMIT 1`
    );
    const pageId = pageRow[0].id;
    await engine.executeRaw(
