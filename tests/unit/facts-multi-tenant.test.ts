...
describe('cross-source isolation', () => {
  test('listFactsByEntity returns only the requested source', async () => {
    await engine.insertFact(
      { fact: 'alpha alice fact', kind: 'fact', entity_slug: 'people/alice-example', source: 'tests/unit' },
      { source_id: 'alpha' },
    );
    await engine.insertFact(
      { fact: 'beta alice fact', kind: 'fact', entity_slug: 'people/alice-example', source: 'tests/unit' },
      { source_id: 'beta' },
    );
  ...
  test('listFactsBySession scopes by source_id', async () => {
    await engine.insertFact(
      { fact: 'alpha topic-X', kind: 'fact', source: 'tests/unit', source_session: 'topic-X' },
      { source_id: 'alpha' },
    );
    await engine.insertFact(
      { fact: 'beta topic-X', kind: 'fact', source: 'tests/unit', source_session: 'topic-X' },
      { source_id: 'beta' },
    );
  ...
  test('CASCADE delete on sources cleans up facts', async () => {
    ...
    await engine.insertFact(
      { fact: 'will cascade', kind: 'fact', source: 'tests/unit' },
      { source_id: 'ephemeral' },
    );
