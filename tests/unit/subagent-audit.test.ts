 ...
 * the readback path used by `zbrain agent logs`. No real engine; purely
 * filesystem.
 */
...
let tmpDir: string;
const savedAuditDir = process.env.ZBRAIN_AUDIT_DIR;
...
afterAll(() => {
  if (savedAuditDir === undefined) delete process.env.ZBRAIN_AUDIT_DIR;
  else process.env.ZBRAIN_AUDIT_DIR = savedAuditDir;
...
describe('logSubagentHeartbeat', () => {
  ...
  test('best-effort: write failure does not throw', () => {
    ...
    process.env.ZBRAIN_AUDIT_DIR = bogus;
    try {
