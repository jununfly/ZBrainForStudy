...
describe('buildHardExcludeClause', () => {
  ...
  test('emits NOT (col LIKE ... OR col LIKE ...)', () => {
    const result = buildHardExcludeClause('p.slug', ['tests/unit/', 'archive/']);
    expect(result).toBe(`AND NOT (p.slug LIKE 'tests/unit/%' OR p.slug LIKE 'archive/%')`);
  });
...
describe('parseHardExcludesEnv', () => {
  test('parses comma-separated prefixes', () => {
    expect(parseHardExcludesEnv('tests/unit/,scratch/,private/'))
      .toEqual(['tests/unit/', 'scratch/', 'private/']);
  });
  ...
  test('trims whitespace and drops empty entries', () => {
    expect(parseHardExcludesEnv(' tests/unit/ , , scratch/ ')).toEqual(['tests/unit/', 'scratch/']);
  });
});
...
describe('resolveHardExcludes', () => {
  ...
  test('caller exclude_slug_prefixes adds to the union', () => {
    ...
    expect(r).toContain('tests/unit/'); // default still present
  });

  test('include_slug_prefixes opts back in', () => {
    const r = resolveHardExcludes(undefined, ['tests/unit/'], undefined);
    expect(r).not.toContain('tests/unit/');
    // Other defaults still present.
    expect(r).toContain('archive/');
  });

  test('env ZBRAIN_SEARCH_EXCLUDE adds to the union', () => {
