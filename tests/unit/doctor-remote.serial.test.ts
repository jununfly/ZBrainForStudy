 ...
 *
 * Strategy: spin up a tiny in-process HTTP server that mimics `zbrain serve --http`
...
import type { ZBrainConfig } from '../../src/core/config.ts';
...
// covered separately in tests/unit/oauth-scope-probe.test.ts (pure-function
...
function makeConfig(overrides: Partial<NonNullable<ZBrainConfig['remote_mcp']>> = {}): ZBrainConfig {
  return {
    engine: 'postgres',
    remote_mcp: {
...
describe('collectRemoteDoctorReport', () => {
  ...
  test('missing client_secret entirely — fails before any HTTP call', async () => {
    ...
    await withEnv({ ZBRAIN_REMOTE_CLIENT_SECRET: undefined }, async () => {
      ...
      expect(creds.message).toContain('ZBRAIN_REMOTE_CLIENT_SECRET');
  ...
  test('missing remote_mcp on config — fails config_integrity', async () => {
    reset();
    const config: ZBrainConfig = { engine: 'postgres' };
    const report = await collectRemoteDoctorReport(config);
    expect(report.status).toBe('fail');
  ...
  test('env var ZBRAIN_REMOTE_CLIENT_SECRET overrides config-file secret', async () => {
    reset();
    await withEnv({ ZBRAIN_REMOTE_CLIENT_SECRET: 'env-supplied-secret' }, async () => {
...
// by tests/unit/thin-client-upgrade-prompt.test.ts. Here we verify the network-error
...
describe('runUpgradeDriftCheck', () => {
  ...
  test('major drift, no prior state → warn with auto-upgrade fix hint', async () => {
    ...
    const tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-doctor-drift-'));
    try {
      await withEnv({ ZBRAIN_HOME: tmpHome }, async () => {
        ...
        expect(result.message).toContain('Run `zbrain upgrade`');
  ...
  test('major drift with prior_failed state → warn with manual-install fix hint', async () => {
    ...
    try {
      ...
      const stateDir = join(tmpHome, '.zbrain');
      ...
      await withEnv({ ZBRAIN_HOME: tmpHome }, async () => {
        ...
        expect(result.message).toContain('Prior `zbrain upgrade` did not advance');
        expect(result.message).toContain('https://github.com/garrytan/zbrain/releases');
        expect(result.message).not.toContain('Run `zbrain upgrade`');
