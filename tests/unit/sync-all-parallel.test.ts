 ...
 * `zbrain sources status` dashboard.
 *
 * Why this exists:
 ...
 *        the per-source lock `zbrain-sync:<source_id>`, NOT the global
 *        `zbrain-sync` lock. Closes the bug class where `sync --all`
...
describe('buildSyncStatusReport', () => {
  ...
  // tests/unit/e2e/sync-status-pglite.test.ts and exercises real SQL.
