...
function runGate(cwd: string): { code: number; stdout: string; stderr: string } {
  // ZBRAIN_SCAN_ROOT pins the gate's scan directory to our fake repo.
  ...
  // failed under shard-concurrency load) into the real zbrain repo and
  ...
  const r = spawnSync('bash', [SCRIPT_PATH], {
    ...
    encoding: 'utf-8',
    timeout: 30_000,
    env: { ...process.env, ZBRAIN_SCAN_ROOT: cwd },
  });
  return {
    code: r.status ?? -1,
    stdout: r.stdout ?? '',
...
async function reconcile(engine: BrainEngine) {
  await engine.insertFact({ fact: 'ok' } as any, { source_id: 'default' }); // zbrain-allow-direct-insert: this is the canonical reconcile path
}
`,
        'utf-8',
      );
  ...
  test('allow-list comment on a DIFFERENT line does NOT exempt the call', () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        `// zbrain-allow-direct-insert: misplaced comment on the wrong line
import type { BrainEngine } from './engine.ts';
...
describe('check-system-of-record.sh — scope correctness', () => {
  test('does NOT scan tests/unit/ — tests legitimately call engine.insertFact for fixtures (Codex R2-#8)', () => {
    const fakeRepo = mkdtempSync(join(tmpdir(), 'gate-test-test-scope-'));
    try {
      spawnSync('git', ['init', '-q'], { cwd: fakeRepo });
      // src/ is clean; tests/unit/ has a violation. The gate should pass.
      const fakeTest = join(fakeRepo, 'tests/unit');
