/**
 * v0.37.7.0 #1167 — `zbrain import --source-id <id>` routes to a brain source.
 *
 * Pre-fix, `zbrain import --source dept-x ./pages` silently fell back to
...
describe('import --source-id (#1167)', () => {
  ...
  beforeEach(async () => {
    ...
    scratchDir = mkdtempSync(join(tmpdir(), 'zbrain-import-src-'));
  ...
  test('--source-id value is NOT treated as a positional dir arg', async () => {
    ...
    // Should NOT throw "Usage: zbrain import <dir>..." because scratchDir
