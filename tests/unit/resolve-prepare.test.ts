...
describe('resolvePrepare', () => {
  afterEach(() => {
    delete process.env.ZBRAIN_PREPARE;
  });
  ...
  test('ZBRAIN_PREPARE=false overrides everything', () => {
    process.env.ZBRAIN_PREPARE = 'false';
    expect(
      resolvePrepare('postgresql://user:pass@host:5432/db?prepare=true'),
    ).toBe(false);
  });

  test('ZBRAIN_PREPARE=true overrides auto-detect on 6543', () => {
    process.env.ZBRAIN_PREPARE = 'true';
    expect(resolvePrepare('postgresql://user:pass@host:6543/db')).toBe(true);
  });

  test('ZBRAIN_PREPARE=0 is falsy', () => {
    process.env.ZBRAIN_PREPARE = '0';
    expect(resolvePrepare('postgresql://user:pass@host:6543/db')).toBe(false);
  });
