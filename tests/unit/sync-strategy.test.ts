...
describe('isSyncable with strategy', () => {
  ...
  test('exclude globs blacklist specific patterns', () => {
    ...
    expect(isSyncable('tests/unit/foo.test.ts', { strategy: 'code', exclude: ['**/*.test.ts'] })).toBe(false);
  });
...
describe('pathToSlug with pageKind', () => {
  ...
  test('repoPrefix is prepended (markdown and code)', () => {
    expect(pathToSlug('guides/foo.md', 'zbrain')).toBe('zbrain/guides/foo');
    expect(pathToSlug('src/foo.ts', 'zbrain', { pageKind: 'code' })).toBe('zbrain/src-foo-ts');
  });
});
