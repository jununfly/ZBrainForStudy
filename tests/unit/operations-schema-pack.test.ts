...
beforeEach(async () => {
  ...
  _resetPackLocatorForTests();
  tmpDir = mkdtempSync(join(tmpdir(), 'zbrain-ops-schema-test-'));
  auditDir = mkdtempSync(join(tmpdir(), 'zbrain-ops-schema-audit-'));
});
...
function seedPack(packName: string): string {
  ...
  writeFileSync(path, `api_version: zbrain-schema-pack-v1
name: ${packName}
version: 1.0.0
description: ""
...
describe('get_active_schema_pack', () => {
  it('returns identity packet for the bundled zbrain-base pack', async () => {
    await withEnv({ ZBRAIN_HOME: tmpDir, ZBRAIN_SCHEMA_PACK: undefined }, async () => {
