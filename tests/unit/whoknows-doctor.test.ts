 ...
 * ZBRAIN_WHOKNOWS_FIXTURE_PATH is set. It does NOT need an engine.
...
describe('whoknows_health doctor check', () => {
  it('resolves the default fixture when cwd is not the repo', async () => {
    try {
      await withEnv({ ZBRAIN_WHOKNOWS_FIXTURE_PATH: undefined }, async () => {
  ...
  it('warns when env override fixture file is missing entirely', async () => {
    try {
      ...
      await withEnv({ ZBRAIN_WHOKNOWS_FIXTURE_PATH: fixturePath }, async () => {
        const check = await whoknowsHealthCheck(stubEngine);
        expect(check.name).toBe('whoknows_health');
  ...
  it('warns when fixture exists but is empty', async () => {
    try {
      const fixturePath = join(workDir, 'tests/unit/fixtures/whoknows-eval.jsonl');
      mkdirSync(join(workDir, 'tests/unit/fixtures'), { recursive: true });
