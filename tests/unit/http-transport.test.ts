 ...
 * round-trip lives in tests/unit/e2e/http-transport.test.ts.
 */
...
async function startTest(cfg: FakeEngineConfig & { lruCap?: number; ipLimit?: number; tokenLimit?: number; corsOrigin?: string; bodyCap?: number; trustProxy?: boolean } = {}): Promise<TestServer> {
  if (cfg.corsOrigin) process.env.ZBRAIN_HTTP_CORS_ORIGIN = cfg.corsOrigin;
  else delete process.env.ZBRAIN_HTTP_CORS_ORIGIN;
  if (cfg.bodyCap) process.env.ZBRAIN_HTTP_MAX_BODY_BYTES = String(cfg.bodyCap);
  else delete process.env.ZBRAIN_HTTP_MAX_BODY_BYTES;
  if (cfg.trustProxy) process.env.ZBRAIN_HTTP_TRUST_PROXY = '1';
  else delete process.env.ZBRAIN_HTTP_TRUST_PROXY;
...
describe('http-transport: auth', () => {
  ...
  beforeAll(async () => {
    srv = await startTest({
      validTokens: new Map([[hash(VALID_TOKEN), { id: 'tok-1', name: 'tests/unit' }]]),
...
describe('http-transport: CORS', () => {
  test('10. no ZBRAIN_HTTP_CORS_ORIGIN + browser request → no ACAO header', async () => {
