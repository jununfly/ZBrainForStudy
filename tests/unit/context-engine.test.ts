/**
 * Tests for the zbrain-context OpenClaw context engine.
 *
 * Validates:
...
import { createZBrainContextEngine, ENGINE_ID, ENGINE_NAME, __resetSdkLoadStateForTests } from '../../src/core/context-engine.ts';
...
function makeWorkspace(opts: WorkspaceOpts = {}) {
  const dir = mkdtempSync(join(tmpdir(), 'zbrain-ce-test-'));
...
describe('zbrain-context engine', () => {
  let tmpDir: string;
  ...
  it('has correct engine info', () => {
    ...
    const engine = createZBrainContextEngine({ workspaceDir: tmpDir });
  ...
  it('ingest is a no-op that returns ingested: true', async () => {
    ...
    const result = await engine.ingest({
      sessionId: 'test-session',
      message: { role: 'user', content: 'tests/unit' } as any,
    });
  ...
  it('handles missing workspace files gracefully', async () => {
    tmpDir = mkdtempSync(join(tmpdir(), 'zbrain-ce-test-'));
