...
describe('insertFact + listFactsByEntity', () => {
  test('inserts a fact and reads it back', async () => {
    const r = await engine.insertFact(
      { fact: 'alice example fact', kind: 'fact', entity_slug: 'people/alice-example', source: 'tests/unit' },
      { source_id: 'default' },
    );
  ...
  test('respects kind CHECK', async () => {
    const r = await engine.insertFact(
      { fact: 'durable', kind: 'preference', entity_slug: 'alice-test', source: 'tests/unit' },
      { source_id: 'default' },
    );
  ...
  test('supersede path: superseding row marks old as expired_at + superseded_by', async () => {
    const old = await engine.insertFact(
      { fact: 'old fact', kind: 'fact', entity_slug: 'super-test', source: 'tests/unit' },
      { source_id: 'default' },
    );
    const newer = await engine.insertFact(
      { fact: 'new fact', kind: 'fact', entity_slug: 'super-test', source: 'tests/unit' },
...
describe('expireFact', () => {
  test('returns true on first call, false on idempotent re-call', async () => {
    const r = await engine.insertFact(
      { fact: 'will expire', kind: 'fact', source: 'tests/unit' },
      { source_id: 'default' },
    );
...
describe('listFactsSince + listFactsBySession', () => {
  test('listFactsSince filters by created_at', async () => {
    ...
    await engine.insertFact(
      { fact: 'recent', kind: 'fact', source: 'tests/unit', source_session: 'topic-since' },
      { source_id: 'default' },
    );
  ...
  test('listFactsBySession filters by source_session', async () => {
    await engine.insertFact(
      { fact: 'topic-A note', kind: 'fact', source: 'tests/unit', source_session: 'topic-A' },
      { source_id: 'default' },
    );
    await engine.insertFact(
      { fact: 'topic-B note', kind: 'fact', source: 'tests/unit', source_session: 'topic-B' },
      { source_id: 'default' },
    );
...
describe('findCandidateDuplicates', () => {
  test('entity-prefiltered: rows from other entities never returned', async () => {
    await engine.insertFact(
      { fact: 'alice fact', kind: 'fact', entity_slug: 'cand-alice', source: 'tests/unit' },
      { source_id: 'default' },
    );
    await engine.insertFact(
      { fact: 'tim fact', kind: 'fact', entity_slug: 'cand-tim', source: 'tests/unit' },
      { source_id: 'default' },
    );
  ...
  test('k cap honored', async () => {
    for (let i = 0; i < 7; i++) {
      await engine.insertFact(
        { fact: `cap-test ${i}`, kind: 'fact', entity_slug: 'cap-entity', source: 'tests/unit' },
        { source_id: 'default' },
      );
    }
  ...
  test('embedding cosine ordering when both sides have embeddings', async () => {
    await engine.insertFact(
      { fact: 'A', kind: 'fact', entity_slug: 'embed-test', source: 'tests/unit', embedding: vec(1, 0, 0) },
      { source_id: 'default' },
    );
    await engine.insertFact(
      { fact: 'B', kind: 'fact', entity_slug: 'embed-test', source: 'tests/unit', embedding: vec(0, 1, 0) },
      { source_id: 'default' },
    );
...
describe('consolidateFact', () => {
  test('marks consolidated_at + consolidated_into; never DELETE', async () => {
    ...
    const fact = await engine.insertFact(
      { fact: 'will be consolidated', kind: 'fact', entity_slug: 'cons-test', source: 'tests/unit' },
      { source_id: 'default' },
    );
