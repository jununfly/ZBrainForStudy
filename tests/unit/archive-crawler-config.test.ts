 ...
 * `archive-crawler.scan_paths:` is explicitly set in zbrain.yml.
 * These tests pin every gate in that contract:
 *   - missing zbrain.yml -> missing_section
 *   - zbrain.yml without the section -> missing_section
...
function writeYaml(content: string): string {
  const path = join(workdir, 'zbrain.yml');
...
describe('loadArchiveCrawlerConfig — D12 missing_section', () => {
  ...
  it('throws missing_section when zbrain.yml does not exist', () => {
  ...
  it('throws missing_section when zbrain.yml exists but has no archive-crawler section', () => {
    writeYaml('storage:\n  db_tracked:\n    - originals/\n');
    expect(() => loadArchiveCrawlerConfig(workdir)).toThrow(ArchiveCrawlerConfigError);
    try {
      loadArchiveCrawlerConfig(workdir);
