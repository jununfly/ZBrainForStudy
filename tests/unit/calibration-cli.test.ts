/**
 * v0.36.1.0 (T7) — zbrain calibration CLI + get_calibration_profile MCP op tests.
 *
...
describe('formatProfileText', () => {
  test('null profile prints helpful cold-brain message', () => {
    ...
    expect(out).toContain('zbrain dream --phase calibration_profile');
  });
...
describe('getCalibrationProfileOp (MCP)', () => {
  ...
  test('throws on empty/non-string holder', async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(ZBrainError);
      expect((err as ZBrainError).problem).toBe('INVALID_HOLDER');
    }
  });
});
