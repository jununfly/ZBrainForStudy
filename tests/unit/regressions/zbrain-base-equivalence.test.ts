// v0.38 zbrain-base byte-for-byte equivalence gate (T5 + T25 CI-blocking).
//
// zbrain-base.yaml MUST reproduce pre-v0.38 hardcoded behavior exactly.
...
// validation time).
//
// If this test fails, zbrain-base.yaml drifted from the source-of-truth
// constants. Either the YAML needs to be updated to match a deliberate
// constant change, or the constant change is unintentional and zbrain-base
...
const BASE_PATH = join(import.meta.dir, '../../../src/core/schema-pack/base/zbrain-base.yaml');

describe('zbrain-base v0.38 parity gate', () => {
  test('every ALL_PAGE_TYPES seed appears in zbrain-base page_types', () => {
  ...
  test('person + company are the only expert_routing default types', () => {
    ...
    // zbrain-base must reproduce this default; user packs opt in others
  ...
  test('alias graph is EMPTY by default (E8 codex F8)', () => {
    // zbrain-base ships with NO alias edges so existing search behavior
  ...
  test('codegen --check passes in process', () => {
    ...
    const pack = loadPackFromFile(BASE_PATH);
    expect(pack.name).toBe('zbrain-base');
