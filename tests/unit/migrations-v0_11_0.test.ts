 ...
 * live in tests/unit/e2e/migration-flow.test.ts (Lane C-5).
 */
...
beforeEach(() => {
  ...
  tmp = mkdtempSync(join(tmpdir(), 'zbrain-v0_11_0-test-'));
...
describe('cron manifest rewrite — zbrain builtins only', () => {
  test('rewrites `agentTurn` entries whose skill is a zbrain builtin', () => {
    const dir = join(tmp, '.claude');
    ...
    expect(after.jobs[0].cmd).toContain('zbrain jobs submit extract');
    expect(after.jobs[0]._zbrain_migrated_by).toBe('v0.11.0');
    expect(after.jobs[1].kind).toBe('shell');
    expect(after.jobs[1].cmd).toContain('zbrain jobs submit backlinks');
  });
