 ...
 * stomp the real ~/.zbrain/migrations/completed.jsonl.
 */
...
const originalGbrainHome = process.env.ZBRAIN_HOME;

beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-migration-resume-'));
  // preferences.ts's gbrainDir() returns `$HOME/.zbrain` when ZBRAIN_HOME
  // is unset. Set HOME only; clear any inherited ZBRAIN_HOME so the test
  // body matches the migrations dir at `$tmpHome/.zbrain/migrations/`.
...
afterEach(() => {
  ...
  if (originalGbrainHome) process.env.ZBRAIN_HOME = originalGbrainHome;
