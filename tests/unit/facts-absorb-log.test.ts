...
// database connection' fired loudly after every zbrain capture. Per CQ1
// suppress via typed access (instanceof ZBrainError && .problem) NOT
...
describe('writeFactsAbsorbLog — WARN-4 disconnected-engine suppression (CQ1 + CV13)', () => {
  ...
  // the same ZBrainError shape src/core/db.ts:151-158 throws.
    ...
    return {
      engine: {
        logIngest: async () => {
          ...
          const { ZBrainError } = await import('../../src/core/types.ts');
  ...
  test('ZBrainError problem="No database connection" SUPPRESSED after first occurrence', async () => {
  ...
  test('ZBrainError with DIFFERENT problem still warns loudly (not the suppressed class)', async () => {
    const { _resetFactsAbsorbDisconnectedFlagForTests } = await import('../../src/core/facts/absorb-log.ts');
    _resetFactsAbsorbDisconnectedFlagForTests();
    ...
    try {
      const { ZBrainError } = await import('../../src/core/types.ts');
      const otherError = new ZBrainError(
  ...
  test('non-ZBrainError (plain Error) still warns loudly', async () => {
    const { _resetFactsAbsorbDisconnectedFlagForTests } = await import('../../src/core/facts/absorb-log.ts');
    _resetFactsAbsorbDisconnectedFlagForTests();
