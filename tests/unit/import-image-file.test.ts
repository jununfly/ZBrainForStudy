...
//
// Real-API embedding is exercised in tests/unit/e2e/voyage-multimodal.test.ts (gated
...
beforeAll(async () => {
  ...
  tmpDir = mkdtempSync(join(tmpdir(), 'zbrain-img-test-'));
});
...
describe('importImageFile happy path (noEmbed)', () => {
  test('imports a PNG fixture, creates a single image chunk + files row', async () => {
    ...
    copyFileSync('tests/unit/fixtures/images/tiny.avif', target);
