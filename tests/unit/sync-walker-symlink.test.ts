 ...
 *
 * Closes the bug class where `zbrain sync --strategy code` could hang on
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'zbrain-walker-'));
});
...
describe('collectSyncableFiles symlink + cycle hardening', () => {
  test('1. self-referencing symlink does not loop', async () => {
    await withEnv({ ZBRAIN_EMBEDDING_MULTIMODAL: undefined }, () => {
  ...
  test('6. multimodal preservation under markdown strategy (codex C5)', async () => {
    ...
    await withEnv({ ZBRAIN_EMBEDDING_MULTIMODAL: 'true' }, () => {
