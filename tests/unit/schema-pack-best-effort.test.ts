...
beforeEach(() => {
  ...
  _resetPackLocatorForTests();
  tmpDir = mkdtempSync(join(tmpdir(), 'zbrain-best-effort-test-'));
});
...
describe('loadActivePackBestEffort', () => {
  it('returns ResolvedPack when load succeeds (default bundled zbrain-base)', async () => {
    // No locator override → defaults to bundled zbrain-base resolution.
    await withEnv({ ZBRAIN_HOME: tmpDir, ZBRAIN_SCHEMA_PACK: undefined }, async () => {
      ...
      expect(result?.manifest.name).toBe('zbrain-base');
    });
  });

  it('returns null when the resolved pack is not on disk', async () => {
    __setPackLocatorForTests(() => null);
    await withEnv({ ZBRAIN_HOME: tmpDir, ZBRAIN_SCHEMA_PACK: 'never-installed' }, async () => {
      const result = await loadActivePackBestEffort(fakeCtx());
  ...
  it('returns null when the pack file is corrupt', async () => {
    ...
    await withEnv({ ZBRAIN_HOME: tmpDir, ZBRAIN_SCHEMA_PACK: 'corrupt-pack' }, async () => {
      const result = await loadActivePackBestEffort(fakeCtx());
      expect(result).toBeNull();
    });
  });
