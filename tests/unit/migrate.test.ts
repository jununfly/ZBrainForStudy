...
describe('migrate', () => {
  ...
  // and are covered in the E2E suite (tests/unit/e2e/mechanical.test.ts)
});
...
// ─────────────────────────────────────────────────────────────────
//
// The base schema shipped 8 zbrain-managed public tables without RLS
...
describe('migration v35 — auto_rls_event_trigger structural guards', () => {
  ...
  test('backfill exemption regex matches the doctor.ts contract', () => {
    const v35 = MIGRATIONS.find(m => m.version === 35);
    const sql = ((v35?.sqlFor as any)?.postgres ?? '') as string;
    // doctor.ts:418 EXEMPT_RE = /^ZBRAIN:RLS_EXEMPT\s+reason=\S.{3,}/
    // The plpgsql side must use the same pattern (via ~) so the two surfaces
    // honor identical exemptions.
    expect(sql).toMatch(/'\^ZBRAIN:RLS_EXEMPT\\s\+reason=\\S\.\{3,\}'/);
  });
...
describe('migrate — runner behavioral (v14 handler + v15 backfill)', () => {
  ...
  test('v15 backfilled any max_stalled=1 rows (smoke: schema default is 5)', async () => {
    await (engine as any).db.exec(
      `INSERT INTO minion_jobs (name, queue, status, max_stalled) VALUES ('tests/unit', 'default', 'waiting', 1)`
    );
    await (engine as any).db.exec(
    ...
    const rows = await (engine as any).db.query(
      `SELECT max_stalled FROM minion_jobs WHERE name = 'tests/unit'`
    );
  ...
  test('v67 columns are nullable — existing facts persist with NULL typed fields (backward compat)', async () => {
    ...
    await db.exec(
      `INSERT INTO facts (source_id, entity_slug, fact, source, valid_from)
       VALUES ('v67-test', 'v67/example', 'plain non-typed claim', 'tests/unit', now())`,
    );
...
// ─────────────────────────────────────────────────────────────────
// resolvePoolSize — ZBRAIN_POOL_SIZE env override
// ─────────────────────────────────────────────────────────────────
//
// Guards the Bug 2 fix: users on constrained poolers (Supabase port 6543)
// must be able to cap the pool size via ZBRAIN_POOL_SIZE. The default
...
describe('resolvePoolSize — env var + explicit override', () => {
  ...
  const original = process.env.ZBRAIN_POOL_SIZE;

  afterAll(() => {
    if (original === undefined) delete process.env.ZBRAIN_POOL_SIZE;
    else process.env.ZBRAIN_POOL_SIZE = original;
  });

  test('returns 10 default when unset and no explicit override', () => {
    delete process.env.ZBRAIN_POOL_SIZE;
    expect(resolvePoolSize()).toBe(10);
  });

  test('reads ZBRAIN_POOL_SIZE as an integer', () => {
    process.env.ZBRAIN_POOL_SIZE = '2';
    expect(resolvePoolSize()).toBe(2);
    process.env.ZBRAIN_POOL_SIZE = '5';
    expect(resolvePoolSize()).toBe(5);
  });

  test('ignores invalid ZBRAIN_POOL_SIZE values', () => {
    process.env.ZBRAIN_POOL_SIZE = 'not-a-number';
    expect(resolvePoolSize()).toBe(10);
    process.env.ZBRAIN_POOL_SIZE = '0';
    expect(resolvePoolSize()).toBe(10);
    process.env.ZBRAIN_POOL_SIZE = '-1';
    expect(resolvePoolSize()).toBe(10);
  });
  ...
  test('explicit argument wins over env + default', () => {
    ...
    process.env.ZBRAIN_POOL_SIZE = '7';
    expect(resolvePoolSize(3)).toBe(3);
  });
});
...
describe('PR #356 — 57014 catch path emits actionable 4-part diagnostic', () => {
  test('runMigrations surfaces SQLSTATE 57014 with fix + verify steps', async () => {
    ...
    // ZBRAIN_MIGRATE_BACKOFF_MS=0 in test env to skip the 5s/15s wait
    ...
    const original = process.env.ZBRAIN_MIGRATE_BACKOFF_MS;
    process.env.ZBRAIN_MIGRATE_BACKOFF_MS = '0';
    ...
    if (original === undefined) delete process.env.ZBRAIN_MIGRATE_BACKOFF_MS;
    else process.env.ZBRAIN_MIGRATE_BACKOFF_MS = original;
    ...
    expect(joined).toContain('zbrain doctor --locks');
    expect(joined).toContain('zbrain apply-migrations --yes');
    expect(joined).toContain('Verify:');
    expect(joined).toContain('zbrain doctor');
...
describe('migration v48 — takes_weight_round_to_grid (v0.32)', () => {
  ...
  test('uses transaction:false (codex review #2 — non-blocking, idempotent via WHERE)', () => {
    ...
    // from a long transaction so other zbrain processes can interleave.
    const v48 = MIGRATIONS.find(m => m.version === 48);
...
describe('migration v49 — eval_takes_quality_runs (v0.32)', () => {
  ...
  // DB-authoritative receipts table for `zbrain eval takes-quality`.
...
// resolveSessionTimeouts — ZBRAIN_*_TIMEOUT env overrides
// ─────────────────────────────────────────────────────────────────
//
...
describe('resolveSessionTimeouts — env var overrides', () => {
  ...
  const origStatement = process.env.ZBRAIN_STATEMENT_TIMEOUT;
  const origIdleTx = process.env.ZBRAIN_IDLE_TX_TIMEOUT;
  const origCheck = process.env.ZBRAIN_CLIENT_CHECK_INTERVAL;
  ...
  afterAll(() => {
    ...
    restore('ZBRAIN_STATEMENT_TIMEOUT', origStatement);
    restore('ZBRAIN_IDLE_TX_TIMEOUT', origIdleTx);
    restore('ZBRAIN_CLIENT_CHECK_INTERVAL', origCheck);
  });

  const resetEnv = () => {
    delete process.env.ZBRAIN_STATEMENT_TIMEOUT;
    delete process.env.ZBRAIN_IDLE_TX_TIMEOUT;
    delete process.env.ZBRAIN_CLIENT_CHECK_INTERVAL;
  };
  ...
  test('env vars override the defaults', () => {
    resetEnv();
    process.env.ZBRAIN_STATEMENT_TIMEOUT = '10min';
    process.env.ZBRAIN_IDLE_TX_TIMEOUT = '30s';
    process.env.ZBRAIN_CLIENT_CHECK_INTERVAL = '15s';
    const t = resolveSessionTimeouts();
  ...
  test("'0' disables a specific GUC", () => {
    resetEnv();
    process.env.ZBRAIN_STATEMENT_TIMEOUT = '0';
    const t = resolveSessionTimeouts();
  ...
  test("'off' disables a specific GUC", () => {
    resetEnv();
    process.env.ZBRAIN_IDLE_TX_TIMEOUT = 'off';
    const t = resolveSessionTimeouts();
    expect(t.statement_timeout).toBe('5min');
  ...
  async function tryResolve(
    ...
    try {
      await engine.executeRaw(
        ...
        [quality, outcome, 'zbrain:test', pageId, rowNum],
      );
      return { ok: true };
    } catch (err) {
...
// reference probe contract separately at tests/unit/schema-bootstrap-coverage.test.ts.
...
describe('migrate v81 — pages_provenance_columns', () => {
  ...
  test('v81 does NOT create any index (provenance is admin-surface only)', () => {
    ...
    // surface only (admin SPA Sources tab + zbrain doctor
...
describe('migrate v81 — round-trip on PGLite', () => {
  ...
  test('directly UPDATE-ing the provenance columns succeeds (no constraint blocks)', async () => {
    ...
    const slug = `wiki/tests/unit/v81-update-${Date.now()}`;
    await engine.putPage(slug, {
      type: 'note',
...
// rows. The migration is the substrate behind v0.40.2.0's `zbrain think`
  ...
  test('insert + SELECT event_type round-trips through facts', async () => {
    await engine.executeRaw(
      `INSERT INTO facts (
        ...
        '2026-04-15T00:00:00Z', 'tests/unit', 'sess-v89',
  ...
  test('NULL event_type round-trips (legacy + metric rows)', async () => {
    await engine.executeRaw(
      `INSERT INTO facts (
        ...
        '2026-04-01T00:00:00Z', 'tests/unit', 'sess-v89-metric',
