...
describe('LocalStorage', () => {
  ...
  beforeAll(() => {
    tmpDir = mkdtempSync(join(tmpdir(), 'zbrain-storage-test-'));
  ...
  test('upload creates file', async () => {
    await storage.upload('tests/unit/file.txt', Buffer.from('hello'));
    expect(existsSync(join(tmpDir, 'tests/unit/file.txt'))).toBe(true);
  });

  test('download returns uploaded data', async () => {
    await storage.upload('tests/unit/roundtrip.bin', Buffer.from('binary data'));
    const data = await storage.download('tests/unit/roundtrip.bin');
  ...
  test('exists returns true for uploaded file', async () => {
    await storage.upload('tests/unit/exists.txt', Buffer.from('x'));
    expect(await storage.exists('tests/unit/exists.txt')).toBe(true);
  });
  ...
  test('delete removes file', async () => {
    await storage.upload('tests/unit/deleteme.txt', Buffer.from('x'));
    await storage.delete('tests/unit/deleteme.txt');
    expect(await storage.exists('tests/unit/deleteme.txt')).toBe(false);
  });
  ...
  test('getUrl returns file:// URL', async () => {
    const url = await storage.getUrl('tests/unit/file.txt');
...
describe('LocalStorage path traversal', () => {
  test('blocks upload path traversal via ../', async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'zbrain-traversal-'));
    try {
...
describe('createStorage', () => {
  test('creates LocalStorage for backend: local', async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'zbrain-factory-test-'));
    try {
      const storage = await createStorage({ backend: 'local', bucket: 'tests/unit', localPath: tmpDir });
  ...
  test('throws for unknown backend', async () => {
    expect(createStorage({ backend: 'unknown' as any, bucket: 'tests/unit' })).rejects.toThrow('Unknown storage backend');
  });

  test('S3Storage requires credentials', async () => {
    expect(createStorage({ backend: 's3', bucket: 'tests/unit' })).rejects.toThrow('accessKeyId');
  });

  test('SupabaseStorage requires projectUrl', async () => {
    expect(createStorage({ backend: 'supabase', bucket: 'tests/unit' })).rejects.toThrow('projectUrl');
  });
});
