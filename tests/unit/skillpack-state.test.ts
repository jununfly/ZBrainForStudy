 ...
 * at ~/.zbrain/skillpack-state.json (codex G1).
 */
...
function makeEntry(over: Partial<SkillpackStateEntry> = {}): SkillpackStateEntry {
  return {
    ...
    workspace: '/Users/tests/unit/workspace',
...
describe('loadState — error paths', () => {
  ...
  test('throws state_schema_unknown on mismatched schema version', () => {
    writeFileSync(statePath, JSON.stringify({ schema_version: 'zbrain-skillpack-state-v99', packs: [] }));
    try {
      loadState({ statePath });
      throw new Error('should have thrown');
    } catch (err) {
