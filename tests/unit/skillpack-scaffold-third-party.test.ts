 ...
 *
 * Git + tarball sources are exercised in tests/unit/skillpack-remote-source.test.ts
...
function buildPackFixture(root: string, opts: FixtureOptions = {}): SkillpackManifest {
  ...
  if (opts.withBootstrap) {
    ...
    writeFileSync(
      ...
      '1. agent: zbrain put_page wiki/example\n2. show user: "Pack installed."\n',
    );
  }
  return manifest;
}
...
describe('runScaffoldThirdParty — happy path', () => {
  ...
  test('displays bootstrap.md when declared and present', async () => {
    ...
    expect(result.bootstrap.text).toContain('agent: zbrain put_page wiki/example');
  });
});

describe('runScaffoldThirdParty — zbrain version gate', () => {
  test('rejects when current version is below zbrain_min_version', async () => {
    freshSandbox();
    buildPackFixture(packDir, { manifestOverrides: { zbrain_min_version: '99.0.0' } });
    const resolved = resolveSource(packDir);
    ...
    try {
      ...
      expect((err as ScaffoldThirdPartyError).code).toBe('zbrain_version_too_old');
    }
  });

  test('accepts exact-version match', async () => {
    freshSandbox();
    buildPackFixture(packDir, { manifestOverrides: { zbrain_min_version: '0.36.0' } });
    const resolved = resolveSource(packDir);
