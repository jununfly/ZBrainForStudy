 ...
 * post-upgrade) lays down `~/.zbrain/.gitignore` containing the single line
...
beforeEach(() => {
  testHome = join(tmpdir(), `zbrain-eg-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('ensureGitignore', () => {
  test('creates ~/.zbrain/.gitignore with single * when missing', async () => {
    await withEnv({ ZBRAIN_HOME: testHome }, async () => {
      ensureGitignore();
  ...
  test('honors ZBRAIN_HOME: writes under $ZBRAIN_HOME/.zbrain/, not $HOME', async () => {
    await withEnv({ ZBRAIN_HOME: testHome }, async () => {
      ensureGitignore();
      const expected = join(testHome, '.zbrain', '.gitignore');
