 ...
 * `tests/unit/helpers/with-env.ts` per the project's R1 lint rule. State-
 * file tests scope `ZBRAIN_HOME` to a per-test tmpdir so they don't
 * touch the developer's real `~/.zbrain/`.
 */
...
describe('loadAlerted', () => {
  test('returns empty Map when alerted.json does not exist', async () => {
    ...
    await withEnv({ ZBRAIN_HOME: stateRoot, OPENCLAW_TELEGRAM_GROUP: '-100' }, async () => {
      const Detector = await loadDetector();
      const d = new Detector();
...
// ─── ZBRAIN_HOME state path override (NEW, D2 verification) ────────────

test('ZBRAIN_HOME env override redirects state file to env-pointed dir', async () => {
  const customRoot = makeStateDir();
  await withEnv({ ZBRAIN_HOME: customRoot, OPENCLAW_TELEGRAM_GROUP: '-100' }, async () => {
    const Detector = await loadDetector();
    const d = new Detector();
