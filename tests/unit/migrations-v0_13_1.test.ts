 ...
 * in tests/unit/apply-migrations.test.ts.
 */
...
// ~/.zbrain/config.json. We can't easily stand that up in a test, so the
...
// with a real engine + config is in tests/unit/e2e/migration-flow.test.ts.
//
// Idempotency behavior is verified by unit testing the writer path
// (tests/unit/writer.test.ts: "validators skip pages with validate:false
