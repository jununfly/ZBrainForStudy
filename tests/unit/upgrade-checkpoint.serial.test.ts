...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-upgrade-checkpoint-test-'));
  originalHome = process.env.ZBRAIN_HOME;
  process.env.ZBRAIN_HOME = tmpHome;
});

afterEach(() => {
  if (originalHome === undefined) delete process.env.ZBRAIN_HOME;
  else process.env.ZBRAIN_HOME = originalHome;
...
describe('writeCheckpoint + loadCheckpoint round-trip', () => {
  ...
  test('loadCheckpoint returns null for malformed JSON', () => {
    ...
    writeCheckpoint(cp);
    // Corrupt the file. zbrainPath resolves to ZBRAIN_HOME/.zbrain/<file>.
    const path = join(tmpHome, '.zbrain', 'upgrade-checkpoint.json');
