...
describe('inspectLock', () => {
  test('returns null when no row exists', async () => {
    const snap = await inspectLock(engine, 'zbrain-sync:test-source');
    expect(snap).toBeNull();
  });

  test('returns LockSnapshot shape after tryAcquireDbLock', async () => {
    const handle = await tryAcquireDbLock(engine, 'zbrain-sync:default');
    expect(handle).not.toBeNull();
    const snap = await inspectLock(engine, 'zbrain-sync:default');
    expect(snap).not.toBeNull();
    expect(snap!.id).toBe('zbrain-sync:default');
  ...
  test('ttl_expired=true after the TTL has elapsed', async () => {
    ...
    const snap = await inspectLock(engine, 'zbrain-sync:stale');
    expect(snap).not.toBeNull();
...
describe('listStaleLocks', () => {
  test('returns empty when no stale rows exist', async () => {
    const handle = await tryAcquireDbLock(engine, 'zbrain-sync:fresh');
    expect(handle).not.toBeNull();
  ...
  test('returns only rows where ttl_expires_at < NOW()', async () => {
    ...
    expect(ids).toEqual(['zbrain-sync:stale-A', 'zbrain-sync:stale-B']);
  ...
  test('orders by acquired_at ascending', async () => {
    ...
    expect(stale[0].id).toBe('zbrain-sync:older-stale');
    expect(stale[1].id).toBe('zbrain-sync:newer-stale');
  });
});

describe('deleteLockRow', () => {
  test('deletes the row + RETURNING returns it when (id, pid) matches', async () => {
    const handle = await tryAcquireDbLock(engine, 'zbrain-sync:to-delete');
    expect(handle).not.toBeNull();
    const result = await deleteLockRow(engine, 'zbrain-sync:to-delete', process.pid);
    expect(result.deleted).toBe(true);
    // Row should be gone.
    const snap = await inspectLock(engine, 'zbrain-sync:to-delete');
    expect(snap).toBeNull();
  ...
  test('refuses to delete when pid does not match (preserves cross-host safety)', async () => {
    ...
    const result = await deleteLockRow(engine, 'zbrain-sync:wrong-pid', 22222);
    expect(result.deleted).toBe(false);
    // Row should still exist.
    const snap = await inspectLock(engine, 'zbrain-sync:wrong-pid');
    expect(snap).not.toBeNull();
  ...
  test('atomic shape: single round trip (RETURNING in same statement)', async () => {
    ...
    const handle = await tryAcquireDbLock(engine, 'zbrain-sync:atomic-test');
    expect(handle).not.toBeNull();
    const r = await deleteLockRow(engine, 'zbrain-sync:atomic-test', process.pid);
    expect(r.deleted).toBe(true);
    // Calling again is a no-op (idempotent).
    const r2 = await deleteLockRow(engine, 'zbrain-sync:atomic-test', process.pid);
