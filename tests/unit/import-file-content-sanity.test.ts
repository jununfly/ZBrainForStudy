...
/** Wrap an importFromContent call with ZBRAIN_HOME + ZBRAIN_AUDIT_DIR
 *  pointed at fresh tempdirs so config and audit writes don't leak
 *  between tests or pollute the developer's real ~/.zbrain. */
async function withIsolatedHome<T>(fn: () => Promise<T>): Promise<T> {
  zbrainHomeDir = mkdtempSync(join(tmpdir(), 'cs-gate-home-'));
  auditDir = mkdtempSync(join(tmpdir(), 'cs-gate-audit-'));
  try {
    return await withEnv({
      ZBRAIN_HOME: zbrainHomeDir,
      ZBRAIN_AUDIT_DIR: auditDir,
    }, fn);
  } finally {
    rmSync(zbrainHomeDir, { recursive: true, force: true });
...
Body.`;
      await expect(
        importFromContent(engine, 'tests/unit/junk', content, { noEmbed: true })
  ...
  test('throws with PAGE_JUNK_PATTERN-tagged message for classifyErrorCode', async () => {
    await withIsolatedHome(async () => {
      ...
      try {
        await importFromContent(engine, 'tests/unit/ray', content, { noEmbed: true });
      } catch (e) {
        caught = e as Error;
      }
...
`;
      try {
        await importFromContent(engine, 'tests/unit/404', content, { noEmbed: true });
      } catch { /* expected */ }
      const page = await engine.getPage('tests/unit/404');
...
describe('importFromContent — soft-block (D9 transition + embed_skip)', () => {
  test('soft-block writes page with embed_skip frontmatter marker', async () => {
    await withIsolatedHome(async () => {
      ...
      const result = await importFromContent(engine, 'tests/unit/big', content, { noEmbed: true });
      expect(result.status).not.toBe('error');
      const page = await engine.getPage('tests/unit/big');
  ...
  test('soft-block deletes existing chunks (D9 transition invariant)', async () => {
    await withIsolatedHome(async () => {
      ...
      await importFromContent(engine, 'tests/unit/grow', small, { noEmbed: true });
      const beforeChunks = await engine.getChunks('tests/unit/grow');
      ...
      await importFromContent(engine, 'tests/unit/grow', big, { noEmbed: true });
      const afterChunks = await engine.getChunks('tests/unit/grow');
  ...
  test('soft-block skips chunking entirely (no new chunks created)', async () => {
    await withIsolatedHome(async () => {
      const content = FRONTMATTER + 'a'.repeat(600_000);
      await importFromContent(engine, 'tests/unit/big2', content, { noEmbed: true });
      const chunks = await engine.getChunks('tests/unit/big2');
...
describe('importFromContent — kill-switch bypass', () => {
  test('ZBRAIN_NO_SANITY=1 lets junk through with bypass audit + stderr', async () => {
    const zbrainHomeDirLocal = mkdtempSync(join(tmpdir(), 'cs-bypass-home-'));
    const auditDirLocal = mkdtempSync(join(tmpdir(), 'cs-bypass-audit-'));
    try {
      await withEnv({
        ZBRAIN_HOME: zbrainHomeDirLocal,
        ZBRAIN_AUDIT_DIR: auditDirLocal,
        ZBRAIN_NO_SANITY: '1',
      }, async () => {
        const content = `---
title: 'Attention Required! | Cloudflare'
...
junk body`;
        const result = await importFromContent(engine, 'tests/unit/bypass', content, { noEmbed: true });
        expect(result.status).not.toBe('error');
        const page = await engine.getPage('tests/unit/bypass');
        expect(page).not.toBeNull();
      ...
      rmSync(zbrainHomeDirLocal, { recursive: true, force: true });
...
describe('importFromContent — normal pages unaffected', () => {
  test('clean page imports successfully', async () => {
    await withIsolatedHome(async () => {
      ...
      const result = await importFromContent(engine, 'tests/unit/clean', content, { noEmbed: true });
      expect(result.status).toBe('imported');
      const page = await engine.getPage('tests/unit/clean');
      expect(page).not.toBeNull();
      const fm = page!.frontmatter as Record<string, unknown>;
      expect(isEmbedSkipped(fm)).toBe(false);
    });
  ...
  test('warn-tier page (50K-500K body) lands normally without embed_skip', async () => {
    await withIsolatedHome(async () => {
      ...
      const result = await importFromContent(engine, 'tests/unit/warn', content, { noEmbed: true });
      expect(result.status).toBe('imported');
      const page = await engine.getPage('tests/unit/warn');
      expect(page).not.toBeNull();
      const fm = page!.frontmatter as Record<string, unknown>;
      expect(isEmbedSkipped(fm)).toBe(false);
    });
