...
const TEST_PID_FILE = '/tmp/zbrain-supervisor-test.pid';
...
function makeHarness(name: string, workerBody: string): IntegrationHarness {
  const tmpRoot = join(tmpdir(), `zbrain-sup-test-${name}-${process.pid}-${Date.now()}`);
...
function readAudit(auditDir: string) {
  const origEnv = process.env.ZBRAIN_AUDIT_DIR;
  process.env.ZBRAIN_AUDIT_DIR = auditDir;
  try {
    return readSupervisorEvents();
  } finally {
    if (origEnv === undefined) delete process.env.ZBRAIN_AUDIT_DIR;
    else process.env.ZBRAIN_AUDIT_DIR = origEnv;
  }
}
...
describe('MinionSupervisor', () => {
  ...
  describe('integration: env-var inheritance regression (codex #9 / eng #8)', () => {
    it('strips inherited ZBRAIN_ALLOW_SHELL_JOBS when allowShellJobs=false, even if parent has it set', async () => {
      const outFile = join(tmpdir(), `zbrain-sup-env-${process.pid}-${Date.now()}.txt`);
      ...
      const h = makeHarness('env-strip-outfile', `printf '%s\\n' "\${ZBRAIN_ALLOW_SHELL_JOBS-UNSET}" > "$OUT_FILE" ; exit 1`);

      try {
        const sup = spawnSupervisor(h, {
          OUT_FILE: outFile,
          ZBRAIN_ALLOW_SHELL_JOBS: '1',  // parent has it
    ...
    it('DOES pass ZBRAIN_ALLOW_SHELL_JOBS to child when allowShellJobs is true', async () => {
      const outFile = join(tmpdir(), `zbrain-sup-env-ok-${process.pid}-${Date.now()}.txt`);
      try { unlinkSync(outFile); } catch { /* may not exist */ }
  ...
  describe('integration: ZBRAIN_SUPERVISED env var (v0.22.14)', () => {
    it('sets ZBRAIN_SUPERVISED=1 on spawned worker child', async () => {
      const outFile = join(tmpdir(), `zbrain-sup-supervised-${process.pid}-${Date.now()}.txt`);
      try { unlinkSync(outFile); } catch { /* may not exist */ }

      // exit 1 required post-D1/D2 to trip SUP_MAX_CRASHES=1; clean exits
      // no longer count toward the crash limit.
      const h = makeHarness('supervised-env', `printf '%s\n' "\${ZBRAIN_SUPERVISED-UNSET}" > "$OUT_FILE" ; exit 1`);
  ...
  describe('integration: --max-rss spawn args (v0.21)', () => {
    it('passes --max-rss 2048 to spawned worker by default', async () => {
      const outFile = join(tmpdir(), `zbrain-sup-maxrss-${process.pid}-${Date.now()}.txt`);
      try { unlinkSync(outFile); } catch { /* may not exist */ }
