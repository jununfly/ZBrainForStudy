...
const FAKE_GIT_DIR = join(tmpdir(), `zbrain-git-remote-test-${process.pid}`);
...
describe('parseRemoteUrl — CGNAT 100.64/10 (Tailscale)', () => {
  test('rejected by default', async () => {
    await withEnv({ ZBRAIN_ALLOW_PRIVATE_REMOTES: undefined }, async () => {
      try {
  ...
  test('accepted with ZBRAIN_ALLOW_PRIVATE_REMOTES=1', async () => {
    await withEnv({ ZBRAIN_ALLOW_PRIVATE_REMOTES: '1' }, async () => {
