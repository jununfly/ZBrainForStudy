/**
 * Tests for `zbrain init --mcp-only` — thin-client setup branch.
 *
 ...
 *
 * Each test sets `ZBRAIN_HOME` to a fresh tempdir so the config write is
 * isolated and we can inspect the resulting `~/.zbrain/config.json` without
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'zbrain-init-mcp-only-'));
  discoveryStatus = 200;
  tokenStatus = 200;
  mcpStatus = 200;
});
...
async function run(args: string[], extraEnv: Record<string, string | undefined> = {}): Promise<RunResult> {
  ...
  delete env.ZBRAIN_REMOTE_ISSUER_URL;
  delete env.ZBRAIN_REMOTE_MCP_URL;
  delete env.ZBRAIN_REMOTE_CLIENT_ID;
...
function configPath(): string { return join(tmp, '.zbrain', 'config.json'); }

describe('zbrain init --mcp-only — happy path', () => {
  test('writes remote_mcp config and creates NO local DB', async () => {
    ...
    expect(existsSync(join(tmp, '.zbrain', 'brain.pglite'))).toBe(false);
  ...
  test('env-var-supplied secret is NOT persisted to config file', async () => {
    const r = await run([
      ...
      '--mcp-url', `http://127.0.0.1:${port}/mcp`,
      '--oauth-client-id', 'cid',
    ], { ZBRAIN_REMOTE_CLIENT_SECRET: 'env-secret' });
    expect(r.exitCode).toBe(0);
    const cfg = JSON.parse(readFileSync(configPath(), 'utf-8'));
    expect(cfg.remote_mcp).toBeDefined();
...
describe('zbrain init --mcp-only — required-flag errors', () => {
...
describe('zbrain init --mcp-only — pre-flight smoke failures', () => {
...
describe('zbrain init re-run guard', () => {
  function seedThinClientConfig() {
    mkdirSync(join(tmp, '.zbrain'), { recursive: true });
  ...
  test('default `zbrain init` (no flags) refuses when remote_mcp is set', async () => {
  ...
  test('`zbrain init --pglite` refuses when remote_mcp is set', async () => {
    seedThinClientConfig();
  ...
  test('`zbrain init --mcp-only` (no --force) refuses when remote_mcp is already set', async () => {
    seedThinClientConfig();
    const r = await run([
      'init', '--mcp-only', '--json',
      '--issuer-url', `http://127.0.0.1:${port}`,
  ...
  test('`zbrain init --mcp-only --force` overwrites existing thin-client config', async () => {
    seedThinClientConfig();
    const r = await run([
