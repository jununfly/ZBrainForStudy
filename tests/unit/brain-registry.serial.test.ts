...
describe('validateMountId', () => {
  ...
  test('rejects empty / non-string', () => {
    expect(() => validateMountId('')).toThrow(ZBrainError);
    expect(() => validateMountId(null as unknown as string)).toThrow(ZBrainError);
    expect(() => validateMountId(undefined as unknown as string)).toThrow(ZBrainError);
    expect(() => validateMountId(42 as unknown as string)).toThrow(ZBrainError);
  });
...
describe('BrainRegistry — lazy init', () => {
  ...
  test('empty/null/undefined id routes to host', async () => {
    ...
    //
    // Hermeticity: dev machines often have a real ~/.zbrain/config.json
    // (the maintainer's own brain). Without ZBRAIN_HOME isolation, the
    ...
    // ZBRAIN_HOME to a guaranteed-empty tempdir so host-init has nothing
    ...
    process.env.ZBRAIN_HOME = isolatedHome;
    try {
      const reg = new BrainRegistry([]);
      // Expect the host-init path to be attempted (it'll fail on missing
      // <isolated>/.zbrain/config.json, but the error will come from
      ...
      if (savedHome !== undefined) process.env.ZBRAIN_HOME = savedHome;
      else delete process.env.ZBRAIN_HOME;
    }
  });
});
