...
//
// Parity gate: `inferTypeFromPack(path, zbrain-base)` must produce
// IDENTICAL output to the legacy `inferType(path)` for every known
// path prefix. If this drifts, zbrain-base.yaml is out of sync with
// the ZBRAIN_BASE_PATH_PREFIXES table in markdown.ts.
//
// Extension test: a user pack adding `paper: { path_prefixes:
// ['papers/'] }` must route `papers/foo.md` to 'paper' (the pack
// declaration), bypassing the zbrain-base fallback.
...
// Representative paths covering every zbrain-base path-prefix entry.
...
describe('inferTypeFromPack (T7a) — zbrain-base parity', () => {
  ...
  test('user pack extends zbrain-base with researcher type', () => {
  ...
  test('pack with empty page_types falls back to zbrain-base defaults', () => {
    // E.g. a pack mid-construction with no page_types declared — should
    // not crash, should match zbrain-base behavior.
