 ...
 *   ZBRAIN_HOME surfaces a PID-safe hint.
 *
...
describe('checkSourceRoutingHealth (#1167)', () => {
  ...
  test('non-default source with zero pages → warn (the #1167 fingerprint)', async () => {
    ...
    expect(r.message).toMatch(/zbrain sources current/);
  });
});
...
describe('checkOauthConfidentialHealth (#1166)', () => {
  ...
  test('public client (auth_method=none, hash=NULL) → ok (v0.34.1.0 shape preserved)', async () => {
    await engine.executeRaw(
      ...
      [['https://e.tests/unit/cb'], ['authorization_code']],
    );
    const r = await checkOauthConfidentialHealth(engine);
    expect(r.status).toBe('ok');
  });
...
describe('checkAutopilotLockScope (#1226)', () => {
  test('no lockfile → ok', async () => {
    ...
    await withEnv({ ZBRAIN_HOME: sandbox, HOME: sandbox }, async () => {
  ...
  test('stale lock outside ZBRAIN_HOME → warn with PID-safe hint', async () => {
    const home = mkdtempSync(join(tmpdir(), 'doctor-lock-home-'));
    const zbrainHome = mkdtempSync(join(tmpdir(), 'doctor-lock-zbrain-'));
    mkdirSync(join(home, '.zbrain'), { recursive: true });
    writeFileSync(join(home, '.zbrain', 'autopilot.lock'), '99999');

    await withEnv({ HOME: home, ZBRAIN_HOME: zbrainHome }, async () => {
      const r = checkAutopilotLockScope();
      expect(r.status).toBe('warn');
    ...
    rmSync(zbrainHome, { recursive: true, force: true });
  });
});
