 ...
 * semantics (e.g. someone hardcoding 'zbrain-sync' elsewhere would no longer match
...
describe('syncLockId (per-source lock helper)', () => {
  test('default source returns zbrain-sync:default', () => {
    expect(syncLockId('default')).toBe('zbrain-sync:default');
  });

  test('non-default source returns zbrain-sync:<id>', () => {
    expect(syncLockId('zion-brain')).toBe('zbrain-sync:zion-brain');
    expect(syncLockId('media-corpus')).toBe('zbrain-sync:media-corpus');
  });

  test('IRON-RULE: SYNC_LOCK_ID back-compat alias resolves to syncLockId(default)', () => {
    expect(SYNC_LOCK_ID).toBe(syncLockId('default'));
    expect(SYNC_LOCK_ID).toBe('zbrain-sync:default');
  });
