/**
 * v0.36.1.x #1100: PGLite + `zbrain apply-migrations` chain spawn test.
 *
 * Spawns `zbrain init --migrate-only` followed by `zbrain apply-migrations
 ...
 *
 * Pre-fix, this exact sequence hit `ZBrain: Timed out waiting for PGLite
 * lock` because:
 *   1. apply-migrations pre-flight schema-version probe held the
 *      single-writer lock briefly and raced the v0.11.0 subprocess.
 *   2. v0.11.0 phase A spawned `zbrain init --migrate-only` as a child;
...
/**
 * Make a shim `zbrain` binary that routes to `bun run <repo>/src/cli.ts`.
 *
 * The v0.11.0 orchestrator chain spawns subprocesses via `execSync('zbrain
 * jobs smoke')` and `execSync('zbrain init --migrate-only')` (the Postgres
 * path; PGLite now routes in-process, but phase B's smoke still shells out).
 * On a developer machine `zbrain` resolves via `bun link`; on CI it
...
function makeGbrainShim(): { binDir: string; cleanup: () => void } {
  const binDir = mkdtempSync(join(tmpdir(), 'zbrain-shim-'));
  const shimPath = join(binDir, 'zbrain');
  writeFileSync(
    shimPath,
...
describe('apply-migrations on fresh PGLite (v0.36.1.x #1100)', () => {
  ...
  test('init --migrate-only → apply-migrations --yes → re-run → --list (all exit 0)', async () => {
    const home = mkdtempSync(join(tmpdir(), 'zbrain-pglite-spawn-'));
    const shim = makeGbrainShim();
    try {
      mkdirSync(join(home, '.zbrain'), { recursive: true });
      writeFileSync(
        join(home, '.zbrain', 'config.json'),
        JSON.stringify({
          engine: 'pglite',
          database_path: join(home, '.zbrain', 'brain.pglite'),
          embedding_dimensions: 1536,
        }) + '\n',
      );
      // PATH shim so orchestrator phase-B execSync('zbrain jobs smoke')
      // and similar resolve to our shim instead of requiring a global
      // install. This matches the contract users hit in production
      // (zbrain on PATH) without depending on `bun link` having run.
      const env = {
        HOME: home,
        ZBRAIN_HOME: home,
      ...
      expect(existsSync(join(home, '.zbrain', 'brain.pglite'))).toBe(true);
