 ...
 *   - Result: `zbrain sync --source X --full` updated `sources.last_sync_at` to look
 *     like binding worked, but actual page rows landed in source_id='default'.
 *   - The 19 tests at tests/unit/source-id-tx-regression.test.ts validate the engine-layer
 *     transaction surface (putPage / addTag / etc.) but do NOT exercise performFullSync.
 *     Confirmed via: grep -c 'performFullSync' tests/unit/source-id-tx-regression.test.ts → 0.
 *
 * Fix (this PR-E follow-up to PR #707):
 *   - runImport accepts opts.sourceId (programmatic-only — no CLI flag, preserves
 *     PR #707's design intent of `zbrain import` being default-only).
 ...
 * `~/atlas/agents/terminal/docs/atlas-needs-from-zbrain-spec-v2.1-2026-05-08.md`
