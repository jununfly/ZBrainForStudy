...
//   - reindex --multimodal cost estimate + dry-run + ZBRAIN_NO_REEMBED bypass
...
beforeEach(async () => {
  ...
  configureGateway({
    ...
    env: { OPENAI_API_KEY: 'tests/unit', VOYAGE_API_KEY: 'tests/unit' },
  });
});
...
describe('reindex --multimodal command (Phase 3)', () => {
  ...
  test('ZBRAIN_NO_REEMBED=1 honored on zero-pending brain (skip path is no-op-clean)', async () => {
    await withEnv({ ZBRAIN_NO_REEMBED: '1' }, async () => {
