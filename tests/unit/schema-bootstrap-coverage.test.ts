 ...
 *
 * Background: zbrain ships an "embedded latest schema" blob
 ...
 * (per-engine integration tests in `tests/unit/bootstrap.test.ts` +
 * `tests/unit/e2e/postgres-bootstrap.test.ts`) to catch step 2 oversights.
 */
...
const COLUMN_EXEMPTIONS = new Set<string>([
  ...
  // zbrain think trajectory injection — all defensively skip
  ...
  // Schema-pack codegen (scripts/generate-zbrain-base.ts) consumes the value
...
test('every ALTER TABLE ADD COLUMN in MIGRATIONS is covered by applyForwardReferenceBootstrap (column-only class)', async () => {
  ...
  if (uncovered.length > 0) {
    ...
    throw new Error(
      ...
      `to COLUMN_EXEMPTIONS in tests/unit/schema-bootstrap-coverage.test.ts with a ` +
