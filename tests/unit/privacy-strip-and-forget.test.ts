 ...
 *     the DB-only expire path so forgets survive zbrain rebuild (Codex R2-#3)
 *
...
// tests/unit/e2e/system-of-record-invariant.test.ts (commit 10).
...
describe('forgetFactInFence — fence path (happy)', () => {
  ...
  test('re-parsing the rewritten fence sees forgotten=true + active=false', async () => {
    ...
    await forgetFactInFence(engine, id, { reason: 'tests/unit' });
