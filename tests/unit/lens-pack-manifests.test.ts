...
//   - zbrain-everything unions all three lens packs' contributions
...
describe('v0.41 T4: all 4 bundled lens packs parse cleanly', () => {
  for (const name of PACK_NAMES) {
    test(`${name}.yaml parses via parseSchemaPackManifest without error`, () => {
      ...
      expect(pack.api_version).toBe('zbrain-schema-pack-v1');
    });
  }
});
...
describe('v0.41 T4: zbrain-creator manifest shape', () => {
  const pack = loadPack('zbrain-creator');

  test('extends zbrain-base', () => {
    expect(pack.extends).toBe('zbrain-base');
  });
...
describe('v0.41 T4: zbrain-investor manifest shape', () => {
  const pack = loadPack('zbrain-investor');

  test('extends zbrain-base + borrows deal/person/company/yc', () => {
    expect(pack.extends).toBe('zbrain-base');
    const borrowEntry = pack.borrow_from.find((b) => b.pack === 'zbrain-base');
...
describe('v0.41 T4: zbrain-engineer manifest shape', () => {
  const pack = loadPack('zbrain-engineer');

  test('extends zbrain-base + borrows code/project', () => {
    expect(pack.extends).toBe('zbrain-base');
    const borrowEntry = pack.borrow_from.find((b) => b.pack === 'zbrain-base');
...
describe('v0.41 T4: zbrain-everything meta-pack shape', () => {
  const pack = loadPack('zbrain-everything');

  test('extends zbrain-investor (chain head)', () => {
    expect(pack.extends).toBe('zbrain-investor');
  });

  test('borrows from zbrain-creator + zbrain-engineer', () => {
    const borrowedPacks = pack.borrow_from.map((b) => b.pack).sort();
    expect(borrowedPacks).toEqual(['zbrain-creator', 'zbrain-engineer']);
  });

  test('borrows atom from creator and learning from engineer', () => {
    const creatorBorrow = pack.borrow_from.find((b) => b.pack === 'zbrain-creator');
    expect(creatorBorrow?.types).toContain('atom');
    const engineerBorrow = pack.borrow_from.find((b) => b.pack === 'zbrain-engineer');
