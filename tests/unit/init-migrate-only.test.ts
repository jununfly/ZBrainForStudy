/**
 * Tests for `zbrain init --migrate-only` — the schema-only primitive used by
 ...
 * missing config must fail loudly with a clear "run zbrain init first" error.
 *
...
function run(args: string[]): { exitCode: number; stdout: string; stderr: string } {
  // Strip DATABASE_URL / ZBRAIN_DATABASE_URL from the subprocess env. The
...
beforeEach(() => {
  ...
  tmp = mkdtempSync(join(tmpdir(), 'zbrain-init-migrate-only-test-'));
});
...
describe('zbrain init --migrate-only — error paths', () => {
  test('errors with clear message when no config exists', () => {
    ...
    expect(existsSync(join(tmp, '.zbrain', 'config.json'))).toBe(false);
  });
...
describe('zbrain init --migrate-only — happy path with PGLite config', () => {
