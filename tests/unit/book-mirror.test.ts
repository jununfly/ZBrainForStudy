 ...
 * the opt-in smoke test (tests/unit/e2e/skill-smoke-openclaw.test.ts when
...
async function runCli(args: string[]): Promise<{
  ...
  // v0.34.1 (#876): set ZBRAIN_HOME to a fresh tempdir so the test isn't
  // sensitive to the developer's local ~/.zbrain/config.json. Pre-fix this
  ...
  // depending on the local DB state. With ZBRAIN_HOME pointing at an
  ...
  const tmpHome = await import('os').then(os =>
    import('fs').then(fs => fs.mkdtempSync(`${os.tmpdir()}/zbrain-test-`)),
  );
  try {
    const proc = Bun.spawn(
      ['bun', 'run', 'src/cli.ts', 'book-mirror', ...args],
      {
        ...
        stdout: 'pipe',
        stderr: 'pipe',
        env: { ...process.env, DATABASE_URL: '', ZBRAIN_HOME: tmpHome },
      },
    );
...
describe('zbrain book-mirror — CLI registration', () => {
...
describe('zbrain book-mirror — source file invariants', () => {
