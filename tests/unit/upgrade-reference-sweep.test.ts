 ...
 * after `zbrain upgrade` so an operator/agent doesn't have to manually
 * run `zbrain skillpack reference --all`.
 *
 * Pins:
 *   - ZBRAIN_SKIP_REFERENCE_SWEEP=1 short-circuits silently
 *   - no detected workspace → silent no-op
 *   - workspace == zbrain repo (dev mode) → silent no-op
 ...
 * spawning a full `zbrain upgrade` subprocess. We swap the cwd via
...
const ZBRAIN_ROOT = process.cwd(); // tests run from zbrain repo root

describe('postUpgradeReferenceSweep', () => {
  it('ZBRAIN_SKIP_REFERENCE_SWEEP=1 short-circuits silently', async () => {
    await withEnv({ ZBRAIN_SKIP_REFERENCE_SWEEP: '1' }, async () => {
      const ws = scratchHostWithSkill('book-mirror', { drift: true });
      await postUpgradeReferenceSweep({ zbrainRoot: ZBRAIN_ROOT, targetWorkspace: ws });
      expect(logs.join('\n')).toBe('');
    });
  });

  it('zero drift (skills present but identical) → silent', async () => {
    await withEnv({ ZBRAIN_SKIP_REFERENCE_SWEEP: undefined }, async () => {
  ...
  it('drift detected → prints header + per-skill summary + footer hints', async () => {
    await withEnv({ ZBRAIN_SKIP_REFERENCE_SWEEP: undefined }, async () => {
      ...
      expect(out).toContain('zbrain skillpack reference <slug>');
      expect(out).toContain('_AGENT_README.md');
      expect(out).toContain('ZBRAIN_SKIP_REFERENCE_SWEEP');
    });
  });

  it('dev-mode guard: workspace IS zbrain → silent', async () => {
    await withEnv({ ZBRAIN_SKIP_REFERENCE_SWEEP: undefined }, async () => {
      // Pass zbrain repo as both zbrainRoot AND targetWorkspace.
      await postUpgradeReferenceSweep({
        zbrainRoot: ZBRAIN_ROOT,
        targetWorkspace: ZBRAIN_ROOT,
      });
      expect(logs.join('\n')).not.toContain('Skillpack reference sweep');
    });
  });
  ...
  it('errors swallowed silently — never blocks post-upgrade', async () => {
    ...
    await withEnv({ ZBRAIN_SKIP_REFERENCE_SWEEP: undefined }, async () => {
      await postUpgradeReferenceSweep({
        zbrainRoot: '/dev/null/no-bundle-here',
