 ...
 *
 * When `~/.zbrain/config.json` has `remote_mcp` set, 9 commands are refused
 ...
 *
 * Strategy: seed `~/.zbrain/config.json` with remote_mcp set in a tempdir
 * `ZBRAIN_HOME`, then spawn `zbrain <cmd>` and assert (a) exit code 1,
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'zbrain-cli-dispatch-'));
});
...
describe('thin-client dispatch guard refuses DB-bound commands', () => {
  ...
  for (const args of refusedCommands) {
    test(`refuses \`zbrain ${args.join(' ')}\` with pinpoint hint`, async () => {
      ...
      // host" message. Hint format: "`zbrain <cmd>` is not routable. <hint>"
      expect(r.stderr).toContain(`zbrain ${args[0]}`);
...
describe('thin-client dispatch guard does NOT refuse safe commands', () => {
  ...
  // does NOT appear.
  test('`zbrain --version` works on thin-client install', async () => {
    seedThinClientConfig();
    const r = await run(['--version']);
    expect(r.exitCode).toBe(0);
    expect(r.stdout).toContain('zbrain');
    expect(r.stderr).not.toContain('thin client');
  });

  test('`zbrain --help` works on thin-client install', async () => {
    seedThinClientConfig();
...
describe('thin-client doctor routes to runRemoteDoctor', () => {
  test('`zbrain doctor` runs remote checks (not DB-bound checks) when remote_mcp is set', async () => {
    seedThinClientConfig();
...
describe('regression — local config still passes through normally', () => {
  test('local PGLite config does NOT trigger thin-client guard for `sync`', async () => {
    // Seed a local PGLite config (no remote_mcp). `zbrain sync` shouldn't
