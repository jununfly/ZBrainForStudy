...
function scratchZbrain(): string {
  const root = mkdtempSync(join(tmpdir(), 'sp-mf-zbrain-'));
  created.push(root);
...
const FENCE_WITH_RECEIPT = `<!-- zbrain:skillpack:begin -->

<!-- Installed by zbrain 0.32.0 — do not hand-edit between markers. -->
<!-- zbrain:skillpack:manifest cumulative-slugs="alpha,beta" version="0.32.0" -->
...
describe('resolveFenceSlugs', () => {
  ...
  it('F-CDX-8: falls back to row parsing when receipt is missing', () => {
    const fence = `<!-- zbrain:skillpack:begin -->
...
<!-- zbrain:skillpack:end -->`;
  ...
  it('F-CDX-8: uses union when receipt and rows drift', () => {
    const fence = `<!-- zbrain:skillpack:begin -->
<!-- zbrain:skillpack:manifest cumulative-slugs="alpha,old-removed" version="0.20.0" -->
...
describe('stripFence', () => {
  it('removes begin/end markers and the receipt comment', () => {
    ...
    expect(out).not.toContain('zbrain:skillpack:begin');
    expect(out).not.toContain('zbrain:skillpack:end');
    expect(out).not.toContain('cumulative-slugs');
    expect(out).not.toContain('Installed by zbrain');
  });
...
describe('runMigrateFence', () => {
  ...
  it('copies missing skill dirs additively when zbrainRoot is set', () => {
    const ws = scratchWorkspace();
    ...
    const result = runMigrateFence({ targetWorkspace: ws, zbrainRoot });
  ...
  it('preserves user-added rows whose slug is not in the bundle (no copy attempt)', () => {
    const ws = scratchWorkspace();
    const zbrainRoot = scratchZbrain();
    const fenceWithUserRow = `<!-- zbrain:skillpack:begin -->
<!-- zbrain:skillpack:manifest cumulative-slugs="alpha,user-extra" version="0.20.0" -->
| "alpha trigger" | \`skills/alpha/SKILL.md\` |
