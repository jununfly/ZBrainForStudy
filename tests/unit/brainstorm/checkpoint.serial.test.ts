...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'zbrain-bs-cp-'));
  homeBackup = process.env.ZBRAIN_HOME;
  process.env.ZBRAIN_HOME = tmp;
});

afterEach(() => {
  if (homeBackup === undefined) delete process.env.ZBRAIN_HOME;
  else process.env.ZBRAIN_HOME = homeBackup;
...
describe('save + load round-trip (TX3 load-bearing — full ideas preserved)', () => {
  ...
  test('atomic write: no .tmp left behind on success', () => {
    ...
    saveCheckpoint(cp);
    const dir = join(tmp, '.zbrain', 'brainstorm');
  ...
  test('loadCheckpoint returns null + stderr WARN on schema mismatch', () => {
    ...
    const cp = fixtureCheckpoint(runId);
    saveCheckpoint(cp);
    const path = join(tmp, '.zbrain', 'brainstorm', `${runId}.json`);
  ...
  test('loadCheckpoint returns null on corrupt JSON', () => {
    ...
    writeFileSync(join(tmp, '.zbrain', 'brainstorm', `${runId}.json`), '{not json}');
    expect(loadCheckpoint(runId)).toBeNull();
  });
});
...
describe('gcStaleCheckpoints (A5 7-day window)', () => {
  test('removes files older than the threshold; returns count', () => {
    ...
    const stalePath = join(tmp, '.zbrain', 'brainstorm', `${stale}.json`);
    ...
    expect(existsSync(join(tmp, '.zbrain', 'brainstorm', `${fresh}.json`))).toBe(true);
  });
...
describe('clearCheckpoint', () => {
  test('removes file when present', () => {
    ...
    const path = join(tmp, '.zbrain', 'brainstorm', `cleartest0000000.json`);
...
describe('isCheckpointFresh', () => {
  ...
  test('false for >7 day old checkpoint', () => {
    ...
    const path = join(tmp, '.zbrain', 'brainstorm', 'oldtest000000000.json');
