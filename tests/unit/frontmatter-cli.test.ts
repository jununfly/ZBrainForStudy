...
describe('zbrain frontmatter CLI (B4)', () => {
  let tmp: string;
  ...
  test('validate --fix writes centralized backup and rewrites in place', () => {
    const f = join(tmp, 'broken.md');
    const zbrainHome = join(tmp, 'home');
    const original = `${fence}\ntype: concept\ntitle: "P "I" L"\n${fence}\n\nbody`;
    writeFileSync(f, original);
    const { stdout, code } = runCli(['validate', f, '--fix'], { ZBRAIN_HOME: zbrainHome });
    expect(code).toBe(0);
    expect(existsSync(f + '.bak')).toBe(false);
    expect(stdout).toContain('centralized backups');
    const backupDir = join(zbrainHome, '.zbrain', 'backups', 'frontmatter');
  ...
  test('validate --fix succeeds on a non-git path (no dirty-tree guard)', () => {
    ...
    const zbrainHome = join(tmp, 'home');
    writeFileSync(f, `${fence}\ntype: concept\ntitle: "A "B" C"\n${fence}\n\nbody`);
    const { code } = runCli(['validate', f, '--fix'], { ZBRAIN_HOME: zbrainHome });
    expect(code).toBe(0);
    expect(existsSync(f + '.bak')).toBe(false);
    expect(existsSync(join(zbrainHome, '.zbrain', 'backups', 'frontmatter'))).toBe(true);
  });
  ...
  test('generate --fix skips catch-all note writes unless explicitly included', () => {
    ...
    const skipped = runCli(['generate', tmp, '--fix', '--json'], { ZBRAIN_HOME: zbrainHome });
    ...
    const included = runCli(['generate', tmp, '--fix', '--json', '--include-catch-all'], { ZBRAIN_HOME: zbrainHome });
