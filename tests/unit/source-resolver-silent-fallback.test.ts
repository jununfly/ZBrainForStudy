...
beforeEach(async () => {
  ...
  cwd = mkdtempSync(join(tmpdir(), 'zbrain-resolver-'));
});

describe('source-resolver silent-fallback tiers (codex P1-F)', () => {
  describe('tier 3 — .zbrain-source dotfile', () => {
    test('valid dotfile content (registered source) is honored', async () => {
      ...
      writeFileSync(join(cwd, '.zbrain-source'), 'alpha\n');
    ...
    test('underscore in dotfile content silently falls through (strict regex rejects)', async () => {
      ...
      writeFileSync(join(cwd, '.zbrain-source'), 'has_underscore\n');
    ...
    test('whitespace-only dotfile content silently falls through', async () => {
      writeFileSync(join(cwd, '.zbrain-source'), '   \n');
      const resolved = await resolveSourceId(engine, null, cwd);
      expect(resolved).toBe('default');
    });

    test('uppercase in dotfile content silently falls through', async () => {
      writeFileSync(join(cwd, '.zbrain-source'), 'DEFAULT\n');
      const resolved = await resolveSourceId(engine, null, cwd);
      expect(resolved).toBe('default');
    });
  });
  ...
  describe('tier 2 — ZBRAIN_SOURCE env (throw-on-invalid contract)', () => {
    test('valid env value is honored', async () => {
      await engine.executeRaw(
        `INSERT INTO sources (id, name, config) VALUES ('delta', 'delta', '{}'::jsonb)
         ON CONFLICT (id) DO NOTHING`,
      );
      await withEnv({ ZBRAIN_SOURCE: 'delta' }, async () => {
        const resolved = await resolveSourceId(engine, null, cwd);
        expect(resolved).toBe('delta');
      });
    });

    test('underscore in ZBRAIN_SOURCE THROWS (tier-2 contract)', async () => {
      await withEnv({ ZBRAIN_SOURCE: 'has_underscore' }, async () => {
        await expect(resolveSourceId(engine, null, cwd)).rejects.toThrow(/ZBRAIN_SOURCE/);
      });
    });
  });
});
