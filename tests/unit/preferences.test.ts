...
beforeEach(() => {
  ...
  tmp = mkdtempSync(join(tmpdir(), 'zbrain-prefs-test-'));
  // preferences.ts's gbrainDir() returns `$HOME/.zbrain` when ZBRAIN_HOME
  // is unset. Test fixtures write to `$tmp/.zbrain/...`, so set HOME only
  // and clear ZBRAIN_HOME — setting ZBRAIN_HOME would route prefs to $tmp
  // directly (no .zbrain suffix), which doesn't match the fixture layout.
...
describe('loadPreferences', () => {
  ...
  test('parses existing JSON file', () => {
    ...
    writeFileSync(
      join(tmp, '.zbrain', 'preferences.json'),
  ...
  test('throws on malformed JSON so callers can surface it', () => {
    mkdirSync(join(tmp, '.zbrain'), { recursive: true });
    writeFileSync(join(tmp, '.zbrain', 'preferences.json'), '{not json');
...
describe('savePreferences', () => {
  ...
  test('creates ~/.zbrain directory if missing', () => {
    // Confirm .zbrain doesn't exist yet
    expect(existsSync(join(tmp, '.zbrain'))).toBe(false);
    savePreferences({ minion_mode: 'off' });
    expect(existsSync(join(tmp, '.zbrain'))).toBe(true);
  });
  ...
  test('cleans up temp directory used for atomic write', () => {
    savePreferences({ minion_mode: 'off' });
    const gbrainDir = join(tmp, '.zbrain');
...
describe('loadCompletedMigrations', () => {
  ...
  test('tolerates malformed lines with a warning, continuing past them', () => {
    const dir = join(tmp, '.zbrain', 'migrations');
