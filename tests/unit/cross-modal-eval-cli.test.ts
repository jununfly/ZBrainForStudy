...
function makeTempDir(): string {
  return mkdtempSync(join(tmpdir(), 'zbrain-cme-cli-'));
}
...
describe('cross-modal-eval CLI helpers', () => {
  ...
  test('ZBRAIN_HOME isolates receipts under <home>/.zbrain/eval-receipts', async () => {
    const dir = makeTempDir();
    try {
      await withEnv({ ZBRAIN_HOME: dir }, async () => {
        const { zbrainPath } = await import('../../src/core/config.ts');
        const path = zbrainPath('eval-receipts');
        expect(path.startsWith(dir)).toBe(true);
        expect(path.endsWith('eval-receipts')).toBe(true);
        expect(path.includes('.zbrain')).toBe(true);
      });
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
