 ...
 *   - --yes override skips
 *   - ZBRAIN_NO_PROBE_PROMPT=1 env var skips
...
describe('maybePromptForCostBeforeProbe', () => {
  ...
  test('ZBRAIN_NO_PROBE_PROMPT=1 skips entirely', async () => {
    await withEnv({ ZBRAIN_NO_PROBE_PROMPT: '1' }, async () => {
  ...
  test('ZBRAIN_PROBE_PROMPT_GRACE_SECONDS env overrides the 10s default', async () => {
    await withEnv({ ZBRAIN_PROBE_PROMPT_GRACE_SECONDS: '0' }, async () => {
      let waitedSeconds = -1;
      const report: ProbeReport = mkSeedReport('1');
      await writeRunRow(engine, report, 100);
