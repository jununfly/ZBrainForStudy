 ...
 *
 * The kill-resume contract is covered by tests/unit/embed-stale.test.ts at the
...
describe('embed-backfill handler — D2 lock contract', () => {
  test('IRON-RULE: second call returns already_in_progress when lock is held', async () => {
    ...
    const lock = await tryAcquireDbLock(engine, 'zbrain-embed-backfill:default', 60);
  ...
  test('different sources do not contend on each other locks', async () => {
    ...
    const lockA = await tryAcquireDbLock(engine, 'zbrain-embed-backfill:default', 60);
