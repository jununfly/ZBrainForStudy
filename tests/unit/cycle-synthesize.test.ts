 ...
 * cycle E2E lives in tests/unit/e2e/.
 */
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'zbrain-synth-test-'));
});
...
describe('discoverTranscripts', () => {
  ...
  test('multiple sources (corpus + meeting transcripts) merged', () => {
    ...
    const meetDir = mkdtempSync(join(tmpdir(), 'zbrain-meet-'));
  ...
  function mockClient(captured: { model?: string }): JudgeClient {
    return {
      create: async (p: any) => {
        ...
        return { content: [{ type: 'text', text: '{"worth_processing": true, "reasons": ["tests/unit"]}' }] } as any;
      },
    };
  }
