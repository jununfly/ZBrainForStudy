 ...
 * surface behind ZBRAIN_HTTP_CORS_ORIGIN with default-deny.
 *
 ...
 * preflight) is verified by tests/unit/e2e/serve-http-oauth.test.ts.
 */
...
describe('parseCorsAllowlistOAuth', () => {
  test('unset → null (default-deny posture)', async () => {
    await withEnv({ ZBRAIN_HTTP_CORS_ORIGIN: undefined }, async () => {
      expect(parseCorsAllowlistOAuth()).toBeNull();
    });
  });

  test('empty string → null', async () => {
    await withEnv({ ZBRAIN_HTTP_CORS_ORIGIN: '' }, async () => {
      expect(parseCorsAllowlistOAuth()).toBeNull();
    });
  });
  ...
  test('single origin → Set of one', async () => {
    await withEnv({ ZBRAIN_HTTP_CORS_ORIGIN: 'https://claude.ai' }, async () => {
  ...
  test('comma-separated origins → Set of N', async () => {
    await withEnv({ ZBRAIN_HTTP_CORS_ORIGIN: 'https://claude.ai,https://chatgpt.com,https://my.app' }, async () => {
      const set = parseCorsAllowlistOAuth();
  ...
  test('whitespace around values is trimmed', async () => {
    await withEnv({ ZBRAIN_HTTP_CORS_ORIGIN: ' https://a.app , https://b.app ' }, async () => {
      const set = parseCorsAllowlistOAuth();
