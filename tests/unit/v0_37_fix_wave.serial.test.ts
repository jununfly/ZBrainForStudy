...
describe('v0.37 Lane B — init paths', () => {
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-v37-test-'));
  ...
  test('B.4 / T-3: loadConfigFileOnly ignores env overrides', async () => {
    ...
    require('fs').mkdirSync(join(tmpHome, '.zbrain'), { recursive: true });
    ...
    process.env.ZBRAIN_EMBEDDING_MODEL = 'voyage:voyage-3-large';
    process.env.ZBRAIN_EMBEDDING_DIMENSIONS = '2048';
    ...
    delete process.env.ZBRAIN_EMBEDDING_MODEL;
    delete process.env.ZBRAIN_EMBEDDING_DIMENSIONS;
...
describe('v0.37 Lane C.3 — ZE key reaches buildGatewayConfig', () => {
  ...
  test('ZBrainConfig type includes zeroentropy_api_key field (TS compile guard)', async () => {
...
// Lane D.1 — engine-kind branching already covered in tests/unit/embedding-dim-check.test.ts
...
// Deferred-TODO ship: zbrain reinit-pglite
describe('v0.37 deferred TODO shipped — zbrain reinit-pglite', () => {
  ...
  test('embeddingMismatchMessage PGLite branch recommends `zbrain reinit-pglite`', async () => {
    ...
    expect(msg).toContain('zbrain reinit-pglite --embedding-model zeroentropyai:zembed-1 --embedding-dimensions 1280');
