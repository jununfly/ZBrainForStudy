/**
 * v0.40.4.0 — `zbrain search stats` graph_signals section.
 *
 * Pins:
 ...
 *
 * Hermetic via PGLite + withEnv for ZBRAIN_AUDIT_DIR.
 */
...
// withEnv-wrapped ZBRAIN_AUDIT_DIR is honored by the audit writer's
...
function mkTmp(): string {
  const d = fs.mkdtempSync(path.join(os.tmpdir(), 'zbrain-graph-stats-test-'));
  tmpDirs.push(d);
  return d;
}
