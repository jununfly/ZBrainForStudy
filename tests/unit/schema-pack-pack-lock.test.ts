...
beforeEach(() => {
  lockDir = mkdtempSync(join(tmpdir(), 'zbrain-pack-lock-test-'));
});
...
describe('acquirePackLock — contention', () => {
  ...
  it('steals lock when holder PID is dead', () => {
    ...
    writeFileSync(lockPath, JSON.stringify({
      pid: 99999, hostname: 'tests/unit', ts: Date.now(), ttlMs: 60_000,
    }), 'utf-8');
  ...
  it('steals lock when TTL is expired even if PID is alive', () => {
    const lockPath = join(lockDir, 'foo.lock');
    writeFileSync(lockPath, JSON.stringify({
      pid: 99999, hostname: 'tests/unit', ts: Date.now() - 120_000, ttlMs: 60_000,
    }), 'utf-8');
  ...
  it('PackLockBusyError carries heldBy + ageMs + ttlMs', () => {
    ...
    writeFileSync(lockPath, JSON.stringify({
      pid: 88888, hostname: 'tests/unit', ts: past, ttlMs: 60_000,
    }), 'utf-8');
