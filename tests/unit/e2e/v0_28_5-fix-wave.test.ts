 ...
 *      apply with no crash.
 *
 *   2. Embedding-dim corruption — `zbrain init --embedding-dimensions 768`
 ...
 * bootstrap is covered by `tests/unit/e2e/postgres-bootstrap.test.ts`; this file
 * is the PGLite-side equivalent specifically for the wedge classes the
 * v0.28.5 wave fixed.
 *
 * Run: bun test tests/unit/e2e/v0_28_5-fix-wave.test.ts
 */
...
describe('v0.28.5 A4 — existing-brain dim mismatch loud failure', () => {
  test('readContentChunksEmbeddingDim correctly identifies a brain at 1536 + mismatch message inlines all four steps', async () => {
    ...
    try {
      ...
      // correct dim and model and points at `zbrain init --pglite`.
      expect(msg).toContain('vector(1536)');
      expect(msg).toContain('vector(768)');
      expect(msg).toContain('zbrain init --pglite --embedding-model ollama:nomic-embed-text --embedding-dimensions 768');
      expect(msg).toContain('PGLite cannot ALTER vector column types');
      expect(msg).toContain('docs/embedding-migrations.md');
      expect(msg).toContain('zbrain embed --stale');
    } finally {
      await engine.disconnect();
    }
  }, 60000);
