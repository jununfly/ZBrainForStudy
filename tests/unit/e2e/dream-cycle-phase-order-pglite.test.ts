 ...
 *
 * Run: bun test tests/unit/e2e/dream-cycle-phase-order-pglite.test.ts
 */
...
async function setupRig(): Promise<TestRig> {
  ...
  const brainDir = mkdtempSync(join(tmpdir(), 'zbrain-cycle8-'));
...
describe('E2E full cycle phase order', () => {
  ...
  test('synthInputFile flag is plumbed through runCycle to runPhaseSynthesize', async () => {
    const rig = await setupRig();
    try {
      const transcript = join(tmpdir(), `zbrain-e2e-cycle8-input-${Date.now()}.txt`);
