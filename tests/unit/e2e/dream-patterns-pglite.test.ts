 ...
 * tests/unit/cycle-patterns.test.ts (asserts queue + waitForCompletion are
 * wired, allow-list reads from filing-rules JSON, slug provenance from
 * subagent_tool_executions, no raw_data dependency).
 *
 * Run: bun test tests/unit/e2e/dream-patterns-pglite.test.ts
 */
...
async function setupRig(): Promise<TestRig> {
  ...
  return {
    engine,
    brainDir: '/tmp/zbrain-patterns-test',
    cleanup: async () => {
