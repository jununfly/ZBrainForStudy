...
beforeAll(async () => {
  if (skip) return;
  tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-lock-recovery-e2e-'));
  await setupDB();
});
...
beforeEach(async () => {
  ...
  repoDir = mkdtempSync(join(tmpdir(), 'zbrain-lock-recovery-repo-'));
...
describeE2E('v0.41.6.0 — sync lock recovery scenarios', () => {
  ...
  test('lock-busy error message includes PID + hostname + age + --break-lock hint', async () => {
    ...
    const eng = getEngine();
    const lockKey = 'zbrain-sync:default';
  ...
  test('--break-lock with TTL-expired row clears the lock', async () => {
    ...
    const snap = await inspectLock(eng, 'zbrain-sync:default');
  ...
  // unit tests/unit/process-cleanup.test.ts. The SIGTERM-during-sync E2E above
  ...
  test.skip('pipe through `head -5` exits cleanly, next sync runs without lock-busy', async () => {
    // Run `zbrain sync ... | head -5` via shell.
