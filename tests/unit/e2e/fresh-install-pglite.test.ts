/**
 * E2E: fresh `zbrain init --pglite` produces a brain that can embed end-to-end.
 *
 ...
 *
 * Hermetic: in-process (NOT a CLI subprocess), ZBRAIN_HOME pinned to a
...
describe('E2E: fresh zbrain init --pglite → import → embed works end-to-end', () => {
  let tmpHome: string;
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-e2e-fresh-'));
  ...
  test('bare `init --pglite`: schema sized to gateway defaults (ZE/1280)', async () => {
    ...
    // fresh install: bare `zbrain init --pglite` with no env or file
    // config.
    resetGateway();
  ...
  test('init → seed page → embed: chunks have non-null embeddings, no dim mismatch', async () => {
    ...
    try {
      ...
      await engine.putPage('tests/unit/e2e-page', {
        type: 'note',
        title: 'E2E Test',
        compiled_truth: 'fresh install end-to-end happy path',
      });
      await engine.upsertChunks('tests/unit/e2e-page', [
