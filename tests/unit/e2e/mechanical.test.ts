 ...
 *
 * Run: DATABASE_URL=... bun test tests/unit/e2e/mechanical.test.ts
 */
...
function makeCtx(opts: { remote?: boolean } = {}): OperationContext {
  return {
    ...
    dryRun: false,
    // Default: trusted local invocation (matches `zbrain call` semantics).
...
// (covered in tests/unit/pglite-engine.test.ts). These tests verify the postgres-js code
...
describeE2E('E2E: Files', () => {
  ...
  test('file_upload stores metadata + file_list shows it', async () => {
    ...
    try {
      ...
      expect(url.url).toContain('zbrain:files/');
    } finally {
  ...
  test('file_upload rejects outside-cwd paths for remote (MCP) callers', async () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'zbrain-e2e-ssrf-'));
...
describeE2E('E2E: Setup Journey', () => {
  ...
  test('zbrain init --non-interactive connects and initializes', () => {
  ...
  test('zbrain import imports fixtures via CLI', () => {
    const result = Bun.spawnSync({
  ...
  test('zbrain search returns results via CLI', () => {
    const result = Bun.spawnSync({
  ...
  test('zbrain stats shows page count via CLI', () => {
    const result = Bun.spawnSync({
  ...
  test('zbrain health runs via CLI', () => {
    const result = Bun.spawnSync({
...
describeE2E('E2E: Init Edge Cases', () => {
  ...
  test('init --non-interactive without URL fails gracefully', () => {
    ...
    delete env.ZBRAIN_DATABASE_URL;
    const result = Bun.spawnSync({
...
describeE2E('E2E: RLS Verification', () => {
  ...
  const cliCwd = join(import.meta.dir, '../..');
  const cliEnv = () => ({ ...process.env, DATABASE_URL: process.env.DATABASE_URL!, ZBRAIN_DATABASE_URL: process.env.DATABASE_URL! });

  // Seed a unique suffix per run so concurrent test DBs / crashed prior
  // runs don't collide. All helper tables follow `zbrain_rls_regression_<suffix>`.
  ...
  test('zbrain doctor fails with exit 1 when a public table is missing RLS', async () => {
    const conn = getConn();
    const tbl = `zbrain_rls_regression_${suffix}`;
    try {
  ...
  test('ZBRAIN:RLS_EXEMPT comment with valid reason exempts a non-RLS public table', async () => {
    const conn = getConn();
    const tbl = `zbrain_rls_exempt_ok_${suffix}`;
    try {
      await conn.unsafe(`CREATE TABLE public.${tbl} (id int)`);
      await conn.unsafe(`ALTER TABLE public.${tbl} DISABLE ROW LEVEL SECURITY`);
      await conn.unsafe(`COMMENT ON TABLE public.${tbl} IS 'ZBRAIN:RLS_EXEMPT reason=e2e test fixture, anon-readable ok'`);
  ...
  test('ZBRAIN:RLS_EXEMPT comment WITHOUT reason= still fails doctor', async () => {
    const conn = getConn();
    const tbl = `zbrain_rls_exempt_bad_${suffix}`;
    try {
      await conn.unsafe(`CREATE TABLE public.${tbl} (id int)`);
      await conn.unsafe(`ALTER TABLE public.${tbl} DISABLE ROW LEVEL SECURITY`);
      // Missing the `reason=<...>` segment — prefix alone is not enough.
      await conn.unsafe(`COMMENT ON TABLE public.${tbl} IS 'ZBRAIN:RLS_EXEMPT'`);
  ...
  test('Non-exempt unrelated COMMENT on a no-RLS table still fails doctor', async () => {
    const conn = getConn();
    const tbl = `zbrain_rls_comment_${suffix}`;
    try {
      await conn.unsafe(`CREATE TABLE public.${tbl} (id int)`);
      await conn.unsafe(`ALTER TABLE public.${tbl} DISABLE ROW LEVEL SECURITY`);
  ...
  test('v24 self-heals when budget_ledger + budget_reservations are missing', async () => {
    ...
    try {
      ...
      // (cleanup, migration from an older zbrain, etc).
...
describeE2E('E2E: Doctor Command', () => {
  // Scope ZBRAIN_HOME to a hermetic tmpdir so `zbrain doctor` doesn't read
  // the developer's local ~/.zbrain/migrations/completed.jsonl. Stale partial
  ...
  let zbrainHome: string;

  beforeAll(async () => {
    await setupDB();
    await importFixtures();
    // Isolate ZBRAIN_HOME to a per-block tempdir so the developer's
    // ~/.zbrain/migrations/completed.jsonl ledger doesn't leak in. Without
    ...
    zbrainHome = mkdtempSync(join(tmpdir(), 'zbrain-doctor-e2e-'));
  ...
  afterAll(async () => {
    await teardownDB();
    if (zbrainHome) rmSync(zbrainHome, { recursive: true, force: true });
  });
  ...
  const cliEnv = () => ({
    ...
    ZBRAIN_DATABASE_URL: process.env.DATABASE_URL!,
    ZBRAIN_HOME: zbrainHome,
  });

  test('zbrain doctor exits 0 on healthy DB', () => {
  ...
  test('zbrain doctor --json produces valid JSON', () => {
    const result = Bun.spawnSync({
      cmd: ['bun', 'run', 'src/cli.ts', 'doctor', '--json'],
      cwd: cliCwd,
      env: cliEnv(),
