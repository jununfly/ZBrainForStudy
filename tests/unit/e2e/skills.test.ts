 ...
 *
 * Tests zbrain skills via OpenClaw agent CLI invocations.
 ...
 * Run: source ~/.zshrc && DATABASE_URL=... bun test tests/unit/e2e/skills.test.ts
 */
...
Action: Sarah to draft VP Sales job description by April 7.
    ...
    const { text, exitCode, durationMs } = runOpenClaw(
      `Ingest this meeting transcript into zbrain. Create or update pages for each person mentioned. Add timeline entries for today's date. Here is the transcript:\n\n${transcript}`,
      180_000,
    );

    console.log(`  Ingest skill completed in ${durationMs}ms`);

    // The agent runs against its own configured zbrain DB, not our test DB.
...
describeT2('E2E Tier 2: Query Skill', () => {
  ...
  test('query skill returns results for known topic', async () => {
    const { text, exitCode, durationMs } = runOpenClaw(
      'Search zbrain for "NovaMind" and tell me what you found.',
      180_000,
    );
...
describeT2('E2E Tier 2: Health Skill', () => {
  ...
  test('health skill reports brain status', async () => {
    const { text, exitCode, durationMs } = runOpenClaw(
      'Run zbrain doctor --json and tell me the results.',
      180_000,
    );
