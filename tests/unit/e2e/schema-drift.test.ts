 ...
 * via the pure helper in `tests/unit/helpers/schema-diff.ts`.
 *
 ...
 * `zbrain doctor --schema-audit` mechanism deferred to v0.26.4.
 *
 * Skips gracefully when DATABASE_URL is unset (matches the existing E2E
 * pattern in tests/unit/e2e/postgres-bootstrap.test.ts and tests/unit/e2e/postgres-jsonb.test.ts).
 *
 * Run: DATABASE_URL=postgresql://... bun test tests/unit/e2e/schema-drift.test.ts
...
// against fresh Postgres. If ZBRAIN_PGLITE_SNAPSHOT is set (ci:local sets it
...
describe.skipIf(skip)('schema drift: PGLite ↔ Postgres post-initSchema parity (E2E)', () => {
  ...
  beforeAll(async () => {
    ...
    // DATABASE_URL, but `zbrain doctor` (used by the CLAUDE.md E2E bootstrap
    ...
    // bypasses that floor. ZBRAIN_TEST_DB=1 only relaxes the localhost requirement
    ...
    // db-name pattern is the floor: zbrain_test, *_test, test_*, *_e2e.
    // Required REGARDLESS of any override — a production db named "production_data"
    // cannot be reset even with ZBRAIN_TEST_DB=1.
    const looksLikeTestDb = /^(zbrain_test|.*_test|test_.*|.*_e2e)$/i.test(dbName);
    const ciOptIn = process.env.ZBRAIN_TEST_DB === '1';
    ...
    if (resetAllowed) {
      ...
      const reason = !looksLikeTestDb
        ? `db name "${dbName}" doesn't match the test pattern (zbrain_test, *_test, test_*, *_e2e). ` +
          `ZBRAIN_TEST_DB=1 does NOT override this — db name is the hard floor.`
        : `host="${host}" is non-local AND ZBRAIN_TEST_DB=1 is not set. ` +
          `Set ZBRAIN_TEST_DB=1 to allow non-local hosts (e.g. CI service names) — ` +
