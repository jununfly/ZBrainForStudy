...
describe('cache gate end-to-end (PGLite)', () => {
  ...
  test('store → lookup HIT (baseline freshness)', async () => {
    const p1 = await seedPage('tests/unit/p1', 'alpha bravo');
  ...
  test('content UPDATE on a result page → lookup MISS (per-page bumped)', async () => {
    const p1 = await seedPage('tests/unit/p1', 'alpha bravo v1');
    const results: SearchResult[] = [
      { page_id: p1, slug: 'tests/unit/p1', title: 'tests/unit/p1', snippet: 'x', score: 1.0 } as unknown as SearchResult,
    ];
    const emb = fakeEmbedding(2);
    await cache.store('alpha bravo', emb, results, fakeMeta(), { sourceId: 'default' });

    // Update content_truth — trigger bumps p1.generation
    await engine.putPage('tests/unit/p1', {
      type: 'note',
      title: 'tests/unit/p1',
  ...
  test('INSERT new page → lookup HIT (bookmark fires but snapshot intact — codex #4 case)', async () => {
    const p1 = await seedPage('tests/unit/p1', 'alpha');
    const results: SearchResult[] = [
      { page_id: p1, slug: 'tests/unit/p1', title: 'tests/unit/p1', snippet: 'a', score: 1.0 } as unknown as SearchResult,
    ];
    ...
    await seedPage('tests/unit/p2', 'beta gamma');
  ...
  test('legacy row (pre-v0.40.3.0 shape) serves normally — IRON-RULE backward compat', async () => {
    const p1 = await seedPage('tests/unit/p1', 'gamma delta');
    const emb = fakeEmbedding(4);
    const results: SearchResult[] = [
      { page_id: p1, slug: 'tests/unit/p1', title: 'tests/unit/p1', snippet: 'g', score: 1.0 } as unknown as SearchResult,
    ];
    ...
    await seedPage('tests/unit/p2', 'unrelated bump');
    await seedPage('tests/unit/p3', 'another unrelated bump');
  ...
  test('soft-delete result page → lookup MISS (trigger bumps generation)', async () => {
    const p1 = await seedPage('tests/unit/p1', 'epsilon');
    const results: SearchResult[] = [
      { page_id: p1, slug: 'tests/unit/p1', title: 'tests/unit/p1', snippet: 'e', score: 1.0 } as unknown as SearchResult,
    ];
    ...
    // pages) is admin-only via `zbrain pages purge-deleted` and is best-
  ...
  test('multi-page result: partial bump invalidates (one of two result pages changed)', async () => {
    const p1 = await seedPage('tests/unit/p1', 'zeta');
    const p2 = await seedPage('tests/unit/p2', 'eta');
    const results: SearchResult[] = [
      { page_id: p1, slug: 'tests/unit/p1', title: 'tests/unit/p1', snippet: 'z', score: 1.0 } as unknown as SearchResult,
      { page_id: p2, slug: 'tests/unit/p2', title: 'tests/unit/p2', snippet: 'e', score: 0.9 } as unknown as SearchResult,
    ];
    const emb = fakeEmbedding(6);
    await cache.store('zeta eta', emb, results, fakeMeta(), { sourceId: 'default' });

    // Bump only p2.
    await engine.putPage('tests/unit/p2', {
      type: 'note',
      title: 'tests/unit/p2',
