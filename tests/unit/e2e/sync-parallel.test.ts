 ...
 *   docker run -d --name zbrain-test-pg -e POSTGRES_USER=postgres \
 *     -e POSTGRES_PASSWORD=postgres -e POSTGRES_DB=zbrain_test \
 *     -p 5435:5432 pgvector/pgvector:pg16
 *   DATABASE_URL=postgresql://postgres:postgres@localhost:5435/zbrain_test \
 *     bun test tests/unit/e2e/sync-parallel.test.ts
 */
...
describeE2E('E2E sync-parallel: T2 happy path + leak probe', () => {
  ...
  test('60-file Postgres sync at concurrency=4 imports all + no connection leak', async () => {
    repoPath = mkdtempSync(join(tmpdir(), 'zbrain-e2e-par-'));
...
describeE2E('E2E sync-parallel: P4 benchmark serial vs concurrency=4', () => {
  ...
  test('120-file benchmark: report serial and parallel wall-clock', async () => {
    ...
    repoSerial = mkdtempSync(join(tmpdir(), 'zbrain-bench-serial-'));
    repoParallel = mkdtempSync(join(tmpdir(), 'zbrain-bench-parallel-'));
