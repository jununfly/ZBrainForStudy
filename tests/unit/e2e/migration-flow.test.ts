 ...
 * `zbrain init --migrate-only`) through Phase G (completed.jsonl append),
 ...
 * handler are covered by the unit-layer tests (tests/unit/handlers.test.ts and
 * tests/unit/autopilot-resolve-cli.test.ts); this E2E locks the schema → prefs
...
// Module-level PATH shim. The orchestrator shells out to `zbrain init
// --migrate-only` and `zbrain jobs smoke`. On source-install test envs
// there's no `zbrain` on $PATH. Install a tiny bash shim that `exec`s
...
if (!SKIP) {
  ...
  fakeBinDir = mkdtempSync(join(tmpdir(), 'zbrain-e2e-bin-'));
  const shim = join(fakeBinDir, 'zbrain');
  writeFileSync(
    shim,
...
function freshTempHome(label: string) {
  const dir = mkdtempSync(join(tmpdir(), `zbrain-e2e-migration-${label}-`));
  // preferences.ts's gbrainDir() returns `$HOME/.zbrain` when ZBRAIN_HOME
  // is unset. Test fixtures write to `$dir/.zbrain/...`, so set HOME only
  // and clear any inherited ZBRAIN_HOME (which would route prefs to $dir
  // directly, no .zbrain suffix).
  process.env.HOME = dir;
  delete process.env.ZBRAIN_HOME;
  // Seed config so Phase A's `zbrain init --migrate-only` has a target.
  mkdirSync(join(dir, '.zbrain'), { recursive: true });
  writeFileSync(
    join(dir, '.zbrain', 'config.json'),
...
beforeAll(() => {
  ...
  origGbrainHome = process.env.ZBRAIN_HOME;
});
...
afterAll(() => {
  ...
  if (origGbrainHome === undefined) delete process.env.ZBRAIN_HOME;
  else process.env.ZBRAIN_HOME = origGbrainHome;
...
describeE2E('E2E: v0.11.0 orchestrator against live Postgres', () => {
  test('fresh install flow: schema → smoke → prefs → host-rewrite → completed', async () => {
    ...
    const prefsPath = join(tmp, '.zbrain', 'preferences.json');
    ...
    // tests/unit/apply-migrations.test.ts + tests/unit/migration-resume.test.ts.
  ...
  test('host rewrite: builtin handlers auto-rewritten, non-builtins queued as JSONL TODOs', async () => {
    ...
    // Fixture: AGENTS.md + cron/jobs.json with a mix of zbrain-builtin and
    ...
    expect(cronAfter.jobs[0].cmd).toContain('zbrain jobs submit sync');
    ...
    expect(agentsMdAfter).toContain('zbrain:subagent-routing v0.11.0');
    expect(agentsMdAfter).toContain('skills/conventions/subagent-routing.md');

    // JSONL TODO file written under ~/.zbrain/migrations/.
    const jsonlPath = join(tmp, '.zbrain', 'migrations', 'pending-host-work.jsonl');
  ...
  test('resumable: partial run → orchestrator re-run → complete', async () => {
    ...
    mkdirSync(join(tmp, '.zbrain', 'migrations'), { recursive: true });
    writeFileSync(
      join(tmp, '.zbrain', 'migrations', 'completed.jsonl'),
      JSON.stringify({
        version: '0.11.0',
        status: 'partial',
