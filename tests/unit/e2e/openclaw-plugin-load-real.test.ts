 ...
 *
 * Why this exists:
 *   The unit/e2e tests in tests/unit/context-engine.test.ts and
 *   tests/unit/e2e/openclaw-context-engine-plugin.test.ts both run STANDALONE —
 ...
 *   4. `openclaw config set plugins.slots.contextEngine zbrain-context` →
 ...
 * tests/unit/e2e/skills.test.ts). Uses an isolated `--profile` so the user's real
...
const PLUGIN_ID = 'zbrain-context-engine';
const ENGINE_ID = 'zbrain-context';
...
const PROFILE = `zbrain-ctx-e2e-${process.pid}`;
...
describe('openclaw-plugin-load-real (Tier 2 e2e)', () => {
  beforeAll(async () => {
    ...
    fixtureDir = mkdtempSync(join(tmpdir(), 'zbrain-ctx-plugin-real-'));

    // Write the fixture's package.json + openclaw.plugin.json from templates.
    const fixtureTemplate = join(repoRoot, 'tests/unit', 'fixtures', 'openclaw-plugin-real');
    writeFileSync(
    ...
    // dangerous-code scanner flags the surrounding zbrain repo (tests use
  ...
  it.skipIf(SKIP)(
    ...
    () => {
      ...
      expect(inspect.plugin.name).toBe('ZBrain Context Engine');
  ...
  it.skipIf(SKIP)(
    'plugins.slots.contextEngine binding to zbrain-context validates cleanly',
    () => {
  ...
  it.skipIf(SKIP)(
    ...
    async () => {
      ...
      const { createZBrainContextEngine } = await import('../../../src/core/context-engine.ts');

      const tmp = mkdtempSync(join(tmpdir(), 'zbrain-ctx-sdk-rt-'));
      try {
        ...
        registerContextEngine!(
          dynamicId,
          () => createZBrainContextEngine({ workspaceDir: tmp }),
        );
        ...
        const engine = createZBrainContextEngine({ workspaceDir: tmp });
