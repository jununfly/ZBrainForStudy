 ...
 * disk. Asserts:
 *   - audit phase writes ~/.zbrain/migrations/v0.22.4-audit.json with the
 *     expected per-source counts.
 *   - emit-todo phase appends one entry per source-with-issues to
 *     ~/.zbrain/migrations/pending-host-work.jsonl, each pointing at
 *     skills/migrations/v0.22.4.md (dotted convention) with the exact
 *     zbrain frontmatter validate <source-path> --fix command.
 ...
 *
 * Run: bun test tests/unit/e2e/frontmatter-migration.test.ts
 */
...
beforeAll(async () => {
  ...
  mkdirSync(join(tmpHome, '.zbrain', 'migrations'), { recursive: true });
  ...
  // Redirect ~/.zbrain/migrations/ output. The orchestrator's gbrainDir()
...
describe('E2E: v0.22.4 frontmatter-guard migration', () => {
  ...
  test('audit JSON report exists and has per-source counts', () => {
    const reportPath = join(tmpHome, '.zbrain', 'migrations', 'v0.22.4-audit.json');
  ...
  test('pending-host-work.jsonl carries one entry per source-with-issues', () => {
    const jsonlPath = join(tmpHome, '.zbrain', 'migrations', 'pending-host-work.jsonl');
    ...
    for (const e of entries) {
      ...
      expect(e.command).toContain('zbrain frontmatter validate');
