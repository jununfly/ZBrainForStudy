/**
 * v0.31 E2E — `zbrain recall --today` markdown render against real Postgres.
...
d('zbrain recall --today (Postgres)', () => {
  test('renders markdown with kind icons', async () => {
    if (!RUN) return;
    const engine = getEngine();
    await engine.insertFact(
      { fact: 'render-event', kind: 'event', entity_slug: 'render-pg-e', source: 'tests/unit' },
      { source_id: 'default' },
    );
    await engine.insertFact(
      { fact: 'render-pref', kind: 'preference', entity_slug: 'render-pg-p', source: 'tests/unit' },
      { source_id: 'default' },
    );
