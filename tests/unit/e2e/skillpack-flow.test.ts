/**
 * E2E flow tests for `zbrain skillpack` — the new v0.33 scaffold+
 * reference+migrate+harvest contract.
 *
 * Real zbrain subprocess against tempdir workspaces. No DATABASE_URL
...
const ZBRAIN_CMD = 'bun';
const ZBRAIN_ARGS = ['run', join(REPO_ROOT, 'src', 'cli.ts')];
...
function runGbrain(args: string[], opts: { cwd?: string } = {}): RunResult {
  const result = spawnSync(ZBRAIN_CMD, [...ZBRAIN_ARGS, ...args], {
...
describe('skillpack flow (E2E)', () => {
  ...
  test('4. reference --apply-clean-hunks applies upstream change to local workspace', () => {
    // Use a private bundle (scratch zbrain root) so we can mutate the
    // bundle source without polluting the real zbrain repo.
    const zbrainRoot = mkdtempSync(join(tmpdir(), 'sp-e2e-gb-'));
    tempdirs.push(zbrainRoot);
    mkdirSync(join(zbrainRoot, 'src'), { recursive: true });
    writeFileSync(join(zbrainRoot, 'src', 'cli.ts'), '// stub');
    mkdirSync(join(zbrainRoot, 'skills', 'apply-demo'), { recursive: true });
    const initial = Array.from({ length: 20 }, (_, i) => `Line ${i + 1}`).join('\n') + '\n';
    writeFileSync(join(zbrainRoot, 'skills', 'apply-demo', 'SKILL.md'), initial);
    writeFileSync(
      join(zbrainRoot, 'openclaw.plugin.json'),
      JSON.stringify({
        name: 'zbrain-test',
        version: '0.33.0-test',
    ...
    const ws = scratchWorkspace();
    // Run scaffold from zbrainRoot so it picks up our scratch bundle.
    const r0 = spawnSync(ZBRAIN_CMD, [...ZBRAIN_ARGS, 'skillpack', 'scaffold', 'apply-demo', '--workspace', ws], {
      cwd: zbrainRoot,
      encoding: 'utf-8',
    ...
    // zbrain ships an upstream change.
    writeFileSync(
      join(zbrainRoot, 'skills', 'apply-demo', 'SKILL.md'),
      initial.replace('Line 10\n', 'Line 10 UPSTREAM\n'),
    );

    // Apply clean hunks.
    const r = spawnSync(ZBRAIN_CMD, [...ZBRAIN_ARGS, 'skillpack', 'reference', 'apply-demo', '--workspace', ws, '--apply-clean-hunks'], {
      cwd: zbrainRoot,
      encoding: 'utf-8',
      env: { ...process.env, OPENCLAW_WORKSPACE: '' },
    });
...
<!-- zbrain:skillpack:begin -->

<!-- zbrain:skillpack:manifest cumulative-slugs="legacy-skill" version="0.32.0" -->

| Trigger | Skill |
|---------|-------|
| "legacy trigger" | \`skills/legacy-skill/SKILL.md\` |

<!-- zbrain:skillpack:end -->
`,
    );
    ...
    expect(rewritten).not.toContain('zbrain:skillpack:begin');
  ...
  test('7. harvest privacy-lint catches Wintermute → exit 1', () => {
    ...
    mkdirSync(join(zbrainRoot, 'src'), { recursive: true });
    writeFileSync(join(zbrainRoot, 'src', 'cli.ts'), '// stub');
    mkdirSync(join(zbrainRoot, 'skills'), { recursive: true });
    writeFileSync(
      join(zbrainRoot, 'openclaw.plugin.json'),
    ...
    const r = spawnSync(
      ZBRAIN_CMD,
      [...ZBRAIN_ARGS, 'skillpack', 'harvest', 'contaminated', '--from', hostRoot],
      { cwd: zbrainRoot, encoding: 'utf-8', env: { ...process.env, OPENCLAW_WORKSPACE: '' } },
    );
    expect(r.status).toBe(1); // lint_failed
    expect((r.stdout ?? '') + (r.stderr ?? '')).toContain('Wintermute');
    expect(existsSync(join(zbrainRoot, 'skills', 'contaminated'))).toBe(false); // rolled back
  });
  ...
  test('8. harvest --no-lint bypasses the privacy linter', () => {
    ...
    expect(r.status).toBe(0);
    expect(existsSync(join(zbrainRoot, 'skills', 'bypass-test', 'SKILL.md'))).toBe(true);
  });
  ...
  test('14. --apply-clean-hunks prints two-way WARNING to STDERR, not stdout', () => {
    ...
    const zbrainRoot = mkdtempSync(join(tmpdir(), 'sp-e2e-gb-warn-'));
    tempdirs.push(zbrainRoot);
    mkdirSync(join(zbrainRoot, 'src'), { recursive: true });
    writeFileSync(join(zbrainRoot, 'src', 'cli.ts'), '// stub');
    mkdirSync(join(zbrainRoot, 'skills', 'warn-demo'), { recursive: true });
    const initial = Array.from({ length: 15 }, (_, i) => `L${i + 1}`).join('\n') + '\n';
    writeFileSync(join(zbrainRoot, 'skills', 'warn-demo', 'SKILL.md'), initial);
    writeFileSync(
      join(zbrainRoot, 'openclaw.plugin.json'),
      JSON.stringify({
    ...
    const ws = scratchWorkspace();
    spawnSync(ZBRAIN_CMD, [...ZBRAIN_ARGS, 'skillpack', 'scaffold', 'warn-demo', '--workspace', ws], {
      cwd: zbrainRoot, encoding: 'utf-8', env: { ...process.env, OPENCLAW_WORKSPACE: '' },
    });
    // Cause drift to make apply do something.
    writeFileSync(
      join(zbrainRoot, 'skills', 'warn-demo', 'SKILL.md'),
      initial.replace('L8\n', 'L8 NEW\n'),
    );

    const r = spawnSync(
      ZBRAIN_CMD,
      [...ZBRAIN_ARGS, 'skillpack', 'reference', 'warn-demo', '--workspace', ws, '--apply-clean-hunks'],
      { cwd: zbrainRoot, encoding: 'utf-8', env: { ...process.env, OPENCLAW_WORKSPACE: '' } },
    );
    expect(r.status).toBe(0);
    ...
    expect(r.stderr).toContain('aligned to zbrain');
  ...
  test('15. --apply-clean-hunks --json does NOT print the WARNING (machine mode)', () => {
    const zbrainRoot = mkdtempSync(join(tmpdir(), 'sp-e2e-gb-json-'));
    tempdirs.push(zbrainRoot);
    mkdirSync(join(zbrainRoot, 'src'), { recursive: true });
    writeFileSync(join(zbrainRoot, 'src', 'cli.ts'), '// stub');
    mkdirSync(join(zbrainRoot, 'skills', 'json-demo'), { recursive: true });
    writeFileSync(join(zbrainRoot, 'skills', 'json-demo', 'SKILL.md'), 'a\nb\nc\n');
    writeFileSync(
      join(zbrainRoot, 'openclaw.plugin.json'),
      JSON.stringify({
        name: 'gb', version: '0.36-test',
    ...
    const ws = scratchWorkspace();
    spawnSync(ZBRAIN_CMD, [...ZBRAIN_ARGS, 'skillpack', 'scaffold', 'json-demo', '--workspace', ws], {
      cwd: zbrainRoot, encoding: 'utf-8', env: { ...process.env, OPENCLAW_WORKSPACE: '' },
    });

    const r = spawnSync(
      ZBRAIN_CMD,
      [...ZBRAIN_ARGS, 'skillpack', 'reference', 'json-demo', '--workspace', ws, '--apply-clean-hunks', '--json'],
      { cwd: zbrainRoot, encoding: 'utf-8', env: { ...process.env, OPENCLAW_WORKSPACE: '' } },
    );
    expect(r.status).toBe(0);
...
<!-- zbrain:skillpack:begin -->
<!-- zbrain:skillpack:manifest cumulative-slugs="lx" version="0.32.0" -->
