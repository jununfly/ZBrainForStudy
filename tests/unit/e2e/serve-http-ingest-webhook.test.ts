 ...
 * source registered inside `zbrain serve --http` per the plan-eng-review E1
 ...
 *
 * The pre-existing `tests/unit/e2e/ingestion-roundtrip.test.ts` covers the
 ...
 *
 * Spawns a real `zbrain serve --http` against real Postgres, mints OAuth
 ...
 *
 * Mirrors the spawn + mint pattern from tests/unit/e2e/serve-http-oauth.test.ts
 * exactly so future maintainers see one pattern, not two.
 *
 * Run: ZBRAIN_DATABASE_URL=... bun test tests/unit/e2e/serve-http-ingest-webhook.test.ts
 */
...
describeE2E('serve-http POST /ingest webhook (v0.38)', () => {
  ...
  test('X-Gbrain-Slug header is accepted (job receives the slug hint)', async () => {
    const token = await mintToken('read write');
    const slug = `webhook/tests/unit/header-${Date.now()}`;
    const res = await postIngest(
      token,
      'text/markdown',
      '# slug header test',
    ...
    // tests/unit/ingestion/ingest-capture.test.ts).
  });
