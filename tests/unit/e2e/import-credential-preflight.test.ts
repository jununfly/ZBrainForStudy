/**
 * v0.41.6.0 D1 E2E — `zbrain import` preflight rejects missing creds.
 *
 * Sibling of sync-credential-preflight.test.ts. Closes outside-voice F4:
 * pre-v0.41.6.0, `zbrain import <dir>` per-file embed wrote N identical
...
beforeAll(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-import-preflight-e2e-'));
...
beforeEach(() => {
  ...
  repoDir = mkdtempSync(join(tmpdir(), 'zbrain-import-preflight-repo-'));
...
describe('v0.41.6.0 D1 E2E — zbrain import preflight rejects missing OPENAI_API_KEY', () => {
