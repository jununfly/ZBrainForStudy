 ...
 *
 * The 21-test unit suite at `tests/unit/context-engine.test.ts` exercises
 * `createZBrainContextEngine` directly — that's the ENGINE, not the PLUGIN.
...
describe('openclaw-context-engine plugin entry', () => {
  it('default export has the expected plugin-entry shape', () => {
    ...
    expect(entry.id).toBe('zbrain-context-engine');
    expect(entry.name).toBe('ZBrain Context Engine');
  ...
  it('factory returns a working ContextEngine bound to the workspace', async () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), 'zbrain-plugin-e2e-'));
    try {
