 ...
 *
 * Mirrors tests/unit/cycle-consolidate.test.ts (PGLite) on Postgres so the
...
d('cycle consolidate phase (Postgres)', () => {
    ...
    for (let i = 0; i < 4; i++) {
      await engine.executeRaw(
        `INSERT INTO facts (source_id, entity_slug, fact, kind, source, confidence, valid_from, embedding, embedded_at)
         VALUES ('default', 'people/post-cons-alice', $1, 'fact', 'tests/unit', 0.9, $2::timestamptz, $3::vector, $2::timestamptz)`,
    ...
    for (let i = 0; i < 4; i++) {
      await engine.executeRaw(
        `INSERT INTO facts (source_id, entity_slug, fact, kind, source, valid_from, embedding, embedded_at)
         VALUES ('default', 'cons-recent', $1, 'fact', 'tests/unit', $2::timestamptz, $3::vector, $2::timestamptz)`,
  ...
  test('dryRun does not write rows even when bucket is eligible', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await engine.executeRaw(
        `INSERT INTO facts (source_id, entity_slug, fact, kind, source, valid_from, embedding, embedded_at)
         VALUES ('default', 'cons-dryrun-pg', $1, 'fact', 'tests/unit', $2::timestamptz, $3::vector, $2::timestamptz)`,
