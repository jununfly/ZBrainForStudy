 ...
 *
 * Spins up `zbrain serve --http` against a real Postgres, registers a
 * client with `read,write,admin` scope, runs `zbrain init --mcp-only`
 * against it from a second tempdir HOME, and exercises the canonical
 * thin-client flows:
 *
 *   - `zbrain init --mcp-only` succeeds and writes remote_mcp config
 *   - `zbrain doctor` reports `mode: thin-client` with all checks green
 *   - `zbrain sync` is refused with the canonical thin-client error
 *   - re-running `zbrain init` refuses without --force
 *
 * Tier B flows (`zbrain remote ping` / `remote doctor`) are stubbed for now
...
async function spawn(args: string[], home: string, extraEnv: Record<string, string | undefined> = {}): Promise<RunResult> {
  ...
  env.ZBRAIN_HOME = home;
  delete env.ZBRAIN_REMOTE_CLIENT_SECRET;
...
describeWhen('thin-client end-to-end (requires DATABASE_URL)', () => {
  let hostHome: string;          // ZBRAIN_HOME for the host (with local engine)
  let clientHome: string;        // ZBRAIN_HOME for the thin client (no engine)
  ...
  beforeAll(async () => {
    hostHome = mkdtempSync(join(tmpdir(), 'zbrain-thin-host-'));
    clientHome = mkdtempSync(join(tmpdir(), 'zbrain-thin-client-'));
  ...
  function parseRegisterClientOutput(out: string): { clientId: string; clientSecret: string } {
    // `zbrain auth register-client` doesn't have --json; parse human output:
    //   Client ID:     <id>
  ...
  test('init --mcp-only succeeds against the live host', async () => {
    ...
    const cfgPath = join(clientHome, '.zbrain', 'config.json');
    expect(existsSync(cfgPath)).toBe(true);
    const cfg = JSON.parse(readFileSync(cfgPath, 'utf-8'));
    expect(cfg.remote_mcp.oauth_client_id).toBe(clientId);
    // No PGLite file
    expect(existsSync(join(clientHome, '.zbrain', 'brain.pglite'))).toBe(false);
  });
  ...
  // ─── Tier B: zbrain remote ping + remote doctor ───

  test('zbrain remote doctor returns the host DoctorReport', async () => {
  ...
  // assertion needs. `zbrain serve --http` does NOT start a job worker
  // (workers run via the separate `zbrain jobs work` process). So a
  ...
  // fall back to the self-imposed `--timeout` firing, but `zbrain remote
  ...
  //      the loop overhead).
  //   2. OR start a `zbrain jobs work` subprocess in this test's beforeAll
  ...
  testRaw.skip('zbrain remote ping triggers autopilot-cycle and returns terminal state', async () => {
  ...
  test('client without admin scope cannot call run_doctor', async () => {
    ...
    // that zbrain remote doctor surfaces an auth-error message. This is the
    ...
    const lowScopeHome = mkdtempSync(join(tmpdir(), 'zbrain-thin-client-lowscope-'));
    try {
