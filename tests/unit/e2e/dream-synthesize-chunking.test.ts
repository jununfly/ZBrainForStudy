 ...
 *
 * Run: bun test tests/unit/e2e/dream-synthesize-chunking.test.ts
 */
...
async function setupRig(): Promise<TestRig> {
  ...
  const brainDir = mkdtempSync(join(tmpdir(), 'zbrain-chunk-brain-'));
  const corpusDir = mkdtempSync(join(tmpdir(), 'zbrain-chunk-corpus-'));
  return {
    engine,
    brainDir,
    corpusDir,
