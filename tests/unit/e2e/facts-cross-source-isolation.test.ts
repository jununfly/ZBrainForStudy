...
d('facts cross-source isolation (Postgres)', () => {
  test('listFactsByEntity scopes by source_id', async () => {
    ...
    await engine.insertFact(
      { fact: 'alpha alice', kind: 'fact', entity_slug: 'people/alice-example', source: 'tests/unit' },
      { source_id: 'alpha' },
    );
    await engine.insertFact(
      { fact: 'beta alice', kind: 'fact', entity_slug: 'people/alice-example', source: 'tests/unit' },
      { source_id: 'beta' },
    );
  ...
  test('CASCADE on sources delete drops the source\'s facts', async () => {
    ...
    await engine.insertFact(
      { fact: 'eph fact', kind: 'fact', source: 'tests/unit' },
      { source_id: 'eph-pg' },
    );
