 ...
 *
 * Run: DATABASE_URL=... bun test tests/unit/e2e/migration-v47-notability.test.ts
 */
...
describeE2E('migration v47: facts.notability ALTER', () => {
  ...
  test('CHECK constraint actually rejects out-of-domain values', async () => {
    ...
    await conn.unsafe(`
      ...
      VALUES ('default', '__v46_check_test_valid__', 'fact', 'tests/unit', 'high')
    `);
    ...
    try {
      await conn.unsafe(`
        INSERT INTO facts (source_id, fact, kind, source, notability)
        VALUES ('default', '__v46_check_test_invalid__', 'fact', 'tests/unit', 'critical')
      `);
    } catch (e) {
      threw = true;
