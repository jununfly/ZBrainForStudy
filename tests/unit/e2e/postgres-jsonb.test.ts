 ...
 *
 * Run: DATABASE_URL=... bun test tests/unit/e2e/postgres-jsonb.test.ts
 */
...
describeE2E('Postgres JSONB round-trip — frontmatter / data / pages_updated / metadata', () => {
  ...
  test('pages.frontmatter — putPage stores object, not string literal', async () => {
    ...
    await engine.putPage('jsonb-tests/unit/frontmatter', {
      type: 'concept',
    ...
    const rows = await conn.unsafe(`
      ...
      FROM pages
      WHERE slug = 'jsonb-tests/unit/frontmatter'
    `);
  ...
  test('raw_data.data — putRawData stores object, not string literal', async () => {
    ...
    await engine.putPage('jsonb-tests/unit/raw', { type: 'concept', title: 't', compiled_truth: '' });
    await engine.putRawData('jsonb-tests/unit/raw', 'unit-test', { kind: 'fixture', count: 42 });
    ...
    const rows = await conn.unsafe(`
      ...
      WHERE p.slug = 'jsonb-tests/unit/raw' AND rd.source = 'unit-test'
    `);
  ...
  test('files.metadata — write site uses sql.json, not string interpolation', async () => {
    ...
    const rows = await conn.unsafe(`
      ...
      FROM files
      WHERE storage_path = 'unit-tests/unit/roundtrip.bin'
    `);
  ...
  test('page_versions.frontmatter — INSERT...SELECT propagates object shape', async () => {
    ...
    await engine.putPage('jsonb-tests/unit/versioned', {
      type: 'concept',
      title: 'versioned',
      compiled_truth: 'v1',
      frontmatter: { mood: 'happy' },
    });
    await engine.createVersion('jsonb-tests/unit/versioned');
    ...
    const rows = await conn.unsafe(`
      ...
      WHERE p.slug = 'jsonb-tests/unit/versioned'
    `);
