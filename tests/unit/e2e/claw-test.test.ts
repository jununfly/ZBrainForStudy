/**
 * zbrain claw-test scripted-mode E2E.
 *
 ...
 * pglite.data, so a compiled zbrain can't init a fresh PGLite brain).
...
const BIN_CACHE = join(REPO_ROOT, 'tests/unit', '.cache');
const BIN_PATH = join(BIN_CACHE, 'zbrain.sh');
const SCENARIOS_DIR = join(REPO_ROOT, 'tests/unit', 'fixtures', 'claw-test-scenarios');
...
describe('zbrain claw-test --scenario fresh-install (scripted)', () => {
    ...
    try {
      const result = spawnSync(BIN_PATH, ['claw-test', '--scenario', 'fresh-install', '--keep-tempdir'], {
        ...
        env: {
          ...process.env,
          ZBRAIN_HOME: tmp,
          ZBRAIN_BIN_OVERRIDE: BIN_PATH,
          ZBRAIN_CLAW_SCENARIOS_DIR: join(REPO_ROOT, 'tests/unit', 'fixtures', 'claw-test-scenarios'),
        },
        encoding: 'utf-8',
        timeout: 120_000,
      });
      ...
      const frictionDir = join(tmp, '.zbrain', 'friction');
  ...
  test('break path: an invented command produces an error friction entry and exits non-zero', () => {
    // We do this by setting ZBRAIN_BIN_OVERRIDE to a script that pretends to be zbrain
    // and rejects the `import` subcommand specifically.
    const tmp = mkdtempSync(join(tmpdir(), 'claw-test-e2e-break-'));
    const fakeBin = join(tmp, 'fake-zbrain');
    try {
      // Write a shim that delegates to real zbrain but rejects 'import' to simulate breakage.
...
describe('zbrain friction render integration', () => {
  test('render produces a markdown report with the redact placeholder', () => {
    ...
    try {
      ...
      const env = { ...process.env, ZBRAIN_HOME: tmp, ZBRAIN_FRICTION_RUN_ID: 'render-e2e' };
      execFileSync(BIN_PATH, ['friction', 'log', '--phase', 'p', '--message', `error at ${home}/.zbrain/x`], { env, encoding: 'utf-8' });
      ...
      expect(out).not.toContain(home + '/.zbrain');
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  }, 30_000);
