 ...
 * a real git repo. Complements tests/unit/dream.test.ts (which exercises the
 * code paths via the library call) by testing the actual CLI output
 * shape and exit-code semantics against real DB state.
 *
 * Run: DATABASE_URL=... bun test tests/unit/e2e/dream.test.ts
 */
...
function makeGitRepo(): string {
  const dir = mkdtempSync(join(tmpdir(), 'zbrain-e2e-dream-'));
...
describeE2E('E2E: zbrain dream CLI against real Postgres', () => {
  let repo: string;
