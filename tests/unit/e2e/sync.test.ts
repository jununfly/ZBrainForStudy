 ...
 * markdown files, run zbrain sync, verify pages appear in the database.
 * Covers first sync, incremental add/modify/delete, and the critical
 * "edit → sync → search returns corrected text" flow.
 *
 * Run: DATABASE_URL=... bun test tests/unit/e2e/sync.test.ts
 */
...
function createTestRepo(): string {
  const dir = mkdtempSync(join(tmpdir(), 'zbrain-sync-e2e-'));
 ...
 * ~/.zbrain/sync-failures.jsonl so running E2E on a developer machine
 * doesn't trash their local sync state.
 */
describeE2E('E2E: sync --skip-failed structured summary loop (v0.22.12, issue #500)', () => {
  let repoPath: string;
  const realFailuresPath = join(homedir(), '.zbrain', 'sync-failures.jsonl');
  let savedFailuresContent: string | null = null;

  beforeAll(async () => {
    await setupDB();

    // Save+clear the real ~/.zbrain/sync-failures.jsonl so the test starts from
    ...
    repoPath = mkdtempSync(join(tmpdir(), 'zbrain-skipfailed-e2e-'));
