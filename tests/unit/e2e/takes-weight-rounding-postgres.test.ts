 ...
 *
 * The PGLite integration test (tests/unit/engine-weight-rounding-integration.test.ts)
 ...
 *     tolerance comparison tested in tests/unit/migrations-v48 — this is the runtime
 *     write path)
 *
...
beforeAll(async () => {
  ...
  await engine.executeRaw(`DELETE FROM pages WHERE slug = 'tests/unit/weight-rounding-postgres'`);
  const page = await engine.putPage('tests/unit/weight-rounding-postgres', {
