/**
 * E2E: gstack /setup-zbrain Path 4 unblock — register a remote source over
 * HTTP MCP, sync it, recover from clone deletion.
 *
 * Spawns a real `zbrain serve --http` against real Postgres with a fake-git
 ...
 *
 * Run: ZBRAIN_DATABASE_URL=... bun test tests/unit/e2e/sources-remote-mcp.test.ts
 */
...
const FIXTURE_DIR = join(tmpdir(), `zbrain-e2e-sources-${process.pid}`);
const ZBRAIN_HOME = join(FIXTURE_DIR, 'zbrain-home');
...
describeE2E('sources-remote-mcp E2E (gstack /setup-zbrain Path 4)', () => {
  ...
  beforeAll(async () => {
    ...
    writeFakeGit();
    rmSync(ZBRAIN_HOME, { recursive: true, force: true });
    mkdirSync(ZBRAIN_HOME, { recursive: true });
    ...
    //   - ZBRAIN_HOME: scope the clone dir to FIXTURE_DIR
    const subprocessEnv = {
      ...process.env,
    ...
    clientId = reg1.match(/Client ID:\s+(zbrain_cl_\S+)/)?.[1];
    const clientSecret = reg1.match(/Client Secret:\s+(zbrain_cs_\S+)/)?.[1];
    ...
    readOnlyClientId = reg2.match(/Client ID:\s+(zbrain_cl_\S+)/)?.[1];
    const readOnlySecret = reg2.match(/Client Secret:\s+(zbrain_cs_\S+)/)?.[1];
    if (!readOnlyClientId || !readOnlySecret) throw new Error('Failed to register read-only client');

    // Start the HTTP server with the fake-git PATH and our ZBRAIN_HOME.
    serverProcess = spawn(
      'bun',
    ...
    for (const id of [clientId, readOnlyClientId].filter(Boolean) as string[]) {
      try {
        execSync(`bun run src/cli.ts auth revoke-client ${id}`, {
          cwd: process.cwd(), encoding: 'utf8', env: { ...process.env, ZBRAIN_HOME },
        });
      } catch (e) {
  ...
  // -------------------------------------------------------------------------
  // Headline flow: gstack /setup-zbrain Path 4 unblock
  // -------------------------------------------------------------------------
  ...
  test('sources_add via MCP: clones, INSERTs, returns row with remote_url', async () => {
    ...
    if (process.env.ZBRAIN_E2E_DEBUG) {
    ...
    expect(existsSync(join(ZBRAIN_HOME, '.zbrain', 'clones', 'e2e-yc-artifacts', '.git'))).toBe(true);
  });
  ...
  test('recovery: rm clone dir → sources_status reports missing', async () => {
    const clonePath = join(ZBRAIN_HOME, '.zbrain', 'clones', 'e2e-yc-artifacts');
  ...
  test('sources_remove deletes row + cleans up the clone (managed path)', async () => {
    ...
    const clonePath = join(ZBRAIN_HOME, '.zbrain', 'clones', 'e2e-removable');
