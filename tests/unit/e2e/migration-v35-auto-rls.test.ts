 ...
 * backfill of existing public.* tables without RLS (modulo the ZBRAIN:RLS_EXEMPT
 * exemption that doctor honors). Postgres-only — PGLite has no RLS or event
 * triggers; that no-op is asserted in tests/unit/migrate.test.ts.
 *
 ...
 * tests start.
 *
 * Run: DATABASE_URL=... bun test tests/unit/e2e/migration-v35-auto-rls.test.ts
 */
...
describeE2E('migration v35: auto_rls_event_trigger', () => {
  ...
  test('backfill respects ZBRAIN:RLS_EXEMPT comment (matches doctor regex)', async () => {
    const conn = getConn();
    await conn`DROP EVENT TRIGGER IF EXISTS auto_rls_on_create_table`;
    try {
      ...
      // Comment must match doctor.ts EXEMPT_RE: /^ZBRAIN:RLS_EXEMPT\s+reason=\S.{3,}/
      // — "test exempt" is 11 chars after `reason=`, well over the .{3,} floor.
      await conn`COMMENT ON TABLE _test_backfill_exempt IS 'ZBRAIN:RLS_EXEMPT reason=test exempt'`;
