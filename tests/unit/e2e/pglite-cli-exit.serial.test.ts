 ...
 *
 * Pre-fix: `zbrain search`, `zbrain query`, `zbrain get` on PGLite
 ...
 * ZBRAIN_HOME tempdir, seeds a brain with 2 pages, runs each verb
 * with a hard timeout, and asserts exit 0. Without the drain helper
 * in cli.ts, every variant would time out.
 *
 * Bonus assertion: `zbrain serve --http` (a daemon) MUST stay alive
...
const SHIM_PATH = join(BIN_CACHE, 'zbrain-pglite-exit-shim.sh');
...
beforeAll(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-pglite-exit-'));
  repoSourceDir = mkdtempSync(join(tmpdir(), 'zbrain-pglite-exit-src-'));

  // Seed a tiny git repo with 2 markdown pages so `zbrain sync` has
  ...
  runEnv = { ...process.env, ZBRAIN_HOME: tmpHome };
  ...
  if (initResult.status !== 0) {
    throw new Error(
      `zbrain init failed (code=${initResult.status}):\n` +
        `STDOUT:\n${initResult.stdout}\n` +
        `STDERR:\n${initResult.stderr}`,
    );
  }
  ...
  if (syncResult.status !== 0) {
    throw new Error(
      `zbrain sync failed (code=${syncResult.status}):\n` +
        `STDOUT:\n${syncResult.stdout}\n` +
        `STDERR:\n${syncResult.stderr}`,
    );
  }
...
describe('v0.41.8.0 — PGLite CLI read commands exit cleanly (#1247/#1269/#1290)', () => {
  test('zbrain search "foxtrot" exits 0 within 15s', async () => {
  ...
  test('zbrain get returns a page body and exits 0 within 15s', async () => {
    const { code, stdout, stderr, durationMs } = await runWithTimeout(
      ['get', 'alpha'],
      15_000,
    );
  ...
  test('zbrain query without --no-expand exits 0 within 15s (no API key)', async () => {
...
describe('v0.41.8.0 — daemon survival (regression guard for narrow force-exit)', () => {
  test('zbrain serve --http stays alive past the timeout window', async () => {
    ...
    if (!wasAlive) {
      throw new Error(
        `zbrain serve --http exited within 3s (code=${earlyCode}). ` +
