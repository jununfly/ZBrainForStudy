...
describeBoth('Engine parity — Postgres vs PGLite', () => {
  ...
  test('hard-exclude is consistent across engines', async () => {
    // Both engines should hide tests/unit/ pages by default; both should opt
    // them back in via include_slug_prefixes.
    await pgEngine.putPage('tests/unit/parity-fixture', {
      type: 'note',
      title: 'parity test fixture',
      compiled_truth: 'parity test fixture content',
      timeline: '',
    });
    await pgEngine.upsertChunks('tests/unit/parity-fixture', [{
      chunk_index: 0,
    ...
    await pgliteEngine.putPage('tests/unit/parity-fixture', {
      type: 'note',
      title: 'parity test fixture',
      compiled_truth: 'parity test fixture content',
      timeline: '',
    });
    await pgliteEngine.upsertChunks('tests/unit/parity-fixture', [{
      chunk_index: 0,
      chunk_text: 'parity test fixture content',
      chunk_source: 'compiled_truth',
      embedding: basisEmbedding(20),
    ...
    expect(pgDefault.map((r: SearchResult) => r.slug)).not.toContain('tests/unit/parity-fixture');
    expect(pgliteDefault.map((r: SearchResult) => r.slug)).not.toContain('tests/unit/parity-fixture');

    const pgOptIn = await pgEngine.searchKeyword('parity test fixture', {
      include_slug_prefixes: ['tests/unit/'],
    });
    const pgliteOptIn = await pgliteEngine.searchKeyword('parity test fixture', {
      include_slug_prefixes: ['tests/unit/'],
    });
    expect(pgOptIn.map((r: SearchResult) => r.slug)).toContain('tests/unit/parity-fixture');
    expect(pgliteOptIn.map((r: SearchResult) => r.slug)).toContain('tests/unit/parity-fixture');
  });
  ...
  // on getPage. A drift here would mean `zbrain migrate --to supabase`
