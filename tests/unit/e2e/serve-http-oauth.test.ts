 ...
 *
 * Spins up a real `zbrain serve --http` against real Postgres, registers an
 ...
 *
 * Run: ZBRAIN_DATABASE_URL=... bun test tests/unit/e2e/serve-http-oauth.test.ts
 */
...
describeE2E('serve-http OAuth 2.1 E2E (v0.26.1 + v0.26.2 + v0.26.3)', () => {
  ...
  beforeAll(async () => {
    ...
    const idMatch = regOutput.match(/Client ID:\s+(zbrain_cl_\S+)/);
    const secretMatch = regOutput.match(/Client Secret:\s+(zbrain_cs_\S+)/);
  ...
  test('mint token via client_credentials grant', async () => {
    ...
    expect(data.access_token).toMatch(/^zbrain_at_/);
  ...
  test('minted token works for tools/call — search executes', async () => {
    ...
    const res = await mcpCall(access_token, 'tools/call', {
      name: 'search',
      arguments: { query: 'zbrain', limit: 1 },
    });
  ...
  test('expired/invalid token is rejected at /mcp', async () => {
    const res = await mcpCall('zbrain_at_totally_fake_token', 'tools/list');
  ...
  test('admin dashboard serves SPA index.html (not Express error)', async () => {
    ...
    expect(html).toContain('ZBrain Admin');
  ...
  test('write-scoped token can call read operations', async () => {
    ...
    const res = await mcpCall(access_token, 'tools/call', {
      name: 'search',
      arguments: { query: 'tests/unit', limit: 1 },
    });
  ...
  test('v0.28.10: /admin/api/full-stats with valid admin cookie returns getStats() body', async () => {
    ...
    const statsRes = await fetch(`${BASE}/admin/api/full-stats`, {
      headers: { Cookie: `zbrain_admin=${cookieValue}` },
    });
  ...
  test('wrong client_secret is rejected at token endpoint', async () => {
    const res = await fetch(`${BASE}/token`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: `grant_type=client_credentials&client_id=${clientId}&client_secret=zbrain_cs_wrong_secret&scope=read`,
    });
  ...
  // the internal-store shape test in tests/unit/oauth.test.ts is not enough on its
  ...
  // tests/unit/oauth.test.ts proves DB DELETE works but does NOT prove the
  ...
  test('v0.26.3: /mcp request persists agent_name + params + error_message', async () => {
    ...
    const sql = postgres(process.env.ZBRAIN_DATABASE_URL || process.env.DATABASE_URL || '', { prepare: false });
    try {
  ...
  // and execute arbitrary commands on the zbrain host.
  //
