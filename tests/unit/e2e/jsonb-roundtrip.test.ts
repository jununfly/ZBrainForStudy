...
describeE2E('E2E: JSONB roundtrip — v0.12.1 reliability wave', () => {
  ...
  test('putPage writes frontmatter as object, not double-encoded string', async () => {
    ...
    await engine.putPage('tests/unit/jsonb-putpage', {
      type: 'concept',
    ...
    const [row] = await sql`
      ...
      FROM pages WHERE slug = 'tests/unit/jsonb-putpage'
    `;
  ...
  test('putRawData writes raw_data.data as object, not double-encoded string', async () => {
    const engine = getEngine();
    await engine.putPage('tests/unit/jsonb-rawdata', {
      type: 'concept',
      title: 'RawData test',
      compiled_truth: 'body',
      timeline: '',
    ...
    await engine.putRawData('tests/unit/jsonb-rawdata', 'unit-test', {
    ...
    const [row] = await sql`
      ...
      WHERE p.slug = 'tests/unit/jsonb-rawdata'
    `;
    expect(row.t).toBe('object');
  ...
  test('logIngest writes pages_updated as array, not double-encoded string', async () => {
    ...
    await engine.logIngest({
      ...
      pages_updated: ['tests/unit/a', 'tests/unit/b', 'tests/unit/c'],
    ...
    expect(row.first).toBe('tests/unit/a');
  });
