 ...
 * tests/unit/, archive/, attachments/, .raw/ are hard-excluded by default.
...
beforeAll(async () => {
  ...
  await engine.putPage('tests/unit/fixtures/widget', {
    type: 'note',
    title: 'Widget test fixture',
    compiled_truth: 'widget test fixture for the test suite',
    timeline: '',
  });
  await engine.upsertChunks('tests/unit/fixtures/widget', [
    {
      chunk_index: 0,
...
describe('searchKeyword default hard-excludes', () => {
  test('tests/unit/ pages are hidden by default', async () => {
    const results = await engine.searchKeyword('widget');
    const slugs = results.map(r => r.slug);
    expect(slugs).not.toContain('tests/unit/fixtures/widget');
  });
...
describe('searchKeyword include_slug_prefixes opt-back-in', () => {
  test('include_slug_prefixes: ["tests/unit/"] surfaces test pages', async () => {
    const results = await engine.searchKeyword('widget', {
      include_slug_prefixes: ['tests/unit/'],
    });
    const slugs = results.map(r => r.slug);
    expect(slugs).toContain('tests/unit/fixtures/widget');
  ...
  test('include_slug_prefixes lets caller opt back into both', async () => {
    const results = await engine.searchKeyword('widget', {
      include_slug_prefixes: ['tests/unit/', 'archive/'],
    });
    const slugs = results.map(r => r.slug);
    expect(slugs).toContain('tests/unit/fixtures/widget');
...
describe('searchVector hard-excludes', () => {
  test('tests/unit/ pages are excluded by default in vector search', async () => {
    const results = await engine.searchVector(basisEmbedding(11));
    // basisEmbedding(11) is the closest direction to tests/unit/fixtures/widget,
