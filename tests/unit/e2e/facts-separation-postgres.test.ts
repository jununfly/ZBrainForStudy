 ...
 *
 * Mirrors tests/unit/facts-separation-pglite.test.ts. Skips gracefully when
...
d("Cross-session recall test (Postgres)", () => {
  ...
  test('expireFact + listSupersessions on real Postgres', async () => {
    ...
    const r1 = await engine.insertFact(
      { fact: 'old', kind: 'fact', entity_slug: 'super-pg', source: 'tests/unit' },
      { source_id: 'default' },
    );
    const r2 = await engine.insertFact(
      { fact: 'new', kind: 'fact', entity_slug: 'super-pg', source: 'tests/unit' },
