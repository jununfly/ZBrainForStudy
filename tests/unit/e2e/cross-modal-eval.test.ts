/**
 * E2E for `zbrain eval cross-modal` runner via mocked gateway chat().
 *
 * Lives under tests/unit/e2e/ so the test-isolation lint (R2 — mock.module quarantine)
 * does not require a *.serial.test.ts rename: tests/unit/e2e/* is exempt from the
...
beforeEach(() => {
  tempDir = mkdtempSync(join(tmpdir(), 'zbrain-cme-e2e-'));
...
describe('zbrain eval cross-modal — runner verdict contract', () => {
