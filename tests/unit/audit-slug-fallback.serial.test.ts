 ...
 * Uses ZBRAIN_AUDIT_DIR override pointed at a tmpdir for hermeticity (the
...
beforeAll(() => {
  auditDir = mkdtempSync(join(tmpdir(), 'zbrain-slug-fallback-audit-'));
  savedEnv = process.env.ZBRAIN_AUDIT_DIR;
  process.env.ZBRAIN_AUDIT_DIR = auditDir;
});

afterAll(() => {
  if (savedEnv === undefined) delete process.env.ZBRAIN_AUDIT_DIR;
  else process.env.ZBRAIN_AUDIT_DIR = savedEnv;
