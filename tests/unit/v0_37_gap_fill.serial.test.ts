 ...
 *  - ZEROENTROPY_API_KEY env merge into ZBrainConfig (Lane C.3)
...
describe('Lane A.7 — chunk-row INSERT default tracks ai/defaults.ts constant', () => {
  ...
  test('upsertChunks without explicit model: row stores DEFAULT_EMBEDDING_MODEL', async () => {
    ...
    await engine.putPage('tests/unit/a7', { type: 'note', title: 'A.7', compiled_truth: 'hello' });
    await engine.upsertChunks('tests/unit/a7', [
...
describe('Lane B — init precedence chain (CLI > env > existing file > default)', () => {
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-v37-b-'));
    origHome = process.env.ZBRAIN_HOME;
    process.env.ZBRAIN_HOME = tmpHome;
  });

  afterEach(() => {
    rmSync(tmpHome, { recursive: true, force: true });
    if (origHome === undefined) delete process.env.ZBRAIN_HOME;
    else process.env.ZBRAIN_HOME = origHome;
  });
  ...
  test('configureGatewayWithMergedPrecedence honors CLI > env > file > gateway-default', async () => {
    ...
    const dotgbrain = join(tmpHome, '.zbrain');
    ...
    await withEnv(
      { ZBRAIN_EMBEDDING_MODEL: 'openai:text-embedding-3-small', ZBRAIN_EMBEDDING_DIMENSIONS: '768' },
      async () => {
        ...
        rg1();
        cg1({
          embedding_model: process.env.ZBRAIN_EMBEDDING_MODEL ?? 'voyage:voyage-3-large',
          embedding_dimensions: parseInt(process.env.ZBRAIN_EMBEDDING_DIMENSIONS!, 10),
          env: { ...process.env },
        });
...
// ─────────────────────────────────────────────────────────────────────
// Lane C.3 — ZEROENTROPY_API_KEY env merge into ZBrainConfig
// ─────────────────────────────────────────────────────────────────────
describe('Lane C.3 — env ZEROENTROPY_API_KEY merges into loadConfig', () => {
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-v37-c-'));
    origHome = process.env.ZBRAIN_HOME;
    process.env.ZBRAIN_HOME = tmpHome;
...
describe('Lane D.2 — embed pre-flight catches dim mismatch before worker pool', () => {
  ...
  beforeAll(async () => {
    ...
    await engine.connect({});
    await engine.initSchema();
    await engine.putPage('tests/unit/d2', { type: 'note', title: 'D.2', compiled_truth: 'hello world' });
    await engine.upsertChunks('tests/unit/d2', [
...
describe('reinit-pglite — backup + reinit', () => {
  ...
  beforeEach(() => {
    tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-v37-reinit-'));
    origHome = process.env.ZBRAIN_HOME;
    process.env.ZBRAIN_HOME = tmpHome;
