...
// trust gate (remote=true rejects per-call opt), zbrain-base loads from
...
describe('loadActivePack boundary helper', () => {
  beforeAll(() => {
    // Tests use the real bundled zbrain-base for tier-7 default; per-test
  ...
  test('default resolution loads zbrain-base from bundled path', async () => {
    _resetPackCacheForTests();
    const pack = await loadActivePack({ cfg: null, remote: false });
    expect(pack.manifest.name).toBe('zbrain-base');
  ...
  test('tier-2 env var ZBRAIN_SCHEMA_PACK wins over tier-6 home config', async () => {
    await withEnv({ ZBRAIN_SCHEMA_PACK: 'env-pack' }, async () => {
      const result = resolveActivePackNameOnly({
  ...
  test('tier-7 default falls back to zbrain-base when nothing set', () => {
    const result = resolveActivePackNameOnly({ cfg: null, remote: false });
    expect(result.pack_name).toBe('zbrain-base');
  ...
  test('injected locator overrides default disk path', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'zbrain-test-pack-'));
    const yamlPath = join(dir, 'pack.yaml');
    writeFileSync(yamlPath, `api_version: zbrain-schema-pack-v1
name: injected-pack
version: 0.1.0
description: test
extends: null
