...
describe('runPhaseConsolidate', () => {
    ...
    for (let i = 0; i < 2; i++) {
      await engine.executeRaw(
        `INSERT INTO facts (source_id, entity_slug, fact, kind, source, valid_from, embedding, embedded_at)
         VALUES ('default', 'cons-skip-count', $1, 'fact', 'tests/unit', $2::timestamptz, $3::vector, $2::timestamptz)`,
  ...
  test('all facts too recent → bucket processed but skipped, 0 work', async () => {
    ...
    for (let i = 0; i < 4; i++) {
      await engine.executeRaw(
        `INSERT INTO facts (source_id, entity_slug, fact, kind, source, valid_from, embedding, embedded_at)
         VALUES ('default', 'cons-skip-age', $1, 'fact', 'tests/unit', $2::timestamptz, $3::vector, $2::timestamptz)`,
    ...
    for (let i = 0; i < 4; i++) {
      await engine.executeRaw(
        `INSERT INTO facts (source_id, entity_slug, fact, kind, source, valid_from, confidence, embedding, embedded_at)
         VALUES ('default', 'people/alice-example', $1, 'fact', 'tests/unit', $2::timestamptz, 0.9, $3::vector, $2::timestamptz)`,
  ...
  test('dryRun honored: counters tick but no rows written', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      await engine.executeRaw(
        `INSERT INTO facts (source_id, entity_slug, fact, kind, source, valid_from, embedding, embedded_at)
         VALUES ('default', 'cons-dryrun', $1, 'fact', 'tests/unit', $2::timestamptz, $3::vector, $2::timestamptz)`,
  ...
  test('skips bucket when no matching page exists in source', async () => {
    ...
    for (let i = 0; i < 4; i++) {
      await engine.executeRaw(
        `INSERT INTO facts (source_id, entity_slug, fact, kind, source, valid_from, embedding, embedded_at)
         VALUES ('default', 'no-page', $1, 'fact', 'tests/unit', $2::timestamptz, $3::vector, $2::timestamptz)`,
