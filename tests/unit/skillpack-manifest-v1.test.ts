...
describe('validateSkillpackManifest — required fields', () => {
  ...
  test('rejects unknown api_version', () => {
    const bad = { ...VALID_MANIFEST, api_version: 'zbrain-skillpack-v99' as 'zbrain-skillpack-v1' };
    try {
      validateSkillpackManifest(bad);
      throw new Error('should have thrown');
    } catch (err) {
...
describe('validateSkillpackManifest — field shape rules', () => {
  ...
  test('rejects zbrain_min_version that is not semver', () => {
    try {
      validateSkillpackManifest({ ...VALID_MANIFEST, zbrain_min_version: 'latest' });
      throw new Error('should have thrown');
    } catch (err) {
      expect((err as SkillpackManifestError).detail?.field).toBe('zbrain_min_version');
    }
  });
...
describe('validateSkillpackManifest — optional array fields', () => {
  ...
  test('rejects array with non-string entries', () => {
    try {
      validateSkillpackManifest({
        ...VALID_MANIFEST,
        unit_tests: ['tests/unit/foo.test.ts', 42] as unknown as string[],
      });
      throw new Error('should have thrown');
    } catch (err) {
