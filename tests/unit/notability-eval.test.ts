 ...
 * default. Operators with a private fixture (`~/.zbrain/eval/
 * notability-real.jsonl`, mined + hand-confirmed via `zbrain notability-
 * eval`) can opt into the larger run by setting ZBRAIN_NOTABILITY_EVAL_REAL=1.
 *
...
describe('JSONL utilities', () => {
  ...
  test('default paths resolve under ~/.zbrain/eval/', () => {
    expect(defaultMiningOutPath()).toContain('.zbrain/eval/notability-mining-candidates.jsonl');
    expect(defaultReviewOutPath()).toContain('.zbrain/eval/notability-real.jsonl');
  });
});
...
describe('public-anonymized fixture — shape contract', () => {
  test('40 cases, ~14 HIGH + ~13 MEDIUM + ~13 LOW', () => {
    const cases = loadJsonlCases<ConfirmedCase>(
      'tests/unit/fixtures/notability-eval-public.jsonl',
    );
...
describe('eval harness — precision@HIGH contract', () => {
  function makeChatStub(predictedTier: 'high' | 'medium' | 'low') {
    __setChatTransportForTests(async (): Promise<ChatResult> => ({
      ...
      model: 'test:stub',
      providerId: 'tests/unit',
    }));
  }

  test('100% perfect predictor → precision@HIGH = 1, recall@HIGH = 1, F1 = 1', async () => {
    const fixture = loadJsonlCases<ConfirmedCase>('tests/unit/fixtures/notability-eval-public.jsonl');
    let tp = 0;
    let fp = 0;
    let fn = 0;
