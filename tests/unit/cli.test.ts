...
describe('CLI structure', () => {
  ...
  // rejected `zbrain reindex` with "Unknown command: reindex" before the
...
describe('CLI dispatch integration', () => {
  test('--version outputs version', async () => {
    ...
    const stdout = await new Response(proc.stdout).text();
    await proc.exited;
    expect(stdout.trim()).toMatch(/^zbrain \d+\.\d+\.\d+/);
  });
  ...
  test('per-command --help prints usage without DB connection', async () => {
    ...
    const stdout = await new Response(proc.stdout).text();
    const exitCode = await proc.exited;
    expect(stdout).toContain('Usage: zbrain get');
  ...
  test('--help prints global help', async () => {
    ...
    expect(stdout).toContain('zbrain <command>');
    expect(exitCode).toBe(0);
  });
