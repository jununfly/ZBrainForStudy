/**
 * v0.41.13 (#1422) — `zbrain dream` surfaces engine-connect failures on stderr.
 *
...
describe('#1422 — dream surfaces connectEngine failures', () => {
  test('connect failure prints WARNING line on stderr (does not swallow silently)', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-dream-warn-'));
    const tmpBrain = mkdtempSync(join(tmpdir(), 'zbrain-dream-brain-'));
    try {
      // Point ZBRAIN_HOME at an empty tempdir so loadConfig sees no config,
      ...
      // ZBRAIN_HOME=/tmp/x yields configDir() === '/tmp/x/.zbrain' (config.ts:687-690).
      mkdirSync(join(tmpHome, '.zbrain'), { recursive: true });
      writeFileSync(join(tmpHome, '.zbrain', 'config.json'), JSON.stringify({
        engine: 'postgres',
  ...
  test('successful connect emits NO WARNING line', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-dream-ok-'));
    const tmpBrain = mkdtempSync(join(tmpdir(), 'zbrain-dream-brain-'));
    try {
