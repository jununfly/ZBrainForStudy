/**
 * v0.31 Phase 6 — `zbrain recall --today` markdown render shape.
 *
...
describe('zbrain recall --today', () => {
  ...
  test('kind icons appear in the rendered output', async () => {
    ...
    await engine.insertFact(
      { fact: 'event-fact', kind: 'event', entity_slug: 'render-test-e', source: 'tests/unit' },
      { source_id: 'default' },
    );
    await engine.insertFact(
      { fact: 'pref-fact', kind: 'preference', entity_slug: 'render-test-p', source: 'tests/unit' },
      { source_id: 'default' },
    );
    await engine.insertFact(
      { fact: 'commit-fact', kind: 'commitment', entity_slug: 'render-test-c', source: 'tests/unit' },
      { source_id: 'default' },
    );
    await engine.insertFact(
      { fact: 'belief-fact', kind: 'belief', entity_slug: 'render-test-b', source: 'tests/unit' },
      { source_id: 'default' },
    );
    await engine.insertFact(
      { fact: 'fact-fact', kind: 'fact', entity_slug: 'render-test-f', source: 'tests/unit' },
      { source_id: 'default' },
    );
...
describe('zbrain recall --as-context', () => {
  test('emits markdown comment-wrapped block', async () => {
    ...
    expect(captured.startsWith('<!-- zbrain hot memory') || captured.includes('zbrain hot memory')).toBe(true);
  });
});
