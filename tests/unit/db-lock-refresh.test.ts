...
describe('LockUnavailableError', () => {
  test('carries the lock id', () => {
    const err = new LockUnavailableError('zbrain-migrate:postgres');
    expect(err).toBeInstanceOf(Error);
    expect(err.name).toBe('LockUnavailableError');
    expect(err.lockId).toBe('zbrain-migrate:postgres');
    expect(err.message).toContain('zbrain-migrate:postgres');
  });
});
...
describe('buildTenantLockId — D4 multi-tenant safety', () => {
  test('postgres engine: queries current_database()', async () => {
    const fakeEngine = {
      ...
      executeRaw: async () => [{ db: 'zbrain_main' }],
    } as unknown as Parameters<typeof buildTenantLockId>[0];
    const id = await buildTenantLockId(fakeEngine, 'zbrain-migrate');
    expect(id).toBe('zbrain-migrate:zbrain_main');
  });
  ...
  test('pglite engine: returns scope:pglite', async () => {
    ...
    const id = await buildTenantLockId(fakeEngine, 'zbrain-migrate');
    expect(id).toBe('zbrain-migrate:pglite');
  });
  ...
  test('failure path: returns scope:unknown rather than throwing', async () => {
    ...
    const id = await buildTenantLockId(fakeEngine, 'zbrain-migrate');
    expect(id).toBe('zbrain-migrate:unknown');
  });
  ...
  test('two scopes share dbname suffix', async () => {
    ...
    const a = await buildTenantLockId(fakeEngine, 'zbrain-migrate');
    const b = await buildTenantLockId(fakeEngine, 'zbrain-hnsw');
    expect(a).toBe('zbrain-migrate:shared');
    expect(b).toBe('zbrain-hnsw:shared');
    expect(a).not.toBe(b);
  });
});
