...
describe('loadStorageConfig — real-disk loader', () => {
  ...
  beforeEach(() => {
    tmp = mkdtempSync(join(tmpdir(), 'zbrain-storage-test-'));
    __resetMissingStorageWarning();
    warnings = [];
  ...
  test('returns null when zbrain.yml does not exist', () => {
    try {
  ...
  test('loads canonical zbrain.yml — the test that would have caught the original gray-matter P0', () => {
    try {
...
storage:
  ...
  db_only:
    ...
    - media/articles/
`;
      writeFileSync(join(tmp, 'zbrain.yml'), yaml);
...
`;
  ...
  test('warns once when zbrain.yml exists but storage section is missing', () => {
    try {
      writeFileSync(join(tmp, 'zbrain.yml'), 'something_else: foo\n');
      const config = loadStorageConfig(tmp);
...
`;
  ...
  test('throws on unreadable zbrain.yml (permission denied) — does not silently disable feature', () => {
    try {
      const yamlPath = join(tmp, 'zbrain.yml');
