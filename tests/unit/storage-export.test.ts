...
beforeEach(async () => {
  tmp = mkdtempSync(join(tmpdir(), 'zbrain-export-test-'));
  outDir = join(tmp, 'out');
  exitCode = null;
  stderr = [];
  stdout = [];
...
describe('export --restore-only resolution chain (D5)', () => {
  ...
  test('uses explicit --repo when provided', async () => {
    // Make a brain repo with zbrain.yml that has empty db_only — so we
...
`,
  ...
  test('falls back to sources default local_path when --repo absent', async () => {
    // Configure default source path, write a real zbrain.yml so the storage
    // config check passes — without zbrain.yml the Codex-P0 guard correctly
