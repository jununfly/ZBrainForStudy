...
// These tests exercise the filesystem walker that feeds `zbrain import`.
...
describe('collectMarkdownFiles — symlink containment', () => {
  ...
  beforeEach(() => {
    ...
    root = mkdtempSync(join(tmpdir(), 'zbrain-walker-root-'));
    secretDir = mkdtempSync(join(tmpdir(), 'zbrain-walker-secret-'));
  });
