...
describe('resolveSourceId priority 2 — ZBRAIN_SOURCE env', () => {
  test('wins over dotfile / registered-path / default', async () => {
    ...
    process.env.ZBRAIN_SOURCE = 'env-wins';
    try {
...
describe('resolveSourceId priority 3 — .zbrain-source dotfile walk-up', () => {
  let tmpdirPath: string;

  beforeEach(() => {
    tmpdirPath = mkdtempSync(join(tmpdir(), 'zbrain-resolver-test-'));
  });
  afterEach(() => {
  ...
  test('finds dotfile in CWD', async () => {
    writeFileSync(join(tmpdirPath, '.zbrain-source'), 'gstack\n');
  ...
  test('walks up ancestors to find dotfile', async () => {
    writeFileSync(join(tmpdirPath, '.zbrain-source'), 'wiki\n');
  ...
  test('ignores dotfile with invalid content', async () => {
    writeFileSync(join(tmpdirPath, '.zbrain-source'), 'INVALID!\n');
    const engine = makeStub(['default'], [], null);
    const id = await resolveSourceId(engine, null, tmpdirPath);
