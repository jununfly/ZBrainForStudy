...
const SYNTHETIC_EMBED_RECIPE: Recipe = {
  ...
  base_url_default: 'https://synthetic.tests/unit/v1',
  ...
  default_headers: {
    ...
    'X-OpenRouter-Title': 'zbrain',
    'X-Title': 'zbrain',
  },
  resolveOpenAICompatConfig() {
    return {
      baseURL: 'https://synthetic.tests/unit/v1',
...
describe('transport-level header sweep (v0.37.2.0)', () => {
  test('1. embed — SDK applies Authorization + default_headers on every request', async () => {
    ...
    expect(h['http-referer'], 'HTTP-Referer must reach the wire').toBe('https://zbrain.ai');
    expect(h['x-openrouter-title'], 'X-OpenRouter-Title must reach the wire').toBe('zbrain');
    expect(h['x-title'], 'X-Title (back-compat) must reach the wire').toBe('zbrain');
  });
  ...
  test('2. chat — SDK applies Authorization + default_headers on every request', async () => {
    ...
    expect(h['http-referer']).toBe('https://zbrain.ai');
    expect(h['x-openrouter-title']).toBe('zbrain');
    expect(h['x-title']).toBe('zbrain');
  });
  ...
  test('3. rerank — manual HTTP path applies Authorization + default_headers', async () => {
    ...
    expect(h['http-referer'], 'rerank: HTTP-Referer must reach the wire').toBe('https://zbrain.ai');
    expect(h['x-openrouter-title']).toBe('zbrain');
    expect(h['x-title']).toBe('zbrain');
