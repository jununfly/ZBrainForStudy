...
interface FakeFile {
  /** Path relative to the tmpdir's `tests/unit/` directory. */
  path: string;
  contents: string;
}
...
function runLintIn(files: FakeFile[], allowlist: string[] = []): RunResult {
  ...
  for (const f of files) {
    const full = join(dir, 'tests/unit', f.path);
  ...
  const r = spawnSync('bash', [LINT_SH, 'tests/unit'], {
    cwd: dir,
    encoding: 'utf-8',
    env: { ...process.env },
  });
...
describe('check-test-isolation.sh', () => {
  ...
  describe('scope', () => {
    ...
    it('skips tests/unit/e2e/ subtree', () => {
      const r = runLintIn([
        {
  ...
  describe('allowlist', () => {
    ...
    it('treats # comments and blank lines in allowlist as no-ops', () => {
      const r = runLintIn(
        ...
        ['# legacy file', '', 'tests/unit/legacy.test.ts'],
      );
      expect(r.status).toBe(0);
    });
  });
