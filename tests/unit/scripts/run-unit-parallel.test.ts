...
beforeAll(() => {
  ...
  TMPROOT = mkdtempSync(join(tmpdir(), 'zbrain-parallel-test-'));
...
describe('failing-on-purpose', () => {
  ...
  writeFileSync(join(TMPROOT, 'tests/unit', 'a-pass.test.ts'), passing);
  writeFileSync(join(TMPROOT, 'tests/unit', 'b-pass.test.ts'), passing);
  writeFileSync(join(TMPROOT, 'tests/unit', 'c-pass.test.ts'), passing);
  writeFileSync(join(TMPROOT, 'tests/unit', 'd-fail.test.ts'), failing);
});
...
describe('run-unit-parallel.sh exit-code propagation (a)', () => {
  ...
  it('exits zero when all shards pass (after removing the failing fixture)', () => {
    rmSync(join(TMPROOT, 'tests/unit', 'd-fail.test.ts'));
    try {
      const r = runWrapper();
