 ...
 *   - tests/unit/foo.test.ts edit → DIFFERENT hash
 *
...
const BASELINE_FILES: Record<string, string> = {
  ...
  "AGENTS.md": "# AGENTS.md\n\nopenclaw entry\n",
  "package.json": '{"name":"tests/unit","version":"0.0.0"}\n',
  "bun.lock": "lockfile-v1\n",
  "tsconfig.json": '{"compilerOptions":{"strict":true}}\n',
  "src/core/db.ts": "export const db = 1;\n",
  "src/cli.ts": "console.log('hi');\n",
  "tests/unit/foo.test.ts": "import {test} from 'bun:test';\ntest('x', () => {});\n",
...
describe("ci-cache-hash.sh — CRITICAL false-pass guards (must invalidate)", () => {
  ...
  it("tests/unit/foo.test.ts edit MUST change hash", () => {
    withSandbox((sb) => {
      const before = hash(sb);
      modify(
        sb,
  ...
  it("package.json edit MUST change hash", () => {
    withSandbox((sb) => {
      const before = hash(sb);
      modify(sb, "package.json", '{"name":"tests/unit","version":"0.0.1"}\n');
      const after = hash(sb);
      expect(after).not.toBe(before);
    });
  });
