 ...
 *
 *  1. EXCLUDE *.serial.test.ts and tests/unit/e2e/ from every shard. Serial
...
describe('test-shard.sh — exclusion contract', () => {
  ...
  it('excludes the tests/unit/e2e/ subtree from every shard', () => {
    for (const shard of [1, 2, 3, 4]) {
      const files = dryRunList(shard, 4);
      const leaks = files.filter(f => f.startsWith('tests/unit/e2e/'));
