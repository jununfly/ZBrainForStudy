 ...
 * `zbrain serve` as a non-daemon (or any other intentional long-
...
describe('shouldForceExitAfterMain — daemon survival gate', () => {
  ...
  test('returns false even when global flags precede `serve`', () => {
    ...
    // flag position. This is the load-bearing case for `zbrain --quiet serve`.
  ...
  test('substring match avoidance: `serves` is NOT `serve`', () => {
    // Future-proofing against a `zbrain serves-foo` subcommand being
