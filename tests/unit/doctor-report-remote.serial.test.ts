...
beforeAll(async () => {
  // v0.37.10.0: doctorReportRemote reads from ~/.zbrain audit files
  ...
  // ZBRAIN_HOME to a tempdir so audit reads return empty.
  tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-doctor-remote-'));
  priorHome = process.env.ZBRAIN_HOME;
...
afterAll(async () => {
  ...
  if (priorHome === undefined) delete process.env.ZBRAIN_HOME;
  else process.env.ZBRAIN_HOME = priorHome;
