 ...
 * `~/.zbrain`. `withEnv` covers the ZBRAIN_AUDIT_DIR escape hatch.
 */
...
beforeEach(() => {
  tmp = mkdtempSync(join(tmpdir(), 'zbrain-budget-test-'));
...
describe('BudgetTracker.reserve', () => {
  test('passes when under cap with known pricing', () => {
    const t = new BudgetTracker({ maxCostUsd: 1.0, label: 'tests/unit', auditPath });
    expect(() =>
      t.reserve({
  ...
  test('throws BudgetExhausted (reason: cost) when projected > cap', () => {
    const t = new BudgetTracker({ maxCostUsd: 0.001, label: 'tests/unit', auditPath });
  ...
  test('throws BudgetExhausted (reason: runtime) when wall-clock cap blown', () => {
    const t = new BudgetTracker({ maxRuntimeMs: 1, label: 'tests/unit', auditPath });
  ...
  test('no cap + unknown pricing: warns once per process, no throw', () => {
    const t = new BudgetTracker({ label: 'tests/unit', auditPath });
    expect(() =>
      t.reserve({
  ...
  test('v0.40.6.1: rerank kind for llama-server-reranker prices at $0 (no TX2 throw under --max-cost)', () => {
    ...
    const t = new BudgetTracker({ maxCostUsd: 0.0001, label: 'tests/unit', auditPath });
    expect(() =>
      t.reserve({
...
describe('BudgetTracker.record', () => {
  test('TX1: cumulative > cap after under-estimated call throws BudgetExhausted', () => {
    const t = new BudgetTracker({ maxCostUsd: 0.01, label: 'tests/unit', auditPath });
