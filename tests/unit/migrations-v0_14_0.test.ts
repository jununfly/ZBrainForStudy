 ...
 *     ~/.zbrain/migrations/pending-host-work.jsonl
 *
...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'zbrain-v0_14_0-'));
  // ZBRAIN_HOME is the parent dir; configDir() appends '.zbrain' itself.
  process.env.ZBRAIN_HOME = tmpHome;
});

afterEach(() => {
  if (originalGbrainHome !== undefined) process.env.ZBRAIN_HOME = originalGbrainHome;
...
describe('Bug 5 — Phase B host-work entry dedup', () => {
  test('first run writes the entry, second run is a skip', async () => {
    ...
    const hostPath = join(tmpHome, '.zbrain', 'migrations', 'pending-host-work.jsonl');
