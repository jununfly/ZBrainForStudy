/**
 * v0.32 — `zbrain recall --rollup` correctness. Pure-function tests on
 ...
 *      `tests/unit/facts-doctor-shape.test.ts:49` — NOT `{slug, count}`.
 */
...
describe('computeRollup JSON shape (Codex round 1 #8 shape-drift regression)', () => {
  test('every row uses the key `entity_slug` (matches engine.getStats and tests/unit/facts-doctor-shape.test.ts:49)', () => {
