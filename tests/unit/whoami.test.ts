 ...
 * tests/unit/e2e/serve-http-oauth.test.ts and tests/unit/e2e/sources-remote-mcp.test.ts.
 */
...
describe('whoami op contract', () => {
  ...
  test('oauth transport returns full client identity', async () => {
    ...
    expect(result.client_id).toBe('zbrain_cl_abc');
  ...
  test('legacy transport (token name as clientId, no zbrain_cl_ prefix)', async () => {
    const auth: AuthInfo = {
      token: 'legacy-token',
...
describe('whoami op metadata', () => {
  ...
  test('not localOnly (must work over HTTP MCP for gstack /setup-zbrain)', () => {
