 ...
 *
 * Pre-v0.40 phantom-redirect acquired the bare `zbrain-sync` lock, blocking
 * every concurrent sync brain-wide. After D16, phantom acquires
 * `zbrain-sync:<sourceId>` — same-source sync still serializes; cross-source
 * sync proceeds unblocked.
 *
 * Pure source-text regression guard. The full behavior is covered by
 * `tests/unit/e2e/phantom-redirect.test.ts` (DATABASE_URL-gated). This file pins
...
describe('phantom-redirect lock contract', () => {
  ...
  test('helper sanity: syncLockId returns zbrain-sync:<source> shape', () => {
