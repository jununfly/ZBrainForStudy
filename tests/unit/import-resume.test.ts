 ...
 * live in `tests/unit/import-checkpoint.test.ts`. This file drives the full
 ...
 *
 * Test isolation:
 *   - `ZBRAIN_HOME` env override via `withEnv` so we NEVER touch the real
 *     `~/.zbrain/import-checkpoint.json`. Pre-v0.33.2 this file did exactly
...
let workspace: string;        // ZBRAIN_HOME target — `${workspace}/.zbrain/` holds the checkpoint file
let zbrainHomeDir: string;    // Resolves to `${workspace}/.zbrain` — the actual checkpoint dir
let cpPath: string;           // The checkpoint file path inside zbrainHomeDir
...
beforeEach(async () => {
  ...
  workspace = mkdtempSync(join(tmpdir(), 'zbrain-import-resume-home-'));
  // ZBRAIN_HOME is the parent dir; configDir() appends '.zbrain' itself.
  // The checkpoint lives at `${workspace}/.zbrain/import-checkpoint.json`.
  zbrainHomeDir = join(workspace, '.zbrain');
  mkdirSync(zbrainHomeDir, { recursive: true });
  cpPath = join(zbrainHomeDir, 'import-checkpoint.json');
  brainDir = mkdtempSync(join(tmpdir(), 'zbrain-import-resume-brain-'));
});
...
describe('runImport checkpoint resume — v0.33.2 path-based', () => {
  test('old positional checkpoint gets discarded with stderr log', async () => {
    await withEnv({ ZBRAIN_HOME: workspace }, async () => {
  ...
  test('failed file does NOT enter completedPaths — next run retries it', async () => {
    await withEnv({ ZBRAIN_HOME: workspace }, async () => {
      ...
      // 'wrong-slug' with a SLUG_MISMATCH failure (tests/unit/e2e/sync.test.ts uses
