 ...
 * Powers `zbrain sources current` so users can verify both the
...
describe('resolveSourceWithTier — tier 3 (dotfile)', () => {
  ...
  beforeEach(() => {
    scratchDir = mkdtempSync(join(tmpdir(), 'zbrain-tier-dotfile-'));
  });
  afterEach(() => {
    rmSync(scratchDir, { recursive: true, force: true });
  });

  test('.zbrain-source dotfile in CWD returns tier=dotfile', async () => {
    writeFileSync(join(scratchDir, '.zbrain-source'), 'team-alpha\n');
    ...
    expect(result.detail).toBe('.zbrain-source');
  });
...
describe('resolveSourceWithTier — priority assertion', () => {
  test('flag wins over env wins over dotfile wins over default', async () => {
    ...
    await withEnv({ ZBRAIN_SOURCE: 'env-src' }, async () => {
