 ...
 * handlers must shell out to the bare string `zbrain`, NOT to
...
describe('v0.13.0 — Frontmatter relationship indexing migration', () => {
  ...
  test('source does NOT build commands from a ZBRAIN constant (Bug 1 regression)', () => {
    // Earlier revisions used `const ZBRAIN = process.execPath` and built
    // commands as `${ZBRAIN} extract ...`. The constant was the vector.
    const src = readFileSync(SRC_PATH, 'utf-8');
    expect(src).not.toMatch(/const\s+ZBRAIN\s*=/);
    expect(src).not.toMatch(/\$\{ZBRAIN\}/);
  });

  test('phase commands invoke bare `zbrain` shell-out (Bug 1 fix)', () => {
    const src = readFileSync(SRC_PATH, 'utf-8');
    // All three phases shell out to bare `zbrain` so the canonical shim
    // on PATH wins. This is the shape v0_12_0 has always used.
    expect(src).toContain("execSync('zbrain init --migrate-only'");
    expect(src).toContain("execSync('zbrain extract links --source db --include-frontmatter'");
    expect(src).toContain("execSync('zbrain call get_stats'");
  });
