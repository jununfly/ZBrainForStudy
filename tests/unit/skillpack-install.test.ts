...
function scratchZbrain(): { zbrainRoot: string; skillsDir: string } {
  const root = mkdtempSync(join(tmpdir(), 'skillpack-zbrain-'));
  created.push(root);
  ...
  return { zbrainRoot: root, skillsDir };
}
...
describe('findZbrainRoot', () => {
  it('walks up to find openclaw.plugin.json + src/cli.ts', () => {
    const { zbrainRoot } = scratchZbrain();
    const nested = join(zbrainRoot, 'a', 'b', 'c');
    mkdirSync(nested, { recursive: true });
    expect(findZbrainRoot(nested)).toBe(zbrainRoot);
  });
  it('returns null when no zbrain root above', () => {
    expect(findZbrainRoot('/tmp/definitely-not-a-zbrain-repo-XYZ')).toBeNull();
  });
});
...
describe('loadBundleManifest', () => {
  it('loads + validates a valid manifest', () => {
    const { zbrainRoot } = scratchZbrain();
    const m = loadBundleManifest(zbrainRoot);
  ...
  it('throws BundleError on malformed JSON', () => {
    const { zbrainRoot } = scratchZbrain();
    writeFileSync(join(zbrainRoot, 'openclaw.plugin.json'), '{ nope');
    expect(() => loadBundleManifest(zbrainRoot)).toThrow(BundleError);
  });
});
...
describe('enumerateBundle (D-CX-10 dependency closure)', () => {
  it('includes skill files AND shared_deps for a single-skill install', () => {
    const { zbrainRoot } = scratchZbrain();
    const m = loadBundleManifest(zbrainRoot);
    const entries = enumerateBundle({ zbrainRoot, skillSlug: 'alpha', manifest: m });
  ...
  it('throws BundleError for unknown skill slug', () => {
    ...
    const m = loadBundleManifest(zbrainRoot);
    expect(() =>
      enumerateBundle({ zbrainRoot, skillSlug: 'nope', manifest: m }),
    ).toThrow(BundleError);
  });
  it('enumerates everything when skillSlug is undefined (--all)', () => {
    const { zbrainRoot } = scratchZbrain();
    const m = loadBundleManifest(zbrainRoot);
    const entries = enumerateBundle({ zbrainRoot, manifest: m });
    const targets = entries.map(e => e.relTarget).sort();
...
describe('buildManagedBlock + updateManagedBlock', () => {
  it('builds a block with all installed slugs as rows', () => {
    const m = loadBundleManifest(scratchZbrain().zbrainRoot);
    const block = buildManagedBlock(m, ['alpha', 'beta']);
    expect(block).toContain('zbrain:skillpack:begin');
    expect(block).toContain('zbrain:skillpack:end');
    expect(block).toContain('`skills/alpha/SKILL.md`');
    expect(block).toContain('`skills/beta/SKILL.md`');
  });
  it('appends block when none exists', () => {
    const block = buildManagedBlock(loadBundleManifest(scratchZbrain().zbrainRoot), ['alpha']);
    const updated = updateManagedBlock('# AGENTS\n\nSome prose.\n', block);
    expect(updated).toContain('zbrain:skillpack:begin');
...
describe('planInstall + applyInstall', () => {
  ...
  it('D-CX-11: concurrent install attempt fails with lock_held', () => {
    ...
    writeFileSync(join(workspace, '.zbrain-skillpack.lock'), '99999');
    const opts = {
      zbrainRoot,
      targetWorkspace: workspace,
      targetSkillsDir: skillsDir,
  ...
  it('D-CX-11: --force-unlock works when lock mtime is sub-ms ahead of Date.now (Linux fs jitter)', () => {
    ...
    const { zbrainRoot } = scratchZbrain();
    const { workspace, skillsDir } = scratchTarget();
    const lockFile = join(workspace, '.zbrain-skillpack.lock');
  ...
  it('managed block is written atomically (tmp then rename)', () => {
    ...
    expect(resolver).toContain('zbrain:skillpack:begin');
    expect(resolver).toContain('zbrain:skillpack:end');
...
describe('managed-block receipt + cumulative semantics (v0.19)', () => {
  ...
  it('install alpha, then install beta (separately) → both rows survive AND receipt lists both', () => {
    ...
    const alphaOpts = { zbrainRoot, targetWorkspace: workspace, targetSkillsDir: skillsDir, skillSlug: 'alpha' };
    applyInstall(planInstall(alphaOpts), alphaOpts);

    const betaOpts = { zbrainRoot, targetWorkspace: workspace, targetSkillsDir: skillsDir, skillSlug: 'beta' };
    applyInstall(planInstall(betaOpts), betaOpts);
  ...
  it('install --all then remove a slug from bundle and re-install --all → removed slug pruned silently', () => {
    ...
    const allOpts = { zbrainRoot, targetWorkspace: workspace, targetSkillsDir: skillsDir, skillSlug: null };
    ...
    const pluginManifest = JSON.parse(
      readFileSync(join(zbrainRoot, 'openclaw.plugin.json'), 'utf-8'),
    );
    ...
    try {
      const allOpts2 = { zbrainRoot, targetWorkspace: workspace, targetSkillsDir: skillsDir, skillSlug: null };
  ...
  it('user hand-adds an unknown row → preserved on reinstall AND stderr warning fires', () => {
    ...
    // unambiguously within zbrain's managed block.
    const path = join(skillsDir, 'RESOLVER.md');
    const orig = readFileSync(path, 'utf-8');
    const endMarker = '<!-- zbrain:skillpack:end -->';
   ...
   * zbrain-written before the receipt feature existed). Receipt is
  ...
  it('pre-v0.19 fence (no receipt) → clean rebuild, receipt now present, no warnings', () => {
    ...
    writeFileSync(
      ...
      [
        ...
        '',
        '<!-- Installed by zbrain 0.18.2 — do not hand-edit between markers. -->',
        '',
        '| Trigger | Skill |',
        '|---------|-------|',
        '| "alpha" | `skills/alpha/SKILL.md` |',
    ...
    try {
      const opts = { zbrainRoot, targetWorkspace: workspace, targetSkillsDir: skillsDir, skillSlug: 'beta' };
      applyInstall(planInstall(opts), opts);
    } finally {
      console.error = origErr;
    }
...
describe('diffSkill', () => {
  it('reports missing files', () => {
    ...
    const diffs = diffSkill(zbrainRoot, 'alpha', skillsDir);
