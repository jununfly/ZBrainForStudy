...
describe('file migration lifecycle', () => {
  ...
  beforeAll(() => {
    brainDir = mkdtempSync(join(tmpdir(), 'zbrain-migration-'));
    storageDir = mkdtempSync(join(tmpdir(), 'zbrain-migration-storage-'));
  ...
  test('REDIRECT: replace files with breadcrumbs', async () => {
    ...
    writeFileSync(join(brainDir, 'raw', '.supabase'), stringify({
      synced_at: new Date().toISOString(), bucket: 'tests/unit', prefix: 'raw/', file_count: 2,
    }));
    ...
    for (const f of ['raw/photo.jpg', 'raw/doc.pdf']) {
      ...
      const breadcrumb = stringify({
        moved_to: 'storage', bucket: 'tests/unit', path: f,
