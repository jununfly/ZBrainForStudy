 ...
 *
 * Hermetic via `withEnv` for `ZBRAIN_AUDIT_DIR` override; tmpdir per
...
function tmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'zbrain-audit-writer-test-'));
}
...
describe('resolveAuditDir', () => {
  it('honors ZBRAIN_AUDIT_DIR override', async () => {
    const dir = makeDir();
    await withEnv({ ZBRAIN_AUDIT_DIR: dir }, async () => {
      expect(resolveAuditDir()).toBe(dir);
    });
  });

  it('falls back to zbrainPath("audit") when override is unset', async () => {
    await withEnv({ ZBRAIN_AUDIT_DIR: undefined }, async () => {
  ...
  it('treats whitespace-only override as unset', async () => {
    await withEnv({ ZBRAIN_AUDIT_DIR: '   ' }, async () => {
      const resolved = resolveAuditDir();
...
describe('createAuditWriter — log()', () => {
  ...
  it('best-effort: write failure stderr-warns but does not throw', async () => {
    ...
    // file at `${root}/blocker`, then point ZBRAIN_AUDIT_DIR at
    ...
    try {
      await withEnv({ ZBRAIN_AUDIT_DIR: badDir }, async () => {
