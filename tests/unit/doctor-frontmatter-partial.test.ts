 ...
 * layers: (a) `tests/unit/brain-writer-partial-scan.test.ts` exercises the
 * scanBrainSources + deadline contract that doctor depends on, and
 * (b) `tests/heavy/frontmatter_scan_wallclock.sh` (manual / nightly)
 * subprocesses real `zbrain doctor` against a synthesized 60K-file brain.
 *
...
describe('doctor frontmatter_integrity — structural rendering (source-grep)', () => {
  test('source contains ZBRAIN_DOCTOR_FM_TIMEOUT_MS handling', () => {
    expect(DOCTOR_SOURCE).toContain('ZBRAIN_DOCTOR_FM_TIMEOUT_MS');
  });
  ...
  test('source renders NOT SCANNED per skipped source with remediation hint', () => {
    ...
    expect(DOCTOR_SOURCE).toContain('zbrain frontmatter validate');
  });
...
describe('doctor frontmatter_integrity — load-bearing render strings', () => {
  ...
  test('source uses fallback hint pointing at ZBRAIN_DOCTOR_FM_TIMEOUT_MS on partial', () => {
    // The fix hint when partial: raise the timeout OR run validate directly.
    const partialHintMatch = DOCTOR_SOURCE.includes('Raise ZBRAIN_DOCTOR_FM_TIMEOUT_MS');
