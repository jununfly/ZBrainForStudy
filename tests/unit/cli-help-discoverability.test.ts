 ...
 *
 * WARN-5: `zbrain capture --help` was showing only the generic
 * `Usage: zbrain capture` line because `capture` was missing from
 ...
 * `zbrain --help` text. Added a BRAIN section to printHelp.
 *
...
function runCli(args: string[]): { stdout: string; stderr: string; status: number } {
  const result = spawnSync('bun', ['run', 'src/cli.ts', ...args], {
    cwd: process.cwd(),
    encoding: 'utf8',
    env: { ...process.env, ZBRAIN_HOME: '/tmp/zbrain-test-help-nonexistent' },
  });
  return {
...
describe('WARN-5 — `zbrain capture --help` reaches the detailed HELP constant', () => {
  ...
  test('output is NOT the generic short-circuit fallback', () => {
    ...
    // Pre-fix output was: "Usage: zbrain capture\n\ngbrain capture - run zbrain --help ..."
    // Post-fix HELP is much longer and includes Examples.
    expect(stdout).toContain('Examples:');
    expect(stdout.split('\n').length).toBeGreaterThan(10);
    expect(stdout).not.toMatch(/^Usage: zbrain capture\s*$/m);
  });
...
describe('WARN-6 — main `zbrain --help` lists capture/brainstorm/lsd', () => {
