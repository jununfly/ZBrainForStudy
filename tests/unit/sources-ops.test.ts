 ...
 * coverage lives in tests/unit/e2e/sources-remote-mcp.test.ts.
 */
...
const FAKE_GIT_DIR = join(tmpdir(), `zbrain-sources-ops-test-${process.pid}`);
const ZBRAIN_HOME = join(FAKE_GIT_DIR, 'zbrain-home');
// zbrainPath() appends `.zbrain` to ZBRAIN_HOME, so the actual clone root the
// production code resolves to is $ZBRAIN_HOME/.zbrain/clones/. Tests that
// hand-craft path fixtures must use this, NOT $ZBRAIN_HOME/clones/.
const CLONE_ROOT = join(ZBRAIN_HOME, '.zbrain', 'clones');
...
describe('removeSource — clone-cleanup', () => {
  test('removes clone IFF managed (local_path under $ZBRAIN_HOME/clones/ + remote_url set)', async () => {
    await withEnv2(async () => {
      const row = await addSource(engine, {
        id: 'cleanup-yes',
        remoteUrl: 'https://github.com/example/repo',
  ...
  test('does NOT remove clone for user-supplied --path (no remote_url)', async () => {
    await withEnv2(async () => {
      const userPath = join(ZBRAIN_HOME, 'user-managed-fixture');
  ...
  test('symlink-target-OUTSIDE-clones: realpath confinement foils escape', async () => {
    await withEnv2(async () => {
      ...
      const target = join(ZBRAIN_HOME, 'sensitive-fixture');
...
describe('getSourceStatus', () => {
  ...
  test('clone_state = "not-applicable" for path-only source (no remote)', async () => {
    await withEnv2(async () => {
      const userPath = join(ZBRAIN_HOME, 'na-fixture');
      mkdirSync(userPath, { recursive: true });
...
describe('isPathContained', () => {
  // Use a sandbox dir, not ZBRAIN_HOME (which has the .zbrain quirk).
  const SANDBOX = join(tmpdir(), `zbrain-isPathContained-${process.pid}`);
  beforeEach(() => {
