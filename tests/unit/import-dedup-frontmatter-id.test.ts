 ...
 *
 * Bug class (infiniteGameExp): `zbrain import /vault/Subdir/` then
 * `zbrain import /vault/` re-ingested the same files under different
 ...
 * is at tests/unit/e2e/import-dedup-postgres.test.ts.
 */
...
beforeEach(async () => {
  ...
  tmpRoot = mkdtempSync(join(tmpdir(), 'zbrain-dedup-fm-'));
});
...
describe('#1309 — overlapping-ingest-roots dedup', () => {
  ...
  test('different frontmatter.id (and therefore different content_hash) imports both, no dedup signal', async () => {
    // zbrain's content_hash includes the frontmatter (minus captured_at /
