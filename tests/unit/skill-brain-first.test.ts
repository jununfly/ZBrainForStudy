 ...
 * /plan-eng-review).
 *
 * Drives the fixture corpus at `tests/unit/fixtures/brain-first-skills/*`.
 ...
 * snapshot tests redirect `ZBRAIN_AUDIT_DIR` to a tempdir.
 */
...
describe('offset helpers', () => {
  test('findFirstBrainRefOffset finds earliest zbrain ref', () => {
    const body = 'Some preamble. zbrain search and later zbrain query.';
    const o = findFirstBrainRefOffset(body);
    expect(o).toBe(body.indexOf('zbrain search'));
  });
...
describe('FORMERLY_HARDCODED_EXEMPT', () => {
  test('preserves all 40+ entries from PR #1206 allowlist', () => {
    ...
    expect(FORMERLY_HARDCODED_EXEMPT.has('zbrain')).toBe(true);
...
describe('analyzeSkillBrainFirst (PR #1206 regression preservation)', () => {
  ...
  test('PR test case: skill with brain search reference is not flagged', () => {
    const content = `---\nname: bf\n---\n# Brain-First\n\nFirst, search the brain for existing context.\nThen use web_search for anything missing. Use zbrain search.\n`;
