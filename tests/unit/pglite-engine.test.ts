...
describe('PGLiteEngine: Pages', () => {
  ...
  test('putPage + getPage round trip', async () => {
    const page = await engine.putPage('tests/unit/hello', testPage);
    expect(page.slug).toBe('tests/unit/hello');
    expect(page.title).toBe('Test Page');
    expect(page.type).toBe('concept');
    expect(page.compiled_truth).toContain('NovaMind');

    const fetched = await engine.getPage('tests/unit/hello');
  ...
  test('putPage upserts on conflict', async () => {
    await engine.putPage('tests/unit/upsert', testPage);
    const updated = await engine.putPage('tests/unit/upsert', {
      ...testPage,
      title: 'Updated Title',
    });
    ...
    const matches = all.filter(p => p.slug === 'tests/unit/upsert');
  ...
  test('deletePage removes page', async () => {
    await engine.putPage('tests/unit/delete-me', testPage);
    await engine.deletePage('tests/unit/delete-me');
    const result = await engine.getPage('tests/unit/delete-me');
    expect(result).toBeNull();
  });
  ...
  test('listPages with tag filter', async () => {
    await engine.putPage('tests/unit/tagged', testPage);
    await engine.addTag('tests/unit/tagged', 'special');

    const tagged = await engine.listPages({ tag: 'special' });
    expect(tagged.length).toBe(1);
    expect(tagged[0].slug).toBe('tests/unit/tagged');
  });
  ...
  test('resolveSlugs exact match', async () => {
    await engine.putPage('tests/unit/exact', testPage);
    const slugs = await engine.resolveSlugs('tests/unit/exact');
    expect(slugs).toEqual(['tests/unit/exact']);
  });
  ...
  test('updateSlug renames page', async () => {
    await engine.putPage('tests/unit/old-name', testPage);
    await engine.updateSlug('tests/unit/old-name', 'tests/unit/new-name');
    expect(await engine.getPage('tests/unit/old-name')).toBeNull();
    expect((await engine.getPage('tests/unit/new-name'))?.title).toBe('Test Page');
  });
  ...
  test('validateSlug normalizes to lowercase', async () => {
    ...
    expect(page.slug).toBe('tests/unit/upper');
  });
});
...
describe('PGLiteEngine: Chunks', () => {
  ...
  test('upsertChunks + getChunks round trip', async () => {
    await engine.putPage('tests/unit/chunks', testPage);
    await engine.upsertChunks('tests/unit/chunks', [
    ...
    const chunks = await engine.getChunks('tests/unit/chunks');
  ...
  test('upsertChunks removes orphan chunks', async () => {
    await engine.putPage('tests/unit/orphan', testPage);
    await engine.upsertChunks('tests/unit/orphan', [
    ...
    const chunks = await engine.getChunks('tests/unit/orphan');
  ...
  test('upsertChunks throws for missing page', async () => {
    await expect(
      engine.upsertChunks('nonexistent/page', [
        { chunk_index: 0, chunk_text: 'tests/unit', chunk_source: 'compiled_truth' },
      ])
    ).rejects.toThrow('Page not found');
  });

  test('deleteChunks removes all chunks for page', async () => {
    await engine.putPage('tests/unit/delete-chunks', testPage);
    await engine.upsertChunks('tests/unit/delete-chunks', [
      { chunk_index: 0, chunk_text: 'Gone', chunk_source: 'compiled_truth' },
    ]);
    await engine.deleteChunks('tests/unit/delete-chunks');
    const chunks = await engine.getChunks('tests/unit/delete-chunks');
    expect(chunks.length).toBe(0);
  });

  test('getChunksWithEmbeddings returns embedding data', async () => {
    await engine.putPage('tests/unit/embed', testPage);
    const embedding = new Float32Array(1536).fill(0.1);
    await engine.upsertChunks('tests/unit/embed', [
      { chunk_index: 0, chunk_text: 'With embedding', chunk_source: 'compiled_truth', embedding },
    ]);
    const chunks = await engine.getChunksWithEmbeddings('tests/unit/embed');
    expect(chunks.length).toBe(1);
...
// PGLite parity for the Postgres E2E in tests/unit/e2e/embed-stale-pagination.
...
describe('PGLiteEngine: stale chunk pagination (D7 + REGRESSION)', () => {
  ...
  test('countStaleChunks counts chunks with NULL embedding only', async () => {
    await engine.putPage('tests/unit/stale-a', testPage);
    await engine.upsertChunks('tests/unit/stale-a', [
  ...
  test('listStaleChunks: cursor pagination across page boundaries', async () => {
    ...
    for (const slug of ['c-a', 'c-b', 'c-c']) {
      await engine.putPage(`tests/unit/${slug}`, testPage);
      await engine.upsertChunks(`tests/unit/${slug}`, [
  ...
  test('listStaleChunks: page split across batches (1 page, 5 chunks, batchSize=2)', async () => {
    await engine.putPage('tests/unit/split', testPage);
    await engine.upsertChunks('tests/unit/split', [
      { chunk_index: 0, chunk_text: 'a', chunk_source: 'compiled_truth' },
      { chunk_index: 1, chunk_text: 'b', chunk_source: 'compiled_truth' },
      { chunk_index: 2, chunk_text: 'c', chunk_source: 'compiled_truth' },
  ...
  test('countStaleChunks + listStaleChunks honor sourceId filter (D7)', async () => {
    ...
    await engine.putPage('tests/unit/shared', testPage, { sourceId: 'default' });
    await engine.upsertChunks('tests/unit/shared', [
    ...
    await engine.putPage('tests/unit/shared', testPage, { sourceId: 'other' });
    await engine.upsertChunks('tests/unit/shared', [
...
describe('PGLiteEngine: Tags', () => {
  beforeEach(async () => {
    await truncateAll();
    await engine.putPage('tests/unit/tags', testPage);
  });

  test('addTag + getTags', async () => {
    await engine.addTag('tests/unit/tags', 'alpha');
    await engine.addTag('tests/unit/tags', 'beta');
    const tags = await engine.getTags('tests/unit/tags');
    expect(tags).toEqual(['alpha', 'beta']);
  });

  test('removeTag', async () => {
    await engine.addTag('tests/unit/tags', 'remove-me');
    await engine.removeTag('tests/unit/tags', 'remove-me');
    const tags = await engine.getTags('tests/unit/tags');
    expect(tags).not.toContain('remove-me');
  });

  test('duplicate tag is idempotent', async () => {
    await engine.addTag('tests/unit/tags', 'dup');
    await engine.addTag('tests/unit/tags', 'dup');
    const tags = await engine.getTags('tests/unit/tags');
...
describe('PGLiteEngine: Timeline', () => {
  beforeEach(async () => {
    await truncateAll();
    await engine.putPage('tests/unit/timeline', testPage);
  });

  test('addTimelineEntry + getTimeline', async () => {
    await engine.addTimelineEntry('tests/unit/timeline', {
      date: '2024-01-15', summary: 'Founded', detail: 'Company founded',
    });
    const entries = await engine.getTimeline('tests/unit/timeline');
    expect(entries.length).toBe(1);
    expect(entries[0].summary).toBe('Founded');
  });

  test('getTimeline with date range', async () => {
    await engine.addTimelineEntry('tests/unit/timeline', { date: '2024-01-01', summary: 'Jan' });
    await engine.addTimelineEntry('tests/unit/timeline', { date: '2024-06-01', summary: 'Jun' });
    await engine.addTimelineEntry('tests/unit/timeline', { date: '2024-12-01', summary: 'Dec' });

    const filtered = await engine.getTimeline('tests/unit/timeline', {
...
describe('PGLiteEngine: RawData', () => {
  beforeEach(async () => {
    await truncateAll();
    await engine.putPage('tests/unit/raw', testPage);
  });

  test('putRawData + getRawData', async () => {
    await engine.putRawData('tests/unit/raw', 'crunchbase', { funding: '$10M' });
    const data = await engine.getRawData('tests/unit/raw', 'crunchbase');
...
describe('PGLiteEngine: Versions', () => {
  beforeEach(async () => {
    await truncateAll();
    await engine.putPage('tests/unit/version', testPage);
  });

  test('createVersion + getVersions', async () => {
    const v = await engine.createVersion('tests/unit/version');
    expect(v.compiled_truth).toBe(testPage.compiled_truth);

    const versions = await engine.getVersions('tests/unit/version');
    expect(versions.length).toBe(1);
  });

  test('revertToVersion restores content', async () => {
    await engine.createVersion('tests/unit/version');
    await engine.putPage('tests/unit/version', { ...testPage, compiled_truth: 'Changed' });

    const versions = await engine.getVersions('tests/unit/version');
    await engine.revertToVersion('tests/unit/version', versions[0].id);

    const page = await engine.getPage('tests/unit/version');
...
describe('PGLiteEngine: IngestLog', () => {
  test('logIngest + getIngestLog', async () => {
    await engine.logIngest({
      ...
      pages_updated: ['tests/unit/a', 'tests/unit/b'], summary: 'Imported 2 pages',
    });
...
describe('PGLiteEngine: Stats & Health', () => {
  beforeAll(async () => {
    await truncateAll();
    await engine.putPage('tests/unit/stats', testPage);
    await engine.upsertChunks('tests/unit/stats', [
      { chunk_index: 0, chunk_text: 'chunk', chunk_source: 'compiled_truth' },
    ]);
    await engine.addTag('tests/unit/stats', 'stat-tag');
  });
...
describe('PGLiteEngine: Transactions', () => {
  ...
  test('transaction commits on success', async () => {
    await engine.transaction(async (tx) => {
      await tx.putPage('tests/unit/tx-ok', testPage);
    });
    const page = await engine.getPage('tests/unit/tx-ok');
  ...
  test('transaction rolls back on error', async () => {
    try {
      await engine.transaction(async (tx) => {
        await tx.putPage('tests/unit/tx-fail', testPage);
        throw new Error('Deliberate rollback');
      });
    } catch { /* expected */ }

    const page = await engine.getPage('tests/unit/tx-fail');
...
describe('PGLiteEngine: Cascade deletes', () => {
  test('deleting a page cascades to chunks, tags, links', async () => {
    await engine.putPage('tests/unit/cascade', testPage);
    await engine.upsertChunks('tests/unit/cascade', [
      { chunk_index: 0, chunk_text: 'cascade chunk', chunk_source: 'compiled_truth' },
    ]);
    await engine.addTag('tests/unit/cascade', 'cascade-tag');

    await engine.deletePage('tests/unit/cascade');

    const chunks = await engine.getChunks('tests/unit/cascade');
    expect(chunks.length).toBe(0);
    const tags = await engine.getTags('tests/unit/cascade');
...
describe('PGLiteEngine: listPages updated_after filter', () => {
  ...
  test('filters pages by updated_at > given date', async () => {
    await engine.putPage('tests/unit/old', testPage);
    // Sleep briefly so the second page has a strictly later updated_at.
    await new Promise(r => setTimeout(r, 10));
    const cutoff = new Date().toISOString();
    await new Promise(r => setTimeout(r, 10));
    await engine.putPage('tests/unit/new', testPage);

    const recent = await engine.listPages({ updated_after: cutoff, limit: 100 });
    const recentSlugs = recent.map(p => p.slug);
    expect(recentSlugs).toContain('tests/unit/new');
    expect(recentSlugs).not.toContain('tests/unit/old');
  });

  test('without updated_after, returns all pages (regression)', async () => {
    await engine.putPage('tests/unit/a', testPage);
    await engine.putPage('tests/unit/b', testPage);
...
describe('PGLiteEngine: Timeline dedup constraint (v6 migration)', () => {
  beforeEach(async () => {
    await truncateAll();
    await engine.putPage('tests/unit/timeline-dedup', testPage);
  });

  test('inserting same (date, summary) twice is silent no-op (idempotent)', async () => {
    await engine.addTimelineEntry('tests/unit/timeline-dedup', { date: '2026-01-15', summary: 'Event A' });
    await engine.addTimelineEntry('tests/unit/timeline-dedup', { date: '2026-01-15', summary: 'Event A' });
    const entries = await engine.getTimeline('tests/unit/timeline-dedup');
    expect(entries.length).toBe(1);
  });

  test('different summary on same date: both inserted', async () => {
    await engine.addTimelineEntry('tests/unit/timeline-dedup', { date: '2026-01-15', summary: 'Morning' });
    await engine.addTimelineEntry('tests/unit/timeline-dedup', { date: '2026-01-15', summary: 'Evening' });
    const entries = await engine.getTimeline('tests/unit/timeline-dedup');
...
describe('PGLiteEngine: v0.13.1 error-wrap on connect() (#223)', () => {
  test('pglite-engine.ts source contains the wrap with #223 hint and nested original error', async () => {
    ...
    // issue and suggest zbrain doctor. Must NOT suggest "missing migrations"
    // as a cause (that was conflating #218 and #223 — migrations run AFTER
    // create()).
    expect(src).toContain('this._db = await PGlite.create');
    expect(src).toContain('https://github.com/garrytan/zbrain/issues/223');
    expect(src).toContain('zbrain doctor');
