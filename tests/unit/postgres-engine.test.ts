 ...
 *
 * Live Postgres coverage for search paths lives in tests/unit/e2e/search-quality.test.ts.
...
describe('postgres-engine / search path timeout isolation', () => {
  ...
  test('connect() with poolSize honors resolvePrepare (PgBouncer regression guard)', () => {
    ...
    // PostgresEngine.connect({poolSize}) (the branch used by `zbrain jobs work`)
