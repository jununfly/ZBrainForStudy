 ...
 * (`zbrain-cycle:<source_id>`) would by themselves let two PGLite cycles
 * for different sources run concurrently — which would corrupt the
 * single-writer invariant.
 *
 * Defense: cycle.ts acquires the GLOBAL file lock (`~/.zbrain/cycle.lock`,
...
beforeAll(async () => {
  // ZBRAIN_HOME isolation so the file lock at ~/.zbrain/cycle.lock doesn't
  // collide with the dev's real zbrain. resetPgliteState would wipe the
  // config table so we don't use it here.
  zbrainHome = mkdtempSync(join(tmpdir(), 'zbrain-pglite-lock-'));
  process.env.ZBRAIN_HOME = zbrainHome;
...
beforeEach(async () => {
  ...
  const lockPath = join(zbrainHome, '.zbrain', 'cycle.lock');
  if (existsSync(lockPath)) {
    try { unlinkSync(lockPath); } catch { /* best-effort */ }
  }
  brainDir = mkdtempSync(join(tmpdir(), 'zbrain-cycle-pglite-ord-'));
});
...
describe('PGLite cycle: file lock + per-source DB lock ordering', () => {
  test('global file lock acquired during PGLite cycle (codex P0-C invariant)', async () => {
    ...
    await runCycle(engine, {
      ...
      yieldBetweenPhases: async () => {
        ...
        if (!lockFileExisted) {
          lockFileExisted = existsSync(join(zbrainHome, '.zbrain', 'cycle.lock'));
        }
      },
    });
  ...
  test('two PGLite cycles for DIFFERENT sources serialize (P0-D regression)', async () => {
    ...
    writeFileSync(
      join(zbrainHome, '.zbrain', 'cycle.lock'),
  ...
  test('DB lock row uses per-source ID even though file lock is global', async () => {
    ...
    expect(dbLockRowSeen!.id).toBe('zbrain-cycle:epsilon');
  });
