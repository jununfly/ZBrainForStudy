 ...
 *   - back-compat for undefined sourceId (legacy 'zbrain-cycle')
 *   - per-source lock IDs (`zbrain-cycle:<id>`)
 ...
 * tests/unit/e2e/cycle-lock-integration.test.ts so they can spin a real engine.
 */
import { describe, test, expect } from 'bun:test';
import { cycleLockIdFor } from '../../src/core/cycle.ts';

describe('cycleLockIdFor', () => {
  test('returns legacy zbrain-cycle for undefined sourceId (back-compat)', () => {
    expect(cycleLockIdFor()).toBe('zbrain-cycle');
    expect(cycleLockIdFor(undefined)).toBe('zbrain-cycle');
  });

  test('returns zbrain-cycle:<source_id> for valid kebab IDs', () => {
    expect(cycleLockIdFor('default')).toBe('zbrain-cycle:default');
    expect(cycleLockIdFor('portfolio')).toBe('zbrain-cycle:portfolio');
    expect(cycleLockIdFor('a')).toBe('zbrain-cycle:a');
    expect(cycleLockIdFor('alpha-beta-gamma')).toBe('zbrain-cycle:alpha-beta-gamma');
  });
  ...
  test('produces DISTINCT lock IDs for different sources', () => {
    ...
    expect(a).not.toBe(b);
    expect(a).not.toBe('zbrain-cycle');
    expect(b).not.toBe('zbrain-cycle');
  });
