...
async function withFreshAuditDir(body: (tmpDir: string) => void | Promise<void>): Promise<void> {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zbrain-rerank-audit-'));
  try {
    await withEnv({ ZBRAIN_AUDIT_DIR: tmpDir }, async () => {
      await body(tmpDir);
    });
  } finally {
...
describe('rerank-audit JSONL round-trip', () => {
  ...
  test('missing audit dir → readRecentRerankFailures returns []', async () => {
    ...
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'zbrain-rerank-empty-'));
    try {
      await withEnv({ ZBRAIN_AUDIT_DIR: path.join(tmpRoot, 'nonexistent') }, async () => {
        const events = readRecentRerankFailures(7);
