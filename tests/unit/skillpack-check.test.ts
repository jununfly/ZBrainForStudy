/**
 * Tests for `zbrain skillpack-check` — the agent-readable health report.
 *
 * Covers:
 *   - Healthy fresh install → exit 0, healthy:true, actions:[], no DB needed.
 *   - Half-migrated (partial entry in completed.jsonl) → exit 1,
 *     healthy:false, actions includes `zbrain apply-migrations --yes`,
 ...
 * state. DATABASE_URL / ZBRAIN_DATABASE_URL stripped so the report runs
...
beforeEach(() => {
  ...
  tmp = mkdtempSync(join(tmpdir(), 'zbrain-skillpack-check-test-'));
});
...
describe('zbrain skillpack-check', () => {
  test('healthy fresh install → exit 0, healthy:true, empty actions', () => {
    ...
    expect(report.summary).toBe('zbrain skillpack healthy');
  ...
  test('half-migrated (partial completed.jsonl) → exit 1, apply-migrations in actions', () => {
    ...
    expect(report.actions).toContain('zbrain apply-migrations --yes');
    expect(report.summary).toContain('zbrain apply-migrations --yes');
