 ...
 *   Full HTTP-transport coverage lives in tests/unit/e2e/serve-http-oauth.test.ts.
 */
...
const FAKE_GIT_DIR = join(tmpdir(), `zbrain-sources-mcp-test-${process.pid}`);
...
describe('sources_* handlers — happy path', () => {
  test('sources_add (with --url) clones, INSERTs, returns row', async () => {
    await withEnv({ ZBRAIN_HOME, PATH: fakePath() }, async () => {
...
// bearer auth + middleware) lives in tests/unit/e2e/serve-http-oauth.test.ts.
// ---------------------------------------------------------------------------
...
describe('sources_* scope enforcement (simulates serve-http gate)', () => {
  ...
  test('gstack /setup-zbrain Path 4 token (read + sources_admin) covers everything', () => {
...
describe('sources_add — remote callers ignore path/clone_dir overrides', () => {
  test('remote sources_admin: clone_dir override is silently ignored', async () => {
    await withEnv({ ZBRAIN_HOME, PATH: fakePath() }, async () => {
      ...
      const row = (await op.handler(ctx, {
        id: 'attack-clone-dir',
        url: 'https://github.com/example/repo',
        clone_dir: '/etc/zbrain-pwned',  // attacker-supplied
      })) as any;
      // Clone landed at the SAFE default, not /etc/zbrain-pwned.
      expect(row.local_path).not.toBe('/etc/zbrain-pwned');
      expect(row.local_path).toContain('clones/attack-clone-dir');
      // /etc/zbrain-pwned was never written.
      expect(existsSync('/etc/zbrain-pwned')).toBe(false);
    });
  });
  ...
  test('local CLI caller (ctx.remote=false) keeps clone_dir override', async () => {
    await withEnv({ ZBRAIN_HOME, PATH: fakePath() }, async () => {
      ...
      const customDir = join(ZBRAIN_HOME, 'custom-clones', 'local-override');
