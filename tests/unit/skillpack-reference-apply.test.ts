 ...
 *
 * Pins:
 *   - clean apply: user's local file gets zbrain's upstream changes
...
function scratchZbrain(): string {
  const root = mkdtempSync(join(tmpdir(), 'sp-refapply-zbrain-'));
  created.push(root);
...
describe('runReferenceApply — happy paths', () => {
  it('applies upstream zbrain changes to a file the user has not edited', () => {
    const zbrainRoot = scratchZbrain();
    const ws = scratchWorkspace();
    runScaffold({ zbrainRoot, targetWorkspace: ws, skillSlug: 'demo' });

    // zbrain ships a new version with line 15 updated. User has not
    // touched the file locally.
    const zbrainSkill = join(zbrainRoot, 'skills', 'demo', 'SKILL.md');
    writeFileSync(
      zbrainSkill,
      readFileSync(zbrainSkill, 'utf-8').replace('Line 15\n', 'Line 15 UPDATED\n'),
    );

    const userSkill = join(ws, 'skills', 'demo', 'SKILL.md');
    const result = runReferenceApply({ zbrainRoot, targetWorkspace: ws, skillSlug: 'demo' });
  ...
  it('two-way limitation: user edits in differing area DO get replaced by zbrain content', () => {
    // D15 contract: this is a TWO-WAY diff against zbrain's current
    // bundle. Without scaffold-time base tracking, we cannot tell
    // whether a difference came from zbrain or from the user. Applied
    // hunks therefore align everything to zbrain. The agent uses
    // --dry-run / reference (read-only) BEFORE applying to decide.
    const zbrainRoot = scratchZbrain();
    const ws = scratchWorkspace();
    runScaffold({ zbrainRoot, targetWorkspace: ws, skillSlug: 'demo' });

    // zbrain changes line 25. User changes line 5 (independent areas).
    const zbrainSkill = join(zbrainRoot, 'skills', 'demo', 'SKILL.md');
    writeFileSync(
      zbrainSkill,
      readFileSync(zbrainSkill, 'utf-8').replace('Line 25\n', 'Line 25 ZBRAIN\n'),
    );
    const userSkill = join(ws, 'skills', 'demo', 'SKILL.md');
    writeFileSync(
      userSkill,
    ...
    // zbrain's change lands…
    expect(readFileSync(userSkill, 'utf-8')).toContain('Line 25 ZBRAIN');
...
describe('runReferenceApply — applied-status surface', () => {
  it('applied_clean status set when every hunk lands without conflict', () => {
    // runReferenceApply uses just-in-time diff (user→zbrain), so its
...
describe('runReferenceApply — binary files', () => {
  it('binary files are reported binary_skip and not touched', () => {
    ...
    const binPath = join(zbrainRoot, 'skills', 'demo', 'icon.png');
...
describe('runReferenceApply — --all is not supported', () => {
  it('throws when called with skillSlug: null', () => {
    ...
    const ws = scratchWorkspace();
    expect(() =>
      runReferenceApply({ zbrainRoot, targetWorkspace: ws, skillSlug: null }),
