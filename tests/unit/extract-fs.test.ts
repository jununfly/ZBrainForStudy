/**
 * Tests for `zbrain extract --source fs` (the default, FS-walking path).
 *
 * Companion to tests/unit/extract-db.test.ts. Specifically guards against the
...
beforeEach(async () => {
  await truncateAll();
  brainDir = mkdtempSync(join(tmpdir(), 'zbrain-extract-fs-'));
}, 15_000);
...
describe('zbrain extract links --source fs', () => {
...
describe('zbrain extract timeline --source fs', () => {
...
describe('zbrain extract --dir default resolution', () => {
  ...
  test('uses configured sources(local_path) when --dir is not passed', async () => {
    ...
    const otherDir = mkdtempSync(join(tmpdir(), 'zbrain-extract-other-'));
  ...
  test('explicit --dir always wins over configured source', async () => {
    ...
    const decoyDir = mkdtempSync(join(tmpdir(), 'zbrain-extract-decoy-'));
    await (engine as any).db.exec(
