 ...
 * presented_secret` plaintext compare. zbrain stores SHA-256 hashes,
...
import { ZBrainOAuthProvider } from '../../src/core/oauth-provider.ts';
...
beforeAll(async () => {
  ...
  provider = new ZBrainOAuthProvider({ sql: sqlQueryForEngine(engine) });
});
...
describe('verifyConfidentialClientSecret (#1166)', () => {
  test('confidential client_secret_post: returns client on correct secret', async () => {
    const reg = await provider.registerClientManual('test-conf', ['authorization_code'], 'read write', ['https://example.tests/unit/cb']);
  ...
  test('wrong secret → throws "Invalid client" (RFC 6749 opaque error)', async () => {
    const reg = await provider.registerClientManual('test-conf', ['authorization_code'], 'read', ['https://example.tests/unit/cb']);
    await expect(
  ...
  test('case-insensitive secret? NO — must be exact match', async () => {
    const reg = await provider.registerClientManual('test-case', ['authorization_code'], 'read', ['https://example.tests/unit/cb']);
  ...
  test('soft-deleted client → throws "Client has been revoked"', async () => {
    const reg = await provider.registerClientManual('to-revoke', ['authorization_code'], 'read', ['https://example.tests/unit/cb']);
    await engine.executeRaw(
...
describe('confidential-client full flow #1166', () => {
  test('verify-then-exchange refresh token end-to-end', async () => {
    const reg = await provider.registerClientManual('full-flow-rt', ['authorization_code', 'refresh_token'], 'read', ['https://example.tests/unit/cb']);
