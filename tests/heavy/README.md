...
Heavy ops-shape tests. Shell scripts that exercise zbrain end-to-end against
...
Put a slow test in `tests/unit/` with the `.slow.test.ts` suffix if it:
...
Each script writes a per-run log to `~/.zbrain/audit/heavy-<script>-<ts>.log`
...
- Honor `$ZBRAIN_HOME` / `$TMP_ROOT` env overrides where relevant
