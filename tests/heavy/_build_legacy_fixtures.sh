...
#   1. Bringing a fresh DB to LATEST shape (via `zbrain doctor` which triggers initSchema)
...
# pattern matches what tests/unit/bootstrap.test.ts uses for PGLite.
#
# Usage:
...
# Step 2: bring to LATEST via zbrain. The CLI's `doctor --json` triggers
