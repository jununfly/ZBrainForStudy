...
<!-- source: https://raw.githubusercontent.com/garrytan/zbrain/master/docs/ZBRAIN_SKILLPACK.md -->
# ZBrain Skillpack: Reference Architecture for AI Agents

This is a reference architecture for how a production AI agent uses zbrain as its
...
stores everything, mechanized so it may be consulted with exceeding speed. ZBrain is
...
`skills/conventions/subagent-routing.md` — it reads `~/.zbrain/preferences.json#minion_mode`
...
auto-rewrites entries whose handler is a zbrain builtin; host-specific handlers (e.g.
...
| [Brain vs Agent Memory](guides/brain-vs-memory.md) | 3 layers: ZBrain (world knowledge), agent memory, session |
...
## Appendix: ZBrain CLI Quick Reference

| Command | Purpose |
|---------|---------|
| `zbrain search "term"` | Keyword search across all brain pages |
| `zbrain query "question"` | Hybrid search (vector + keyword + RRF) |
| `zbrain get <slug>` | Read a specific brain page by slug |
| `zbrain sync` | Sync local markdown repo to zbrain index |
| `zbrain import <path>` | Import files into the brain |
| `zbrain embed --stale` | Re-embed pages with stale or missing embeddings |
| `zbrain integrations` | Manage integration recipes (senses + reflexes) |
| `zbrain stats` | Show brain statistics (page count, last sync, etc.) |
| `zbrain doctor` | Diagnose brain health issues |
| `zbrain check-update` | Check for new versions and integration recipes |

Run `zbrain --help` for the full command reference.
...
- [Recommended Schema](ZBRAIN_RECOMMENDED_SCHEMA.md) — Directory structure for your brain repo
- [Verification Runbook](ZBRAIN_VERIFY.md) — End-to-end installation verification
