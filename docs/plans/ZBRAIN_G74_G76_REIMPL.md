...
> 数据文件: `zbrain-g74-g76-reimpl.json` | 最后更新: 2026-08-09 16:54:25
...
│   └── [~][X+] 1-1-5. 真 LLM 的 5 个 eval 命令（cross-modal / longmemeval / takes-quality / suspected-contradictions / brainstorm）
    ...
    ├── [ ][X+] 1-1-5-2. eval-longmemeval（#60，替换 G58 占位）
    ├── [ ][X+] 1-1-5-3. eval-takes-quality（#61，新建 EvalTakesQuality variant）
    ├── [ ][X+] 1-1-5-4. eval-suspected-contradictions（#62，最大一族 judge×153）
    └── [ ][X+] 1-1-5-5. eval-brainstorm（#63，前置 port 3 引擎方法）
...
### 当前施工：1-1-5 真 LLM 5 命令启动（cross-modal #59 已收口 → 剩 4 族待启）
...
**阶段8 / 1-1-5 启动（2026-08-09）：#59 eval-cross-modal 全保真 port 完成。** TS 源 eval-cross-modal.ts(849行) 逐文件搬到 `crates/zbrain-core/src/eval/cross_modal.rs`（run_eval 函数式注入 Fn(ChatRequest)->Fut 闭包使 harness hermetic 可单测；4 策略 parse_model_json：stripFences→首{..末}切片→repair_json→nuclear；default_dimensions/default_slots；infer_slug_from_skill_path/output_path；estimate_cost；RECEIPT_SCHEMA_VERSION）。CLI 侧 `EvalCrossModalArgs` + `run_eval_cross_modal_command` 经 `resolve_recipe_strict`+`instantiate_chat` 接真实 `Arc<dyn ChatProvider>`，无 key 诚实降级 EXIT1 不写伪收据；`RENAMED_EVAL_SUBCOMMANDS` 把 `cross-modal` 重定向到 `eval-cross-modal`。校验：cargo check -p zbrain-core/-p zbrain-cli 绿；core cross_modal 单测 21 passed；cli lib 325 passed；core lib 全量 2439 passed；clippy regex-in-loop=0；e2e 空 brain 诚实降级全绿（缺 flag/output 不存在/空文件 EXIT1 清晰报错；无 key 报 Missing API key EXIT1 不落收据）。修复 1 个真移植 bug：repair_json 原 str::replace 字面替换未用正则→尾逗号未修，改 Regex::replace_all（单引号修复用捕获边界+固定点有界循环，因 regex crate 无 lookbehind/lookahead）。剩余 4 族（#60 longmemeval / #61 takes-quality / #62 suspected-contradictions / #63 brainstorm）待启动，各建独立子节点（1-1-5-2..5），开工前先「TS 源 vs Rust 现状」对账。
