...
| G5 | doctor 9 项健康检查未迁（eval_drift / takes_weight_grid 已迁 2026-07-15） | `crates/zbrain-cli/src/lib.rs:87` `UNMIGRATED_TS_DOCTOR_CHECKS` 常量 + 锚点测试(`unmigrated_ts_doctor_checks_are_anchored` 断言长度 8..=12；每迁出一项加 `xxx_is_no_longer_unmigrated` 守卫) | TS doctor 子系统（embedding_health / sync_freshness / search_mode / federation_health / schema_packs / resolver_health / skill_conformance / frontmatter_integrity / brain_score） | 迁移某检查 = 把其条目移出常量、落地为真实 check（如 `reranker_health` 已迁出；`eval_drift` 已迁出 → `zbrain_core::eval_drift`，Part11 1-5-4 首端口；`takes_weight_grid` 已迁出 → `zbrain_core::takes_fence::check_takes_weight_grid`，Part11 1-5-5 首端口，用 `list_takes` 公共 API 翻页扫描全表、避开 `execute_raw` 逃生舱） | open |
