...
> 数据文件: `zbrain-ts-to-rust-part12-cycle-migration.json` | 最后更新: 2026-08-05 18:05:37

[x][X+] 1. Part12 - cycle 大迁移 (按能力簇切)
...
├── [x][X+] 1-6. orchestration 主循环迁移 (runCycle 2057行 + base-phase/budget-meter/drift/phantom-redirect/phases/; 消费者 dream->runCycle; Rust cycle.rs 仅745行 dispatch 骨架)
...
│   ├── [x][X+] 1-6-2. 周期锁 (per-source cycle lock; busy→skipped/cycle_already_running, 失败→failed/lock_acquisition_error; 复用 sync/lock.rs 基建)
...
│   └── [x][X+] 1-6-8. 消费者切换 (CLI dream 子命令 → Rust run_cycle; --json/--dry-run/--pull/--phase/--dir/--input/--date/--from/--to/--unsafe-bypass-dream-guard; printHuman totals; failed→exit 1; TS cycle.ts+cycle/ 目录+dream.ts 删除)
