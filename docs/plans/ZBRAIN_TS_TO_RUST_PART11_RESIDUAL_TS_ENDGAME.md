...
- [x] 1-6-4-4. features auto-fix 引擎地基 (page-level: list_stale_pages + put_page_embedding + add_timeline_entry + link 解析器)
...
- [x] 1-6-4-8. storage status 命令迁移 (config parser + disk walk + get_storage_status + formatters + CLI 接线)
- [x] 1-6-4-9. publish 命令迁移 (make_shareable + render_markdown + encrypt + generate_html + CLI 接线)
- [x] 1-6-4-10. resolvers 命令 + Resolver SDK 迁移 (interface/registry/url-reachable/x-api + CLI)
