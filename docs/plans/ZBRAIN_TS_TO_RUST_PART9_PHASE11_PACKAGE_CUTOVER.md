...
> 数据文件: `zbrain-ts-to-rust-part9-phase11-package-cutover.json` | 最后更新: 2026-07-15 11:01:33
...
- Q: 第六轮(收尾): D尾巴9文件如何处理? → 重算D只剩9个(前5轮已消化~160全集)。删7个(nightly probe簇: audit-quality-probe/nightly-probe-adapters/nightly-quality-probe + core/index.ts barrel + contextual-reindex-per-chunk handler + 2测试),Rust已覆盖(nightly_probe.rs头注Ported from, registry.rs按名注册reindex)。保留plugin-loader.ts+test: Rust无ZBRAIN_PLUGIN_PATH等价加载器,按AGENTS.md铁律未迁移能力不盲删,登记KNOWN-GAPS G36+加进ts_keep_seeds避免复算噪音。清理死链: test-weights.json2条+contextual-retrieval-service.ts注释。gate: tsc75->64(消失的11error全来自被删nightly-quality-probe.test,零新增)/D重算=0/cargo绿/zbrain --help存活。净删1218行。 (D=0,安全删除全集消化完毕,仅剩G36有意保留)
<!-- ROADMAP_SECTION_END -->
