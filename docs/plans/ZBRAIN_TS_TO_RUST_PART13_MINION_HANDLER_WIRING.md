...
> 数据文件: `zbrain-ts-to-rust-part13-minion-handler-wiring.json` | 最后更新: 2026-08-16
...
│   ├── [x][Y+] 1-3-3. extract handler — 经 G74/G76 1-2-4 已 port（fs 抽取，非全脑）
...
└── [x][Y+] 1-6. G77 图维护族 CLI verb（backfill-links / reconcile-links / edges-backfill）
    ├── [x][Y+] 1-6-1. rebuild-md-links verb (遍历 md 页重跑 markdown link 抽取→upsert page_links)
    ├── [x][Y+] 1-6-2. reconcile-links verb (md→code edges 对账)
