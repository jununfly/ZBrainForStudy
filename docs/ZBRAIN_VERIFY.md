# ZBrain Installation Verification Runbook

Run these checks after install to confirm every part of ZBrain is working.
...
**Expected:** The agent references ZBRAIN_SKILLPACK.md Section 2 and describes
...
install paste (read `docs/ZBRAIN_SKILLPACK.md`).
...
```bash
zbrain check-update --json
```

**Expected:** Returns JSON with `current_version`, `latest_version`,
`update_available` (boolean). The cron `zbrain-update-check` is registered.

**If it fails:** Run step 7 from the install paste. See ZBRAIN_SKILLPACK.md
Section 17.
...
**Expected:** Page count in `zbrain stats` should be close to the file count.
...
- Transaction mode breaks `engine.transaction()` and causes `.begin() is not a
  function` errors.
- Fix: switch to Session mode pooler string, then run `zbrain sync --full`
...
```bash
zbrain search "<text from the correction>"
```
...
- Is `zbrain sync --watch` still alive (if using watch mode)?
- Run `zbrain config get sync.last_run` to see when sync last ran.
- Run `zbrain sync --repo /data/brain` manually and check for errors.
...
**Expected:** The agent uses `zbrain search` or `zbrain query` FIRST, not grep
...
```bash
zbrain stats | grep -E 'links|timeline'
```
...
```bash
zbrain extract links --source db --dry-run | head -5    # preview
zbrain extract links --source db                         # commit
zbrain extract timeline --source db
zbrain stats                                             # confirm > 0
```
...
zbrain graph-query people/<some-person-slug> --depth 2
```
...
(`frontmatter->>'key'` returned NULL, GIN indexes were inert). `zbrain upgrade`
runs `zbrain repair-jsonb` automatically via the `v0_12_2` orchestrator.
Verify the repair succeeded.

**Command:**

```bash
zbrain repair-jsonb --dry-run --json
```
...
```bash
zbrain call list_pages '{"frontmatterKey": "type", "frontmatterValue": "person"}'
```
...
# 2. Sync recency
zbrain config get sync.last_run

# 3. Page count + embed coverage
zbrain stats

# 4. Search works
zbrain search "test query from your brain content"
