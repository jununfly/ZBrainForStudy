# ZBrain v0: Postgres-Native Personal Knowledge Brain
...
ZBrain is a compiled intelligence system. Not a note-taking app. Not "chat with your notes."
...
There's already a production-grade RAG system (Ruby on Rails, Postgres + pgvector) with 3-tier chunking, hybrid search with RRF, multi-query expansion, and 4-layer dedup. ZBrain ports these proven patterns to a standalone Bun + TypeScript tool.
...
**Library-first distribution.** zbrain is an npm package. OpenClaw installs it as a dependency (`bun add zbrain`), imports the engine directly. Zero-overhead function calls, shared connection pool, TypeScript types. The CLI and MCP server are thin wrappers over the same engine.
...
**Auto-embed during import.** No separate embed step. `zbrain import` chunks and embeds in one pass. Progress bar shows status. `--no-embed` flag for users who want to defer. `embedded_at` column enables `zbrain embed --stale` for backfill.
...
+-------------------+     +-------------------+     +-------------------+
|                   |     |                   |     |                   |
| bun add zbrain    |     | GitHub Releases   |     | zbrain serve      |
| import { Postgres |     | npx zbrain        |     | in mcp.json       |
|   Engine }        |     |                   |     |                   |
|                   |     |                   |     |                   |
...
OpenClaw is the AI orchestrator that uses zbrain as its knowledge backend. This is the most common install path.

```bash
# 1. Install zbrain as a ClawHub skill
clawhub install zbrain
...
#   Output: "Brain ready. Run: zbrain import <your-repo>"

# 3. Import your data
zbrain import /path/to/markdown/wiki/
# Progress bar: 7,471 files, auto-chunk, auto-embed
# ~30s for text import, ~10-15 min for embedding

# 4. Query
zbrain query "what does PG say about doing things that don't scale?"
```
...
`zbrain init --supabase` runs through these steps:
...
Step 3: Config
  ├── Write ~/.zbrain/config.json (0600 permissions)
...
Step 5: First Query
  └── "Try: zbrain query 'what does PG say about doing things that don't scale?'"
```
...
```
zbrain init [--supabase|--url <conn>]     # create brain
zbrain get <slug>                          # read a page
zbrain put <slug> [< file.md]             # write/update a page
zbrain search <query>                      # keyword search (tsvector)
zbrain query <question>                    # hybrid search (RRF + expansion)
zbrain ingest <file> [--type ...]         # ingest a source document
zbrain link <from> <to> [--type <type>]   # create typed link
zbrain unlink <from> <to>                 # remove link
zbrain graph <slug> [--depth 5]           # traverse link graph (recursive CTE)
zbrain backlinks <slug>                    # incoming links
zbrain tags <slug>                         # list tags
zbrain tag <slug> <tag>                    # add tag
zbrain untag <slug> <tag>                  # remove tag
zbrain timeline [<slug>]                   # view timeline
zbrain timeline-add <slug> <date> <text>  # add timeline entry
zbrain list [--type] [--tag] [--limit]    # list with filters
zbrain stats                               # brain statistics
zbrain health                              # brain health dashboard
zbrain import <dir> [--no-embed]          # import from markdown directory
zbrain export [--dir ./export/]           # export to markdown (round-trip)
zbrain embed [<slug>|--all|--stale]       # generate/refresh embeddings
zbrain serve                               # MCP server (stdio)
zbrain call <tool> '<json>'               # raw tool invocation
zbrain upgrade                             # self-update (npm, binary, ClawHub)
zbrain version                             # version info
zbrain config [get|set] <key> [value]     # brain config
```
...
2. **Smart slug resolution.** Fuzzy matching via pg_trgm for reads. Writes require exact slugs. `zbrain get "dont scale"` resolves to `concepts/do-things-that-dont-scale`.
3. **Brain health dashboard.** `zbrain health` shows page count, embed coverage, stale pages, orphans, dead links.
4. **Normalized timeline.** `timeline_entries` table only (no TEXT column). `detail` field supports markdown.
5. **Page version control.** `page_versions` table stores full snapshots (compiled_truth + frontmatter + links + tags). `zbrain history`, `zbrain diff`, `zbrain revert` commands. Revert re-chunks and re-embeds.
6. **Typed links + graph traversal.** `link_type` column (knows, invested_in, works_at, etc.). `zbrain graph` uses recursive CTE with max depth (default 5, configurable via `--depth`).
...
- Supabase service role key in `~/.zbrain/config.json` (0600 permissions)
- MCP stdio transport is inherently local (client spawns `zbrain serve` as subprocess)
- No multi-user, no RLS, no OAuth in v0
- Multi-user path (future): Supabase RLS + per-user API keys

## Upgrade mechanism

`zbrain upgrade` detects the installation method and updates accordingly:

| Path | How |
|------|-----|
| npm | `bun update zbrain` (or npm equivalent) |
| Compiled binary | Download new binary to temp dir, atomic rename swap, exec new process |
| ClawHub | `clawhub update zbrain` |
...
Budget alternative: `zbrain import --chunker recursive` skips sentence-level embeddings, then `zbrain embed --rechunk --chunker semantic` upgrades later.
...
1. `zbrain import /data/brain/` migrates all 7,471 files losslessly
2. `zbrain export` round-trips to semantically identical markdown
3. `zbrain query "what does PG say about doing things that don't scale?"` returns relevant hybrid search results
4. `zbrain serve` starts MCP server connectable by Claude Code
5. All 3 chunkers produce correct output with test fixtures
6. `zbrain init --supabase` works end-to-end
7. `bun test` passes all tests
8. `clawhub install zbrain` installs the skill and runs guided setup
9. `bun add zbrain` + `import { PostgresEngine } from 'zbrain'` works in external project
10. Drift tests pass: CLI and MCP produce identical results
11. `zbrain health` outputs accurate brain health metrics
...
- **`zbrain ask` natural language CLI alias.** Trivial to add. P1 TODO.
