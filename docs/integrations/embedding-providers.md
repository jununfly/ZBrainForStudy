...
4. **Submit a recipe**: clone, copy `src/core/ai/recipes/voyage.ts` as the gold-standard openai-compat template, register in `src/core/ai/recipes/index.ts`, add a per-recipe smoke test under `tests/unit/ai/recipe-<name>.test.ts`. The recipe contract test (`tests/unit/ai/recipes-contract.test.ts`) and IRON RULE regression test pin the structural invariants.
